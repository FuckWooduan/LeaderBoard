 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.jna.platform.win32;

/**
 * @author JPEXS
 */
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.jna.platform.win32.WinReg.HKEY;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.jna.platform.win32.WinReg.HKEYByReference;
...
public abstract class Advapi32Util {
    ...
    public static boolean registryKeyExists(HKEY root, String key, boolean use64BitKey) {
        ...
        int rc = Advapi32.INSTANCE.RegOpenKeyEx(
                root, key, 0, WinNT.KEY_READ | (use64BitKey ? 0 : WinNT.KEY_WOW64_32KEY), phkKey);
        switch (rc) {
    ...
    public static boolean registryCreateKey(HKEY hKey, String keyName) {
        ...
        int rc = Advapi32.INSTANCE.RegCreateKeyEx(
                hKey,
                keyName,
                0,
                null,
    ...
    public static void registrySetIntValue(HKEY root, String keyPath, String name, int value) {
        HKEYByReference phkKey = new HKEYByReference();
        int rc = Advapi32.INSTANCE.RegOpenKeyEx(
                root, keyPath, 0, WinNT.KEY_READ | WinNT.KEY_WRITE | WinNT.KEY_WOW64_32KEY, phkKey);
        if (rc != W32Errors.ERROR_SUCCESS) {
            throw new Win32Exception(rc);
        }
        try {
    ...
    public static void registrySetExpandableStringValue(HKEY hKey, String name, String value) {
        char[] data = Native.toCharArray(value);
        int rc = Advapi32.INSTANCE.RegSetValueEx(
                hKey, name, 0, WinNT.REG_EXPAND_SZ, data, data.length * Native.WCHAR_SIZE);
        if (rc != W32Errors.ERROR_SUCCESS) {
            throw new Win32Exception(rc);
        }
    }
    ...
    public static String[] registryGetKeys(HKEY hKey) {
        ...
        int rc = Advapi32.INSTANCE.RegQueryInfoKey(
                hKey, null, null, null, lpcSubKeys, lpcMaxSubKeyLen, null, null, null, null, null, null);
        if (rc != W32Errors.ERROR_SUCCESS) {
            throw new Win32Exception(rc);
        }
    ...
    public static TreeMap<String, Object> registryGetValues(HKEY hKey) {
        ...
        int rc = Advapi32.INSTANCE.RegQueryInfoKey(
                hKey, null, null, null, null, null, null, lpcValues, lpcMaxValueNameLen, lpcMaxValueLen, null, null);
        if (rc != W32Errors.ERROR_SUCCESS) {
            throw new Win32Exception(rc);
        }
