...
public class AntialiasTools {
    ...
    public static void flattenCubicRecursive(
            Vec2 p0, Vec2 p1, Vec2 p2, Vec2 p3, double tol2, List<Vec2> out, int depth) {
    ...
    public static void flattenQuadraticRecursive(Vec2 p0, Vec2 p1, Vec2 p2, double tol2, List<Vec2> out, int depth) {
    ...
    private static PreparedContours prepareContours(List<List<Vec2>> contours, int windingRule, int imageHeight) {
        if (contours == null || contours.isEmpty()) {
            return new PreparedContours(
                    windingRule, new ArrayList<PreparedContour>(), new PreparedEdge[imageHeight][], 0, 0, 0, 0);
        }
        ...
        return new PreparedContours(
                windingRule, preparedContours, arrayBuckets, globalMinX, globalMinY, globalMaxX, globalMaxY);
    }
    ...
    public static final class SceneRasterizerMSAA {
        ...
        public void fillContoursWithPaint(
                List<List<Vec2>> contours, int windingRule, Paint paint, AffineTransform paintTransform) {
            fillContoursWithPaint(contours, windingRule, paint, paintTransform, false, null);
        }

        public void fillContoursWithPaint(
                List<List<Vec2>> contours,
                int windingRule,
                Paint paint,
                AffineTransform paintTransform,
                boolean smooth,
                BufferedImage imagePaint) {
        ...
        private void fillContoursWithPaintImage(List<List<Vec2>> contours, int windingRule, BufferedImage paintImage) {
            ...
            for (int py = minY; py < maxY; py++) {
                for (int px = minX; px < maxX; px++) {
                    ...
                    for (int s = 0; s < sampleCount; s++) {
                        ...
                        if (pointInContours(sx, sy, contours, windingRule)
                                && passesClipFixed(toFixed(sx), toFixed(sy))) {
        ...
        private BufferedImage createPaintImage(
                Paint paint, AffineTransform paintTransform, boolean smooth, BufferedImage textureImage) {
    ...
    static final class PreparedContour {
        ...
        PreparedContour(IVec2[] points, PreparedEdge[] edges, int minX, int minY, int maxX, int maxY) {
            this.points = points;
            this.edges = edges;
            this.minX = minX;
            this.minY = minY;
