...
public class SoundFormat {
    ...
    public byte[] decode(SOUNDINFO soundInfo, List<ByteArrayRange> dataRanges, int skipSamples) throws IOException {
        ...
        for (ByteArrayRange dataRange : dataRanges) {
            SWFInputStream sis =
                    new SWFInputStream(null, dataRange.getArray(), 0, dataRange.getPos() + dataRange.getLength());
        ...
        if (skipSamples > 0) {
            ...
            if (data.length > 0) {
                data = Arrays.copyOfRange(data, skipSamples * 2 * (stereo ? 2 : 1), data.length);
            }
            return data;
        }
            ...
            if (nextI < decodedData.length) {
                ...
                if (resamplingFromStereo) {
                    if (nextI + 3 >= decodedData.length) {
                        ...
                        nextRight =
                                ((decodedData[nextI + 2] & 0xff) + ((decodedData[nextI + 3] & 0xff) << 8)) << 16 >> 16;
                    }
                }
            }
    ...
    public boolean createWav(SOUNDINFO soundInfo, List<ByteArrayRange> dataRanges, OutputStream os, int skipSamples)
            throws IOException {
        ...
        if (soundInfo == null) {
            ...
            int outPoint = (soundInfo.hasOutPoint
                    ? (int) Math.round(soundInfo.outPoint * samplingRate / 44100.0)
                    : Integer.MAX_VALUE);
            baosFiltered = new ByteArrayOutputStream();
            ...
            for (int i = inPointBytes; i < outPointBytes; i += (stereo ? 4 : 2)) {
                ...
                if (soundInfo.hasEnvelope) {
                    for (int e = 0; e < soundInfo.envelopeRecords.length - 1; e++) {
                        int envPosBytes = inPointBytes
                                + (int) (soundInfo.envelopeRecords[e].pos44
                                        * samplingRate
                                        / 44100.0
                                        * 2
                                        * (stereo ? 2 : 1));
                        int envNextPosBytes = inPointBytes
                                + (int) (soundInfo.envelopeRecords[e + 1].pos44
                                        * samplingRate
                                        / 44100.0
                                        * 2
                                        * (stereo ? 2 : 1));
                        if (i >= envPosBytes && i <= envNextPosBytes) {
                            ...
                            int leftLevel = (int) (soundInfo.envelopeRecords[e].leftLevel
                                    + (soundInfo.envelopeRecords[e + 1].leftLevel
                                                    - soundInfo.envelopeRecords[e].leftLevel)
                                            * pos);
                            int rightLevel = (int) (soundInfo.envelopeRecords[e].rightLevel
                                    + (soundInfo.envelopeRecords[e + 1].rightLevel
                                                    - soundInfo.envelopeRecords[e].rightLevel)
                                            * pos);
    ...
    public static void createWavFromPcmData(
            OutputStream fos, int soundRateHz, boolean sample16bit, boolean stereo, byte[] data) throws IOException {
