    ...
    public synchronized GraphTargetItem get(int index) {
        if (path != null) {
            if (index >= this.size() || index < 0) {
                Logger.getLogger(TranslateStack.class.getName())
                        .log(Level.FINE, "{0}: Attempt to Get item outside of bounds of stack", path);
                return getPop();
            }
        }
    ...
    public synchronized GraphTargetItem peek() {
        if (path != null) {
            if (this.isEmpty()) {
                ...
                Logger.getLogger(TranslateStack.class.getName())
                        .log(Level.FINE, "{0}: Attempt to Peek empty stack", path);
                return getPop();
            }
        }
        return super.peek();
    ...
    public synchronized GraphTargetItem peek(int index) {
        if (path != null) {
            if (index > this.size()) {
                ...
                Logger.getLogger(TranslateStack.class.getName())
                        .log(Level.FINE, "{0}: Attempt to Peek item from stack", path);
                return getPop();
            }
        }
    ...
    public synchronized GraphTargetItem pop() {
                ...
                if (item instanceof PushItem) {
                    ...
                    if (pi.value instanceof SetTemporaryItem) {
                        ...
                        return new TemporaryItem(
                                pi.dialect, pi.value.getSrc(), pi.value.getLineStartItem(), pi.value, st.tempIndex);
                    } else if (pi.value instanceof DuplicateSourceItem) {
                        DuplicateSourceItem ds = (DuplicateSourceItem) pi.value;
                        // connectedOutput.set(i, new SetTemporaryItem(pi.dialect, ds.getSrc(), ds.getLineStartItem(),
                        // ds.value, ds.tempIndex,"push"));
                        connectedOutput.remove(i);
                        return ds; // new TemporaryItem(pi.dialect, pi.value.getSrc(), pi.value.getLineStartItem(),
                        // ds.value, ds.tempIndex);
                    } else if (pi.value instanceof DuplicateItem) {
                        DuplicateItem d = (DuplicateItem) pi.value;
                        // connectedOutput.remove(i);
                        // connectedOutput.set(i, new SetTemporaryItem(pi.dialect, d.getSrc(), d.getLineStartItem(),
                        // d.value, d.tempIndex));
                        connectedOutput.remove(i);
                        return new TemporaryItem(
                                pi.dialect, pi.value.getSrc(), pi.value.getLineStartItem(), pi.value, d.tempIndex);
                        ...
                        connectedOutput.set(
                                i,
                                new SetTemporaryItem(
                                        pi.dialect,
                                        pi.value.getSrc(),
                                        pi.value.getLineStartItem(),
                                        pi.value,
                                        temp,
                                        "push",
                                        1));
                        return new TemporaryItem(
                                pi.dialect, pi.value.getSrc(), pi.value.getLineStartItem(), pi.value, temp);
                    }
                }
            }
        }
        ...
        if (path != null) {
            if (this.isEmpty()) {
                ...
                pop = null;
                Logger.getLogger(TranslateStack.class.getName())
                        .log(Level.FINE, "{0}: Attempt to Pop empty stack", path);
                emptyPopCount++;
                return oldpop;
            }
        }
