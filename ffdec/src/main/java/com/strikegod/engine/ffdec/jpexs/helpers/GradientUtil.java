...
public final class GradientUtil {
    ...
    public static RGBA colorAt(RGBA[] colors, int[] ratio, float k, ColorInterpolation mode) {
        ...
        if (mode == ColorInterpolation.SRGB) {
            ...
            return new RGBA(
                    clamp255(Math.round(r * 255f)), clamp255(Math.round(g * 255f)), clamp255(Math.round(b * 255f)), a);
        }
    }
     ...
     */
    public static SplitResult splitIntoHalves(RGBA[] colors, int[] ratio, ColorInterpolation mode) {
     ...
     * SvgGradient.colorAt(...).
     */
    private static Stops extractRange(RGBA[] colors, int[] ratio, int from, int to, ColorInterpolation mode) {
        Stops out = new Stops();
     ...
     */
    private static RGBA stopColorAtOrExisting(RGBA[] colors, int[] ratio, int pos, ColorInterpolation mode) {
        for (int i = 0; i < ratio.length; i++) {
            if (ratio[i] == pos) {
                return colors[i];
            }
