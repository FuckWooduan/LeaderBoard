...
public class ABCSimpleUsageDetector {
    ...
    public void detect() {
        ...
        ABCWalker walker = new ABCWalker() {
            ...
            protected void handleUsageMethodInfo(int index, String usageDescription) {
                ...
                if (m.flagHas_optional()) {
                    for (int i = 0; i < m.optional.length; i++) {
                        handleUsageValueKind(
                                abc,
                                m.optional[i].value_kind,
                                m.optional[i].value_index,
                                usageDescription + "/optional/op" + i);
                    }
                }
            ...
            protected void handleUsageMethodBody(int index, String usageDescription) {
                ...
                for (int i = 0; i < body.exceptions.length; i++) {
                    handleUsageMultiname(
                            body.exceptions[i].name_index, usageDescription + "/exceptions/ex" + i + "/name");
                    handleUsageMultiname(
                            body.exceptions[i].type_index, usageDescription + "/exceptions/ex" + i + "/type");
                }
            ...
            @Override
            protected void handleMetadataInfo(
                    ABC abc,
                    int index,
                    Trait trait,
                    int scriptIndex,
                    int scriptTraitIndex,
                    int classIndex,
                    int traitIndex,
                    int traitMetadataIndex,
                    ABCWalker.WalkType walkType) {
            ...
            @Override
            protected void handleTraitMethodGetterSetter(
                    ABC abc,
            ...
            protected void handleTraitMethodBase(
                    ABC abc,
                    Trait trait,
                    int scriptIndex,
                    int scriptTraitIndex,
            ...
            @Override
            protected void handleTraitFunction(
                    ABC abc,
                    TraitFunction trait,
                    int scriptIndex,
                    int scriptTraitIndex,
            ...
            @Override
            protected void handleTraitSlotConst(
                    ABC abc,
            ...
            @Override
            protected void handleMethodInfo(
                    ABC abc,
                    int index,
                    int scriptIndex,
                    int scriptTraitIndex,
                ...
                if (initializer) {
                    switch (walkType) {
                        case Class:
                            if (scriptIndex != -1) {
                                handleUsageMethodInfo(
                                        index,
                                        "si" + scriptIndex + "/traits/t" + scriptTraitIndex + "/class_info/cinit");
                            } else {
                        ...
                        case Instance:
                            if (scriptIndex != -1) {
                                handleUsageMethodInfo(
                                        index,
                                        "si" + scriptIndex + "/traits/t" + scriptTraitIndex + "/instance_info/iinit");
                            } else {
        ...
        for (ItemKind kind : usages.keySet()) {
            ...
            for (int i = kind.hasReservedZeroIndex() ? 1 : 0;
                    i < usages.get(kind).size();
                    i++) {
    ...
    private boolean handleUsage(ItemKind kind, int index, String usageDescription) {
        if (!usages.containsKey(kind) || index < 0 || index > usages.get(kind).size() - 1) {
            Logger.getLogger(ABCSimpleUsageDetector.class.getName())
                    .log(Level.WARNING, "{0} with index {1} not found for usage {2}", new Object[] {
                        kind, index, usageDescription
                    });
            return false;
        }
