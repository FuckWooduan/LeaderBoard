...
import com.strikegod.engine.ffdec.jpexs.helpers.ConcreteClasses;
...
public abstract class SHAPERECORD implements Cloneable, NeedsCharacters, Serializable {
    ...
    public static RECT getBounds(
            List<SHAPERECORD> records, LINESTYLEARRAY lineStyles, int shapeNum, boolean edgeBounds) {
        int x = 0;
        int y = 0;
        int max_x = 0;
        int max_y = 0;
    ...
    public static void shapeListToImage(
            int windingRule,
            int shapeNum,
            SWF swf,
            List<SHAPE> shapes,
        ...
        for (int y = 0; y < cols; y++) {
            for (int x = 0; x < cols; x++) {
                ...
                if (DRAW_BOUNDING_BOX) {
                    ...
                    transformation = Matrix.getTranslateInstance(bounds.Xmin, bounds.Ymin)
                            .preConcatenate(transformation);
                    TextTag.drawBorder(
                            swf,
                            image,
                            borderColor,
            ...
            switch (type) {
                ...
                case PathIterator.SEG_CUBICTO:
                    ...
                    for (int i = 0; i < quadCoords.length; i++) {
                        ...
                        cer.anchorDeltaX = multiplier
                                * (((int) Math.round(quadCoords[i][2])) - ((int) Math.round(quadCoords[i][0])));
                        cer.anchorDeltaY = multiplier
                                * (((int) Math.round(quadCoords[i][3])) - ((int) Math.round(quadCoords[i][1])));
                        cer.calculateBits();
    ...
    public static Shape twipToPixelShape(Shape s) {
        ...
        s = AffineTransform.getScaleInstance(1 / SWF.unitDivisor, 1 / SWF.unitDivisor)
                .createTransformedShape(s);
        s = AffineTransform.getTranslateInstance(-dx / SWF.unitDivisor, -dy / SWF.unitDivisor)
                .createTransformedShape(s);
        return s;
    }
