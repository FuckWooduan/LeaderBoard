...
public class ValueKind {
    ...
    private static final int[] kinds = new int[] {
        0x03, 0x04, 0x06, 0x02, 0x01, 0x0B, 0x0A, 0x0C, 0x00, 0x08, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x05, 0x1E
    };

    /**
     * Kind names
     */
    private static final String[] kindNames = new String[] {
        "Int",
        "UInt",
        "Double",
        "Decimal/Float",
    ...
    public String toString(ABC abc) {
        ...
        switch (value_kind) {
            ...
            case CONSTANT_PrivateNs:
                ret = "\""
                        + abc.constants
                                .getNamespace(value_index)
                                .getName(abc.constants)
                                .toRawString() + "\""; // assume not null name
                break;
        }
        return ret;
    }
    ...
    public String toASMString(ABC abc) {
        ...
        switch (value_kind) {
            ...
            case CONSTANT_StaticProtectedNs:
            case CONSTANT_PrivateNs:
                String nsVal = abc.constants.getNamespace(value_index).getKindStr() + "(\""
                        + abc.constants
                                .getNamespace(value_index)
                                .getName(abc.constants)
                                .toRawString() + "\")"; // assume not null name
