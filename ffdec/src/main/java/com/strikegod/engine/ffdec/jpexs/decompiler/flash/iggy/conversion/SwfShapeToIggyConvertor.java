    ...
    public static IggyShape convertShape(int shapeNum, SHAPE swfShape) {
        ...
        for (SHAPERECORD rec : swfShape.shapeRecords) {
            if (rec instanceof StraightEdgeRecord) {
                ...
                nodes.add(new IggyShapeNode(
                        curX, curY, controlX, controlY, IggyShapeNode.NODE_TYPE_CURVE_POINT, 0, first));
        ...
        IggyShape ret = new IggyShape(
                normalizeLengths(bounds.Xmin),
                normalizeLengths(bounds.Ymin),
                normalizeLengths(bounds.Xmax),
                normalizeLengths(bounds.Ymax),
                nodes);
        return ret;
    }
}
