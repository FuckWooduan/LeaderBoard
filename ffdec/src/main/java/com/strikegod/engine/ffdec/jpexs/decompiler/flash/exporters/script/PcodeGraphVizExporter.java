...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.action.ActionGraph;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.script.graphviz.AbstractLexer;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.script.graphviz.Flasm3Lexer;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.script.graphviz.FlasmLexer;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.script.graphviz.Token;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.script.graphviz.TokenType;
...
public class PcodeGraphVizExporter {
     ...
     * @throws InterruptedException On interrupt
     */
    public void exportAs3(AbcIndexing abcIndex, ABC abc, MethodBody body, GraphTextWriter writer)
            throws InterruptedException {
        AVM2Graph gr = new AVM2Graph(
                -1,
                abcIndex,
                body.getCode(),
                abc,
                ...
                new HashMap<>(),
                new ArrayList<>(),
                new HashMap<>()); // body.getCode().visitCode(body));
        export(gr, writer);
    }
    ...
    private void exportGraph(Graph graph, GraphTextWriter writer) throws InterruptedException {
        ...
        for (int e = 0; e < exceptions.size(); e++) {
            ...
            writer.append("ex_" + e + "_begin:se -> " + getBlockName(graphSource, exStartPart)
                    + ":nw [style=dashed arrowhead=none];\r\n");
            ...
            if (exEndPart != null) {
                ...
                writer.append(getBlockName(graphSource, exEndPart) + ":sw -> ex_" + e
                        + "_end:ne [style=dashed dir=back arrowtail=none];\r\n");
            }
        }
        ...
        for (GraphPart part : allBlocks) {
            ...
            for (int j = part.start; j <= part.end; j++) {
                if (j < graphSource.size()) {
                    if (knownAddresses.contains(graphSource.get(j).getAddress())) {
                        blkCodeBuilder
                                .append("ofs")
                                .append(Helper.formatAddress(graphSource.get(j).getAddress()))
                                .append(":\r\n");
                    }
            ...
            writer.append(partBlockName + " [" + blkStyle
                    + " fontname=\"Courier New\" label=<<TABLE BORDER=\"0\" CELLBORDER=\"0\" CELLSPACING=\"0\"><TR><TD BALIGN=\"LEFT\">"
                    + labelStr + "</TD></TR></TABLE>>];\r\n");
