 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.docs;

import com.strikegod.engine.ffdec.jpexs.decompiler.flash.ApplicationInfo;
...
public class As3PCodeDocs extends AbstractDocs {
    ...
    static {
        prop = ResourceBundle.getBundle("com.strikegod.engine.ffdec.jpexs.decompiler.flash.locales.docs.pcode.AS3");
     ...
     */
    public static String getDocsForIns(
            String insName,
            boolean showDataSize,
            boolean ui,
            boolean withStyle,
            boolean nightMode,
            int argumentToHilight) {
    ...
    public static String getDocsForIns(
        ...
        final String cacheKey =
                def.instructionName + "|" + (showDataSize ? 1 : 0) + "|" + (ui ? 1 : 0) + "|" + (standalone ? 1 : 0);
        ...
        String stackBefore = def.hasFlag(AVM2InstructionFlag.UNKNOWN_STACK)
                ? ""
                : getProperty("instruction." + insName + ".stackBefore");
        String stackAfter = def.hasFlag(AVM2InstructionFlag.UNKNOWN_STACK)
                ? ""
                : getProperty("instruction." + insName + ".stackAfter");
        ...
        String stack = def.hasFlag(AVM2InstructionFlag.UNKNOWN_STACK)
                ? getProperty("ui.unknown")
                : stackBefore + "<span class=\"stack-to\">" + getProperty("ui.stack.to") + "</span>" + stackAfter;
        String operandsDoc = def.hasFlag(AVM2InstructionFlag.UNKNOWN_OPERANDS)
                ? getProperty("ui.unknown")
                : getProperty("instruction." + insName + ".operands");
        ...
        sb.append("<div class=\"instruction-signature\"><span class=\"instruction-code\">")
                .append(String.format("0x%02X", def.instructionCode))
                .append("</span> <strong class=\"instruction-name\">")
                .append(insName)
                .append("</strong>");
        ...
        sb.append("<div class=\"short-description\">")
                .append(insShortDescription)
                .append("</div>")
                .append(NEWLINE);

        if (!insDescription.trim().isEmpty()) {
            sb.append("<div class=\"description\">")
                    .append("<strong class=\"description-title\">")
                    .append(getProperty("ui.description"))
                    .append("</strong>")
                    .append(insDescription)
                    .append("</div>")
                    .append(NEWLINE);
        }
        sb.append("<div class=\"stack\"><strong class=\"stack-title\">")
                .append(getProperty("ui.stack"))
                .append("</strong><span class=\"stack-values "
                        + (def.hasFlag(AVM2InstructionFlag.UNKNOWN_STACK) ? " unknown" : "") + "\">")
                .append(stack)
                .append("</span>")
                .append("</div>")
                .append(NEWLINE);
        ...
        for (AVM2InstructionFlag fl : flags) {
            if (!flagsPrinted) {
                flagsPrinted = true;
                sb.append("<strong class=\"flags-title\">")
                        .append(getProperty("ui.flags"))
                        .append("</strong>")
                        .append("<br />")
                        .append(NEWLINE)
                        .append("<ul class=\"flags\">")
                        .append(NEWLINE);
            }
            sb.append("\t<li class=\"flag flag-")
                    .append(makeIdent(fl.toString()))
                    .append("\">")
                    .append(flagDescriptions.get(fl));
            if (fl == AVM2InstructionFlag.DEPRECATED) {
                String depDetail = getProperty("instruction." + insName + ".deprecated");
                if (depDetail != null) {
                    sb.append(": <span class=\"flag-deprecated-detail\">")
                            .append(depDetail)
                            .append("</span>");
                }
            }
    ...
    public static String getJs() {
        ...
        String js = "";
        InputStream is = As3PCodeDocs.class.getResourceAsStream(
                "/com/strikegod/engine/ffdec/jpexs/decompiler/flash/docs/docs.js");
        if (is == null) {
            Logger.getLogger(As3PCodeDocs.class.getName())
                    .log(Level.SEVERE, "docs.js needed for documentation not found");
        } else {
    ...
    public static String getAllInstructionDocs(boolean nightMode) {
        ...
        for (AVM2InstructionFlag f : AVM2InstructionFlag.values()) {
            jsData += "flags[\"" + makeIdent(f.toString()) + "\"] = \""
                    + Helper.escapeJavaString(flagDescriptions.get(f)) + "\";" + NEWLINE;
        ...
        sb.append(">");
        sb.append("\t\t<h1>")
                .append(getProperty("ui.list.heading"))
                .append("</h1>")
                .append(NEWLINE);
        ...
        for (String name : s) {
            ...
            sb.append("\t\t\t\t")
                    .append(getDocsForIns(def, true, false, false, nightMode, -1)
                            .trim()
    ...
    protected static String htmlHeader(String js, String style, boolean nightMode) {
        ...
        sb.append("\t\t<meta charset=\"UTF-8\">")
                ...
                .append("\t\t<title>")
                .append(getProperty("ui.list.documentTitle"))
                .append("</title>")
                .append(NEWLINE)
                .append("\t</head>")
                .append(NEWLINE);
