...
public class CubicToQuad {
    ...
    private double minDistanceToQuad(Point point, Point p1, Point c1, Point p2) {
        ...
        for (int i = 0; i < candidates.size(); i++) {
            double distance =
                    calcPointQuad(a, b, c, candidates.get(i)).sub(point).dist();
     ...
     *  - errorBound is maximum allowed distance
     */
    private boolean isSegmentApproximationClose(
            Point a,
            Point b,
            Point c,
            Point d,
    ...
    private boolean _isApproximationClose(
            Point a, Point b, Point c, Point d, List<Point[]> quadCurves, double errorBound) {
    ...
    private boolean isApproximationClose(
            double p1x,
            double p1y,
            double c1x,
            double c1y,
        ...
        Point[] pc = calcPowerCoefficients(
                new Point(p1x, p1y), new Point(c1x, c1y), new Point(c2x, c2y), new Point(p2x, p2y));
     ...
     */
    private double[][] subdivideCubic(
            double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4, double t) {
        double u = 1 - t;
        double v = t;
     ...
     */
    public List<Double> cubicToQuad(
            double p1x,
            double p1y,
            double c1x,
            double c1y,
     ...
     * @return List of quadratic Bezier curve points
     */
    private List<Double> _cubicToQuad(
            double p1x,
            double p1y,
            double c1x,
            double c1y,
        ...
        for (int segmentsCount = 1; segmentsCount <= 8; segmentsCount++) {
            ...
            if (segmentsCount == 1
                    && (approximation.get(0)[1].sub(p1).dot(c1.sub(p1)) < 0
     ...
     */
    private List<Double> solveInflections(
            double x1, double y1, double x2, double y2, double x3, double y3, double x4, double y4) {
        double p = -(x4 * (y1 - 2 * y2 + y3))
                + x3 * (2 * y1 - 3 * y2 + y4)
                + x1 * (y2 - 2 * y3 + y4)
                - x2 * (y1 - 3 * y3 + 2 * y4);
