 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.instructions;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.LocalDataArea;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.model.ClassAVM2Item;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.model.DecrementAVM2Item;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.model.IncrementAVM2Item;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.types.traits.TraitWithSlot;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.graph.model.DuplicateSourceItem;
...
public abstract class InstructionDefinition implements Serializable {
     ...
     */
    public void verify(LocalDataArea lda, AVM2ConstantPool constants, AVM2Instruction ins)
            ...
            if (operand == AVM2Code.DAT_MULTINAME_INDEX) {
                ...
                if (idx <= 0 || idx >= constants.getMultinameCount()) {
                    throw new AVM2VerifyErrorException(
                            AVM2VerifyErrorException.CPOOL_INDEX_OUT_OF_RANGE,
                            lda.isDebug(),
                            new Object[] {idx, constants.getMultinameCount()});
                }
                ...
                if (idx <= 0 || idx >= constants.getDoubleCount()) {
                    throw new AVM2VerifyErrorException(
                            AVM2VerifyErrorException.CPOOL_INDEX_OUT_OF_RANGE,
                            lda.isDebug(),
                            new Object[] {idx, constants.getDoubleCount()});
                }
                ...
                if (idx <= 0 || idx >= constants.getIntCount()) {
                    throw new AVM2VerifyErrorException(
                            AVM2VerifyErrorException.CPOOL_INDEX_OUT_OF_RANGE,
                            lda.isDebug(),
                            new Object[] {idx, constants.getIntCount()});
                }
                ...
                if (idx <= 0 || idx >= constants.getUIntCount()) {
                    throw new AVM2VerifyErrorException(
                            AVM2VerifyErrorException.CPOOL_INDEX_OUT_OF_RANGE,
                            lda.isDebug(),
                            new Object[] {idx, constants.getUIntCount()});
                }
                ...
                if (idx <= 0 || idx >= constants.getStringCount()) {
                    throw new AVM2VerifyErrorException(
                            AVM2VerifyErrorException.CPOOL_INDEX_OUT_OF_RANGE,
                            lda.isDebug(),
                            new Object[] {idx, constants.getStringCount()});
                }
    ...
    protected void illegalOpCode(LocalDataArea lda, AVM2Instruction ins) throws AVM2VerifyErrorException {
        throw new AVM2VerifyErrorException(AVM2VerifyErrorException.ILLEGAL_OPCODE, lda.isDebug(), new Object[] {
            lda.methodName, instructionCode, ins.getAddress()
        });
    }
    ...
    public void translate(
            ...
            LinkedIdentityHashSet<SetLocalAVM2Item> bottomSetLocals)
            throws InterruptedException {
     ...
     */
    protected FullMultinameAVM2Item resolveMultiname(
            AVM2LocalData localData,
            boolean property,
            TranslateStack stack,
        ...
        return new FullMultinameAVM2Item(
                property,
                ins,
     ...
     */
    protected static Multiname searchSlotName(
            int slotIndex, AVM2LocalData localData, GraphTargetItem obj, Reference<GraphTargetItem> realObj) {
     ...
     * @return Slot multiname or null if not found
     */
    private static Multiname searchSlotName(
            int slotIndex,
            AVM2LocalData localData,
            GraphTargetItem obj,
            int multiNameIndex,
            Reference<GraphTargetItem> realObj) {
        ...
        if ((obj instanceof ExceptionAVM2Item)
                && (multiNameIndex == -1 || ((ExceptionAVM2Item) obj).exception.name_index == multiNameIndex)) {
        ...
        if (obj instanceof FindPropertyAVM2Item) {
            ...
            for (GraphTargetItem item : localData.localScopeStack) {
                Multiname ret = searchSlotName(
                        slotIndex,
                        localData,
                        item,
                        ((FullMultinameAVM2Item) findProp.propertyName).multinameIndex,
                        realObj);
                if (ret != null) {
                    return ret;
                }
    ...
    public void handleSetProperty(
        ...
        FullMultinameAVM2Item multiname =
                resolveMultiname(localData, true, stack, localData.getConstants(), multinameIndex, ins, output);
        ...
        */
        if (value instanceof IncrementAVM2Item || value instanceof DecrementAVM2Item) {
            ...
            if (value.value instanceof GetPropertyAVM2Item) {
                ...
                if (getProp.object instanceof DuplicateItem && obj instanceof DuplicateSourceItem) {
                    if (getProp.object.getThroughDuplicate() == obj.getThroughDuplicate()) {
                        ...
                        if (!output.isEmpty() && output.get(output.size() - 1) instanceof SetTemporaryItem) {
                            ...
                            if (st.tempIndex == d.tempIndex) {
                                ...
                                if (isIncrement) {
                                    stack.addToOutput(new PreIncrementAVM2Item(
                                            value.getSrc(), value.lineStartItem, getProp, value.value.returnType()));
                                } else {
                                    stack.addToOutput(new PreDecrementAVM2Item(
                                            value.getSrc(), value.lineStartItem, getProp, value.value.returnType()));
                                }
                                return;
                            }
                        ...
                        if (getProp.object.getThroughDuplicate() == obj.getThroughDuplicate()) {
                            ...
                            if (!output.isEmpty() && output.get(output.size() - 1) instanceof SetTemporaryItem) {
                                ...
                                if (st.tempIndex == d.tempIndex) {
                                    ...
                                    getProp.object = st.value;
                                    if (isIncrement) {
                                        stack.addToOutput(new PostIncrementAVM2Item(
                                                value.getSrc(), value.lineStartItem, getProp, TypeItem.NUMBER));
                                    } else {
                                        stack.addToOutput(new PostDecrementAVM2Item(
                                                value.getSrc(), value.lineStartItem, getProp, TypeItem.NUMBER));
                                    }
                                    return;
                                }
                                ...
                                if (getProp.propertyName instanceof FullMultinameAVM2Item) {
                                    ...
                                    if (fm.name instanceof SetLocalAVM2Item) {
                                        ...
                                        if (valueLocalReg.regIndex == setLocValue.regIndex
                                            ...
                                            stack.pop();
                                            if (isIncrement) {
                                                stack.push(new PreIncrementAVM2Item(
                                                        setLocValue.value.getSrc(),
                                                        setLocValue.value.getLineStartItem(),
                                                        getProp,
                                                        TypeItem.NUMBER));
                                            } else {
                                                stack.push(new PreDecrementAVM2Item(
                                                        setLocValue.value.getSrc(),
                                                        setLocValue.value.getLineStartItem(),
                                                        getProp,
                                                        TypeItem.NUMBER));
                        ...
                        if (setLocValue.value.value instanceof DuplicateItem) {
                            ...
                            if (output.size() >= 2
                                ...
                                if (ds.tempIndex == d.tempIndex) {
                                    if (output.size() >= 3
                                            && output.get(output.size() - 3) instanceof SetTemporaryItem) {
                                                        ...
                                                        if (getProp.propertyName instanceof FullMultinameAVM2Item) {
                                                            ...
                                                            if (fm.name instanceof SetLocalAVM2Item) {
                                                                SetLocalAVM2Item setLocName =
                                                                        (SetLocalAVM2Item) fm.name;
                                                                if (valueLocalReg.regIndex == setLocValue.regIndex
                                                                        && objLocalReg.regIndex == setLocObj.regIndex
                                                                        && nameLocalReg.regIndex
                                                                                == setLocName.regIndex) {
                                                                    getProp.object = setLocObj.value;
                                                                    fm.name = setLocName.value;
                                                                    output.remove(output.size() - 1);
                                                                    ...
                                                                    stack.moveToStack(output);
                                                                    if (isIncrement) {
                                                                        stack.push(new PostIncrementAVM2Item(
                                                                                setLocValue.value.getSrc(),
                                                                                setLocValue.value.getLineStartItem(),
                                                                                getProp,
                                                                                TypeItem.NUMBER));
                                                                    } else {
                                                                        stack.push(new PostDecrementAVM2Item(
                                                                                setLocValue.value.getSrc(),
                                                                                setLocValue.value.getLineStartItem(),
                                                                                getProp,
                                                                                TypeItem.NUMBER));
                                                        ...
                                                        if (getProp.propertyName instanceof FullMultinameAVM2Item) {
                                                            ...
                                                            if (fm.compareSame(multiname)) {
                                                                if (objLocalReg.regIndex == setLocObj.regIndex
                                                                        && valueLocalReg.regIndex
                                                                                == setLocValue.regIndex) {
                                                                    getProp.object = setLocObj.value;
                                                                    output.remove(output.size() - 1);
                                                                    output.remove(output.size() - 1);
                                                                    output.remove(output.size() - 1);
            ...
            if (!stack.isEmpty() && stack.peek() instanceof DuplicateSourceItem) {
                ...
                if (d.tempIndex == ds.tempIndex) {
                    if (!output.isEmpty() && output.get(output.size() - 1) instanceof SetTemporaryItem) {
                        SetTemporaryItem st = (SetTemporaryItem) output.get(output.size() - 1);
                        if (st.tempIndex == d.tempIndex) {
                            if (st.value instanceof IncrementAVM2Item || st.value instanceof DecrementAVM2Item) {
            ...
            if (value.value instanceof GetPropertyAVM2Item) {
                ...
                if (getProp.propertyName instanceof FullMultinameAVM2Item) {
                    ...
                    if (fm.compareSame(multiname)) {
                        if (isIncrement) {
                            stack.addToOutput(new PreIncrementAVM2Item(
                                    value.getSrc(), value.getLineStartItem(), getProp, TypeItem.NUMBER));
                        } else {
                            stack.addToOutput(new PreDecrementAVM2Item(
                                    value.getSrc(), value.getLineStartItem(), getProp, TypeItem.NUMBER));
                        }
        ...
        */
        if (value instanceof IncrementAVM2Item || value instanceof DecrementAVM2Item) {
            ...
            if (value.value instanceof DuplicateItem) {
                ...
                if (!output.isEmpty()
                        && output.get(output.size() - 1) instanceof PushItem
            ...
            if (ce.commands.size() == 2) {
                if (ce.commands.get(0) instanceof SetLocalAVM2Item && ce.commands.get(1) instanceof LocalRegAVM2Item) {
                    ...
                    if (setLocal.regIndex == localReg.regIndex) {
                        ...
                        if (src != null) {
                            if (localData
                                            .getSetLocalUsages(localData.code.adr2pos(src.getAddress()))
                                            .size()
                                    == 1) {
        ...
        if (init) {
            result = new InitPropertyAVM2Item(
                    ins,
                    localData.lineStartInstruction,
                    obj,
                    multiname,
            ...
            result = new SetPropertyAVM2Item(
                    ins,
                    localData.lineStartInstruction,
                    obj,
                    multiname,
     ...
     */
    private GraphTargetItem checkIncDec(
            boolean standalone,
            int multinameIndex,
            AVM2Instruction ins,
            AVM2LocalData localData,
            GraphTargetItem item,
            LocalRegAVM2Item valueLocalReg,
            LocalRegAVM2Item nameLocalReg,
            LocalRegAVM2Item objLocalReg) {
        if (item instanceof SetLocalAVM2Item) {
            SetLocalAVM2Item valueSetLocalReg = (SetLocalAVM2Item) item;
            if ((valueSetLocalReg.value instanceof IncrementAVM2Item)
                    || (valueSetLocalReg.value instanceof DecrementAVM2Item)) {
                boolean isIncrement = (valueSetLocalReg.value instanceof IncrementAVM2Item);
                boolean hasConvert = valueSetLocalReg.value.value
                        instanceof ConvertAVM2Item; // in air there is convert added when postincrement

                if (valueSetLocalReg.value.value.getNotCoercedNoDup() instanceof GetPropertyAVM2Item) {
                    GetPropertyAVM2Item getProperty =
                            (GetPropertyAVM2Item) valueSetLocalReg.value.value.getNotCoercedNoDup();
                    FullMultinameAVM2Item propertyName = ((FullMultinameAVM2Item) getProperty.propertyName);
                    SetLocalAVM2Item nameSetLocalReg = null;
                    if (propertyName.name instanceof SetLocalAVM2Item) {
                        nameSetLocalReg = (SetLocalAVM2Item) propertyName.name;
                    ...
                    if (getProperty.object.getThroughDuplicate() instanceof SetLocalAVM2Item) {
                        ...
                        if ((valueLocalReg.regIndex == valueSetLocalReg.regIndex)
                                && (propertyName.multinameIndex == multinameIndex)
                                && ((nameLocalReg == null && nameSetLocalReg == null)
                                        || (nameLocalReg != null
                                                && nameSetLocalReg != null
                                                && nameLocalReg.regIndex == nameSetLocalReg.regIndex))
                                && (objLocalReg.regIndex == objSetLocalReg.regIndex)) {
                            if (nameSetLocalReg != null) {
                                propertyName.name = nameSetLocalReg.value;
                            }
                            ...
                            if (isIncrement) {
                                if (hasConvert && standalone) {
                                    return new PostIncrementAVM2Item(
                                            ins, localData.lineStartInstruction, getProperty, TypeItem.NUMBER);
                                }
                                // TestIncDec3 with result
                                return new PreIncrementAVM2Item(
                                        ins, localData.lineStartInstruction, getProperty, TypeItem.NUMBER);
                            } else {
                                if (hasConvert && standalone) {
                                    return new PostDecrementAVM2Item(
                                            ins, localData.lineStartInstruction, getProperty, TypeItem.NUMBER);
                                }
                                // TestIncDec3 with result
                                return new PreDecrementAVM2Item(
                                        ins, localData.lineStartInstruction, getProperty, TypeItem.NUMBER);
                            }
                        }
                    }
