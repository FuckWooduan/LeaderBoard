...
public class ShapeTransformer {
        ...
        for (FILLSTYLE fs : fillStyleToTransform) {
            switch (fs.fillStyleType) {
                ...
                case FILLSTYLE.REPEATING_BITMAP:
                    fs.bitmapMatrix =
                            new Matrix(fs.bitmapMatrix).preConcatenate(matrix).toMATRIX();
                    break;
                case FILLSTYLE.LINEAR_GRADIENT:
                case FILLSTYLE.RADIAL_GRADIENT:
                case FILLSTYLE.FOCAL_RADIAL_GRADIENT:
                    fs.gradientMatrix =
                            new Matrix(fs.gradientMatrix).preConcatenate(matrix).toMATRIX();
                    break;
            }
        }
    }
     ...
     */
    public void transformMorphStyles(
            Matrix matrix,
            MORPHFILLSTYLEARRAY fillStyles,
            MORPHLINESTYLEARRAY lineStyles,
            int morphShapeNum,
            boolean doStart,
            boolean doEnd) {
        ...
        for (MORPHFILLSTYLE fs : fillStyleToTransform) {
            switch (fs.fillStyleType) {
                ...
                case FILLSTYLE.REPEATING_BITMAP:
                    if (doStart) {
                        fs.startBitmapMatrix = new Matrix(fs.startBitmapMatrix)
                                .preConcatenate(matrix)
                                .toMATRIX();
                    }
                    if (doEnd) {
                        fs.endBitmapMatrix = new Matrix(fs.endBitmapMatrix)
                                .preConcatenate(matrix)
                                .toMATRIX();
                    }
                    break;
                ...
                case FILLSTYLE.FOCAL_RADIAL_GRADIENT:
                    if (doStart) {
                        fs.startGradientMatrix = new Matrix(fs.startGradientMatrix)
                                .preConcatenate(matrix)
                                .toMATRIX();
                    }
                    if (doEnd) {
                        fs.endGradientMatrix = new Matrix(fs.endGradientMatrix)
                                .preConcatenate(matrix)
                                .toMATRIX();
                    }
                    break;
