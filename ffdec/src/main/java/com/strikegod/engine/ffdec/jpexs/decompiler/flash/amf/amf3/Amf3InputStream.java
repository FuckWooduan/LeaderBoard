 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3;

import com.strikegod.engine.ffdec.jpexs.decompiler.flash.EndOfStreamException;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.types.ArrayType;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.types.BasicType;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.types.ByteArrayType;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.types.DateType;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.types.DictionaryType;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.types.ObjectType;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.types.VectorDoubleType;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.types.VectorIntType;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.types.VectorObjectType;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.types.VectorUIntType;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.types.XmlDocType;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.types.XmlType;
...
public class Amf3InputStream extends InputStream {
    ...
    private void renameLastDump(String wholeName) {
        if (dumpInfo != null) {
            if (!dumpInfo.getChildInfos().isEmpty()) {
                DumpInfo u29DumpInfo =
                        dumpInfo.getChildInfos().get(dumpInfo.getChildInfos().size() - 1);
        ...
        if (dumpInfo != null) {
            if (!dumpInfo.getChildInfos().isEmpty()) {
                DumpInfo u29DumpInfo =
                        dumpInfo.getChildInfos().get(dumpInfo.getChildInfos().size() - 1);
                u29DumpInfo.name = wholeName;
                long lastBytePos = u29DumpInfo.startByte
                        + u29DumpInfo.lengthBytes
                        - 1; // last byte of U29 is least significant (U29 is big endian)

                int remainingBitLength = ((int) u29DumpInfo.lengthBytes) * 8 - bitNames.size();
                long u29val = ((long) (Long) u29DumpInfo.previewValue);

                DumpInfo restDumpInfo = new DumpInfo(
                        restName,
                        "UB(" + remainingBitLength + ")",
                        u29val >> bitNames.size(),
                        lastBytePos,
                        0,
                        u29DumpInfo.lengthBytes,
                        remainingBitLength);
    ...
    public Object readValue(
            String name,
            Map<String, ObjectTypeSerializeHandler> serializers,
            ...
            switch (marker) {
                ...
                case Marker.ARRAY:
                    ...
                    if (arrayNoRefFlag == 1) {
                        ...
                        for (int i = 0; i < denseCount; i++) {
                            try {
                                densePart.add(
                                        readValue("denseValue", serializers, objectTable, traitsTable, stringTable));
                            } catch (NoSerializerExistsException nse) {
                        ...
                        endDumpLevel();
                        LOGGER.log(Level.FINER, "Array value: dense_size={0},assocSize={1}", new Object[] {
                            densePart.size(), assocPart.size()
                        });
                        result = retArray;
                ...
                case Marker.OBJECT:
                    ...
                    if (objectNoRefFlag == 1) {
                        ...
                        if (objectTraitsNoRefFlag == 1) {
                            ...
                            if (objectTraitsExtFlag == 1) {
                                ...
                                if (!serializers.containsKey(className)) {
                                    throw new NoSerializerExistsException(
                                            className,
                                            new ObjectType(
                                                    new Traits(className, false, new ArrayList<>()),
                                                    (byte[]) null,
                                                    new HashMap<>()),
                                            null);
                                }
                                ...
                                Map<String, Object> serMembers =
                                        serializers.get(className).readObject(className, mis);
                            ...
                            LOGGER.log(Level.FINER, "Traits value: reference({0}) - traitsize={1}", new Object[] {
                                refIndexTraits, traits.getSealedMemberNames().size()
                            });
                        }
                        ...
                        if (!traits.getSealedMemberNames().isEmpty()) {
                            ...
                            for (int i = 0; i < traits.getSealedMemberNames().size(); i++) {
                                try {
                                    sealedMemberValues.add(readValue(
                                            "sealedMemberValue", serializers, objectTable, traitsTable, stringTable));
                                } catch (NoSerializerExistsException nse) {
                                    sealedMemberValues.add(nse.getIncompleteData());
                                    for (int j = i + 1;
                                            j < traits.getSealedMemberNames().size();
                                            j++) {
                        ...
                        if (traits.isDynamic()) {
                            ...
                            while (!(dynamicMemberName = readUtf8Vr("name", stringTable)).isEmpty()) {
                                try {
                                    Object dynamicMemberValue =
                                            readValue("value", serializers, objectTable, traitsTable, stringTable);
                                    ...
                                    if (dumpInfo != null) {
                                        DumpInfo valueDumpInfo = dumpInfo.getChildInfos()
                                                .remove(dumpInfo.getChildInfos().size() - 1);
                                        DumpInfo nameDumpInfo = dumpInfo.getChildInfos()
                                                .remove(dumpInfo.getChildInfos().size() - 1);
                                        DumpInfo memberDumpInfo = new DumpInfo(
                                                "member",
                                                "dynamic-member",
                                                "",
                                                nameDumpInfo.startByte,
                                                nameDumpInfo.lengthBytes + valueDumpInfo.lengthBytes);
                                        ...
                                        memberDumpInfo.previewValue = "" + nameDumpInfo.previewValue
                                                + (valueDumpInfo.previewValue != null
                                                        ? " : " + valueDumpInfo.previewValue
                                                        : "");
                        ...
                        LOGGER.log(
                                Level.FINER,
                                "Object value: dynamic={0},className={1},sealedSize={2},dynamicSize={3}",
                                new Object[] {
                                    traits.isDynamic(),
                                    traits.getClassName(),
                                    sealedMembers.size(),
                ...
                case Marker.VECTOR_INT:
                    ...
                    if (vectorIntNoRefFlag == 1) {
                        ...
                        LOGGER.log(Level.FINER, "Vector<int> value: fixed={0}, size={1}]", new Object[] {
                            fixed, vectorIntCountItems
                        });
                ...
                case Marker.VECTOR_UINT:
                    ...
                    if (vectorUIntNoRefFlag == 1) {
                        ...
                        LOGGER.log(Level.FINER, "Vector<uint> value: fixed={0}, size={1}]", new Object[] {
                            fixed, vectorUIntCountItems
                        });
                ...
                case Marker.VECTOR_DOUBLE:
                    ...
                    if (vectorDoubleNoRefFlag == 1) {
                        ...
                        LOGGER.log(Level.FINER, "Vector<double> value: fixed={0}, size={1}]", new Object[] {
                            fixed, vectorDoubleCountItems
                        });
                ...
                case Marker.VECTOR_OBJECT:
                    ...
                    if (vectorObjectNoRefFlag == 1) {
                        ...
                        LOGGER.log(
                                Level.FINER,
                                "Vector<Object> value: fixed={0}, size={1}, typeName:{2}]",
                                new Object[] {fixed, vectorObjectCountItems, objectTypeName});
