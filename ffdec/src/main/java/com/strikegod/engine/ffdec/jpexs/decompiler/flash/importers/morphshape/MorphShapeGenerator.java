 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.importers.morphshape;
...
public class MorphShapeGenerator {
     ...
     */
    public void generate(DefineMorphShape2Tag morphShape, ShapeTag startShape, ShapeTag endShape)
            throws StyleMismatchException {
        ...
        for (int a = 0; a < startExport.shapes.size(); a++) {
            ...
            FILLSTYLE startFillStyle =
                    startFillStyleIndex == -1 ? null : startExport.fillStyles.get(startFillStyleIndex);
            int startLineStyleIndex = startExport.lineStyleIndices.get(a);
            LINESTYLE2 startLineStyle =
                    startLineStyleIndex == -1 ? null : startExport.lineStyles.get(startLineStyleIndex);
        ...
        if (usedBs.size() < endExport.shapes.size()) {
            for (int b = 0; b < endExport.shapes.size(); b++) {
                if (!usedBs.contains(b)) {
                    ...
                    int endFillStyleIndex = endExport.fillStyleIndices.get(b);
                    FILLSTYLE endFillStyle =
                            endFillStyleIndex == -1 ? null : endExport.fillStyles.get(endFillStyleIndex);
                    int endLineStyleIndex = endExport.lineStyleIndices.get(b);
                    LINESTYLE2 endLineStyle =
                            endLineStyleIndex == -1 ? null : endExport.lineStyles.get(endLineStyleIndex);
        ...
        for (int i = 0; i < startLineStyles.size(); i++) {
            LINESTYLE2 lsStart = startLineStyles.get(i);
            LINESTYLE2 lsEnd = endLineStyles.get(i);
            if (lsEnd.hasFillFlag
                    && lsEnd.fillType.hasBitmap()
                    && lsStart.fillType.bitmapId != lsEnd.fillType.bitmapId) {
    ...
    private SHAPERECORD bezierToRecord(BezierEdge be) {
        if (be.points.size() == 2) {
            ...
            ser.deltaX =
                    (int) Math.round(be.points.get(1).getX() - be.points.get(0).getX());
            ser.deltaY =
                    (int) Math.round(be.points.get(1).getY() - be.points.get(0).getY());
        ...
        if (be.points.size() == 3) {
            ...
            cer.controlDeltaY =
                    (int) Math.round(be.points.get(1).getY() - be.points.get(0).getY());
            cer.anchorDeltaX =
                    (int) Math.round(be.points.get(2).getX() - be.points.get(1).getX());
            cer.anchorDeltaY =
                    (int) Math.round(be.points.get(2).getY() - be.points.get(1).getY());
            return cer;
        }
        return null;
    }
