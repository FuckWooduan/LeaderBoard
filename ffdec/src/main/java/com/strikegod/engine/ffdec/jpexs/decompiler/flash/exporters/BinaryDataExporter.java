...
public class BinaryDataExporter {
     ...
     */
    public List<File> exportBinaryData(
            AbortRetryIgnoreHandler handler,
            String outdir,
            ReadOnlyTagList tags,
            BinaryDataExportSettings settings,
            EventListener evl)
    ...
    public List<File> exportBinaryData(
            AbortRetryIgnoreHandler handler,
            String outdir,
            List<BinaryDataInterface> binaryDatas,
            BinaryDataExportSettings settings,
            EventListener evl)
            throws IOException, InterruptedException {
        ...
        for (final BinaryDataInterface t : binaryDatas) {
            ...
            final File file =
                    new File(outdir + File.separator + Helper.makeFileName(t.getCharacterExportFileName() + ext));
            new RetryTask(
                            () -> {
            ...
            if (Configuration.as3ExportNamesUseClassNamesOnly.get() && !classNames.isEmpty()) {
                for (String className : classNames) {
                    File classFile = new File(
                            outdir + File.separator + Helper.makeFileName(t.getClassExportFileName(className) + ext));
                    new RetryTask(
                                    () -> {
                                        Files.copy(
