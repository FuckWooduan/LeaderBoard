...
package com.strikegod.engine.ffdec.macromedia.asc.util;
...
public class Decimal128 {
        ...
        do {
            for (int i = 0; i < in.length; i++) {
                ...
                if (Character.isDigit(c) || c == '.') {
                    if (seenS) break; // invalid input
                    else break preparse; // parse as BigDecimal
                }
                if (c == '+') {
                    if (seenS) break; // invalid input
                ...
                if (c == 'N') {
                    if (((i + 2) < in.length) && (in[i + 1] == 'a') && (in[i + 2] == 'N')) {
                        ...
                        if ((i + 3) < in.length) {
                            ...
                            if (value.precision() > 33) value = BigDecimalZERO;
                        }
                        return;
                    } else break;
                }
    ...
    public Decimal128(double val) {
        ...
        } else value = new BigDecimal(val, MathContext.DECIMAL128);
    }
    ...
    public Decimal128(double val, Decimal128Context ctx) {
        ...
        if (Double.isNaN(val)) {
            ...
            flags |= DEC_INF; // sign already set above.
        } else value = new BigDecimal(val, ctx.mathCtx());
    }
    ...
    private static final int mask[] = {0x000, 0x001, 0x003, 0x007, 0x00F, 0x01F, 0x03F, 0x07F, 0x0FF, 0x1FF, 0x3FF
    }; // can't declare static inside IEEERep
    ...
    private class IEEERep {
        ...
        void addUnit(short unit) {
            ...
            if (totalDigits == 34) throw new IllegalArgumentException("trying to add more than 34 digits to IEEE128");
            if (totalDigits == 33) {
                if (unit > 9) throw new IllegalArgumentException("trying to add more than 34 digits to IEEE128");
                topDigit = unit;
                totalDigits++;
                return;
            }
            ...
            if (currentShift <= 22) {
                ...
                if (onThis != 0) data[currentDatum] |= dpd << currentShift;
        ...
        byte[] getRep() {
            if (!finalized) {
                int combo = 0;
                if (negative) data[0] |= 0x80000000;
                if (infinite) combo = 0x1E;
                else if (sNaN || qNaN) {
                    combo = 0x1F;
                    if (sNaN) exponent = 0x800; // high order bit on in continuation field
                    else exponent = 0;
                } else {
                    // finite number
                    if (topDigit < 8) combo = topDigit | ((exponent >>> 9) & 0x18);
                    else combo = 0x18 | (((exponent >>> 12) & 0x3) << 1) | (topDigit - 8);
                }
        ...
        IEEERep(byte[] rep) { // use this constructor to take apart a representation
            int ndx = 0;
            for (int i = 0; i < 16; i += 4) {
                data[ndx++] = ((rep[i] & 0xFF) << 24)
                        + ((rep[i + 1] & 0xFF) << 16)
                        + ((rep[i + 2] & 0xFF) << 8)
                        + (rep[i + 3] & 0xFF);
            }
            if ((data[0] & 0x80000000) != 0) negative = true;
            int combo = (data[0] >> 26) & 0x1F;
            if (combo == 0x1E) infinite = true;
            else if (combo == 0x1F) {
                if ((data[0] & 0x02000000) != 0) sNaN = true;
                else qNaN = true;
            } else {
                // finite number
                int expmsb;
    ...
    public byte[] toIEEE(Decimal128Context ctx) {
        ...
        if ((flags & DEC_NEG) != 0) ieee.setNeg();
        ...
        do { // dummy loop to allow break
            if ((flags & DEC_SPECIAL) == 0) {
                ...
                if ((digits > DECIMAL128_Pmax) || (ae > DECIMAL128_Emax) || (ae < DECIMAL128_Emin)) {
    ...
    private void initFromIEEE(byte[] rep) {
        ...
        if (rep.length != 16) throw new IllegalArgumentException("Decimal128 needs 16 bytes for representation");
        IEEERep ieee = new IEEERep(rep);
        if (ieee.isNegative()) flags = DEC_NEG;
        else flags = 0;
        if (ieee.isNaN()) {
            value = BigDecimalZERO; // some day support fancier NaNs
            if (ieee.isSNaN()) flags |= DEC_SNAN;
            else flags |= DEC_NAN;
    ...
    public double doubleValue() {
        if ((flags & (DEC_NAN | DEC_SNAN)) != 0) return Double.NaN;
        if ((flags & DEC_INF) != 0) {
            if ((flags & DEC_NEG) != 0) return Double.NEGATIVE_INFINITY;
    ...
    public String toString() {
        ...
        if (((flags & DEC_SPECIAL) != 0) && ((flags & DEC_NEG) != 0)) result.append('-');
    ...
    public Decimal128 add(Decimal128 addend, Decimal128Context ctx) {
        ...
        if (result.value.compareTo(BigDecimalZERO) < 0) result.flags |= DEC_NEG;
    ...
    public boolean greaterThanOrEqual(Decimal128 num) {
        // return true if I'm >= num
        Decimal128 comp = this.compareTo(num, Decimal128Context.DECIMAL128);
        if (comp == ONE || comp == ZERO) return true;
        else return false;
    }
    ...
    private Decimal128 compareTo(Decimal128 val, Decimal128Context ctx) {
        ...
        if ((this.flags & DEC_INF) != 0) {
            ...
            if ((val.flags & DEC_NEG) != 0) return ONE;
            else return NEG1;
        } else {
