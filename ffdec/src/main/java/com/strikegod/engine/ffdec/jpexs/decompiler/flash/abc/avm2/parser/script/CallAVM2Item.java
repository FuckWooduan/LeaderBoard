...
import com.strikegod.engine.ffdec.jpexs.decompiler.graph.TypeFunctionItem;
...
public class CallAVM2Item extends AVM2Item {
    ...
    public List<GraphSourceItem> toSource(
                    ...
                    for (int i = 0; i < mi.param_types.length; i++) {
                        ...
                        if (mi.param_types[i] == 0) {
                            ...
                            type = new TypeItem(abc.constants
                                    .getMultiname(mi.param_types[i])
                                    .getNameWithNamespace(
                                            new LinkedHashSet<>() /*???*/, abc, abc.constants, true /*??*/));
                        }
        ...
        if (propIndex != -1) {
            ...
            return toSourceMerge(localData, generator, obj, arguments, ins(insCode, propIndex, arguments.size()));
        }

        if (callable instanceof IndexAVM2Item) {
            return ((IndexAVM2Item) callable)
                    .toSource(localData, generator, needsReturn, true, arguments, false, false);
        }
        if (callable instanceof NamespacedAVM2Item) {
            return ((NamespacedAVM2Item) callable)
                    .toSource(localData, generator, needsReturn, true, arguments, false, false);
        }
        ...
        return toSourceMerge(
                ...
                generator,
                callable,
                ins(AVM2Instructions.GetGlobalScope) /*ASC2 uses getlocal0 here*/,
                arguments,
                ins(AVM2Instructions.Call, arguments.size()),
