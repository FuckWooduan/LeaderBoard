...
package com.strikegod.engine.ffdec.jpexs.graphics;
...
// Extended by JPEXS to support MITER_CLIP join style
final class Stroker implements PathConsumer2D {
    ...
    /**
     * Constant value for join style. Miter clip - JPEXS
     */
    ...
    private void drawMiter(
        ...
        // JPEXS
        if (joinStyle == JOIN_MITER_CLIP) {
            float len = (float) Math.sqrt(lenSq);

            float miterDistance = miterLimit;
            float percent = miterDistance / len;
            if (percent > 1) {
                percent = 1;
            }

            float x2 = x0 + percent * (miter[0] - x0);
            float y2 = y0 + percent * (miter[1] - y0);
            float m = (y2 - y0) / (x2 - x0);
            float m_bc = -1 / m;

            float x = x2 + 10;
            float y = m_bc * (x - x2) + y2;

            float int1[] = new float[2];
            float int2[] = new float[2];

            computeIntersection((x0 - pdx) + omx, (y0 - pdy) + omy, x0 + omx, y0 + omy, x2, y2, x, y, int1, 0);
            computeIntersection((dx + x0) + mx, (dy + y0) + my, x0 + mx, y0 + my, x2, y2, x, y, int2, 0);
            emitLineTo(int1[0], int1[1], rev);
            emitLineTo(int2[0], int2[1], rev);
        } else {
    ...
    private void drawJoin(
            float pdx, float pdy, float x0, float y0, float dx, float dy, float omx, float omy, float mx, float my) {
        if (prev != DRAWING_OP_TO) {
            ...
            if (joinStyle == JOIN_MITER || joinStyle == JOIN_MITER_CLIP) {
