 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.action.parser.pcode;

import com.strikegod.engine.ffdec.jpexs.decompiler.flash.ValueTooLargeException;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.action.special.ActionDeobfuscateJump;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.action.special.ActionDeobfuscatePop;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.action.special.ActionEnd;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.action.special.ActionNop;
...
public class ASMParser {
    ...
    private static Action parseAction(
            String instructionName, FlasmLexer lexer, List<String> constantPool, int version, String charset)
        ...
        } else if (instructionName.length() == 10
                && instructionName.substring(0, 8).compareToIgnoreCase("Unknown_") == 0) {
    ...
    private static List<Action> parseAllActions(FlasmLexer lexer, int version, String charset)
            throws IOException, ActionParseException {
        ...
        while (true) {
            ...
            if (symb.type == ASMParsedSymbol.TYPE_LABEL) {
                ...
                throw new ActionParseException(
                        "Label or Instruction name expected, found:" + symb.type + " " + symb.value, lexer.yyline());
            }
        }
    }
     ...
     */
    public static ActionList parse(
            long address, boolean ignoreNops, String source, int version, boolean throwOnError, String charset)
            throws IOException, ActionParseException {
        ...
        for (Action link : ret) {
            ...
            if (link instanceof ActionJump) {
                ...
                for (Label label : labels) {
                    ...
                    if (actionJump.identifier.equals(label.name)) {
                        int offset =
                                (int) (label.address - (actionJump.getAddress() + actionJump.getTotalActionLength()));
                        if (offset < -0x8000 || offset > 0x7fff) {
                            String message =
                                    "ActionJump offset is too large. offset: " + offset + ", jump action addr: ofs"
                                            + Helper.formatAddress(link.getAddress()) + ", target label: " + label.name;
                            if (throwOnError) {
                ...
                for (Label label : labels) {
                    if (actionIf.identifier.equals(label.name)) {
                        ...
                        if (offset < -0x8000 || offset > 0x7fff) {
                            String message =
                                    "ActionIf offset is too large. offset: " + offset + ", jump action addr: ofs"
                                            + Helper.formatAddress(link.getAddress()) + ", target label: " + label.name;
                            if (throwOnError) {
                                Integer line = lineMap.get(link);
                                if (line == null) {
            ...
            if (!found) {
                String message = "TARGET NOT FOUND - identifier:" + identifier + " addr: ofs"
                        + Helper.formatAddress(link.getAddress());
                if (throwOnError) {
                    Integer line = lineMap.get(link);
                    if (line == null) {
                        line = -1;
