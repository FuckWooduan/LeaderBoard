...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.instructions.SetTypeIns;
...
public class SetSlotIns extends InstructionDefinition implements SetTypeIns {
    ...
    public static void handleSetSlot(
            AVM2LocalData localData,
            TranslateStack stack,
            AVM2Instruction ins,
            List<GraphTargetItem> output,
        ...
        if (slotname != null) {
            if (value instanceof LocalRegAVM2Item) {
                ...
                String slotNameStr = slotname.getName(
        ...
        if (value.getNotCoerced().getThroughDuplicate() instanceof IncrementAVM2Item) {
            GraphTargetItem inside = ((IncrementAVM2Item) value.getNotCoerced())
                    .value
                    .getThroughRegister()
                    .getNotCoerced()
                    .getThroughDuplicate();
                    ...
                    if (!stack.isEmpty()) {
                        ...
                        if (top == inside) {
                            ...
                            stack.push(new PostIncrementAVM2Item(
                                    ins, localData.lineStartInstruction, inside, getNumberType(value)));
        ...
        if (value.getNotCoerced().getThroughDuplicate() instanceof DecrementAVM2Item) {
            GraphTargetItem inside = ((DecrementAVM2Item) value.getNotCoerced())
                    .value
                    .getThroughRegister()
                    .getNotCoerced()
                    .getThroughDuplicate();
            ...
            for (Trait t : localData.methodBody.traits.traits) {
                if (t instanceof TraitSlotConst) {
                    ...
                    if (tsc.slot_id == slotIndex) {
                        slotType = AbcIndexing.multinameToType(
                                localData.usedDeobfuscations, tsc.type_index, localData.abc, localData.abc.constants);
                        break;
                    }
                }
            }
        ...
        SetSlotAVM2Item result = new SetSlotAVM2Item(
                ins, localData.lineStartInstruction, obj, objnoreg, slotIndex, slotname, value, slotType);
        ...
        if (value.getNotCoerced() instanceof CompoundableBinaryOp) {
            if (!obj.hasSideEffect()) {
                ...
                if (binaryOp.getLeftSide().getNotCoerced() instanceof GetSlotAVM2Item) {
                    GetSlotAVM2Item getSlot =
                            (GetSlotAVM2Item) binaryOp.getLeftSide().getNotCoerced();
