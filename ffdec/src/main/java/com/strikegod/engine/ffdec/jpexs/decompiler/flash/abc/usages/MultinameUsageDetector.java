 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.usages;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.usages.multinames.ClassNameMultinameUsage;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.usages.multinames.ConstVarNameMultinameUsage;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.usages.multinames.ConstVarTypeMultinameUsage;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.usages.multinames.MethodBodyMultinameUsage;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.usages.multinames.MethodNameMultinameUsage;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.usages.multinames.MethodParamsMultinameUsage;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.usages.multinames.MethodReturnTypeMultinameUsage;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.usages.multinames.SuperClassMultinameUsage;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.usages.multinames.SuperInterfaceMultinameUsage;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.usages.multinames.TraitMultinameUsage;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.usages.multinames.TypeNameMultinameUsage;
...
public class MultinameUsageDetector implements UsageDetector {
    ...
    public List<Usage> findMultinameUsage(ABC abc, int multinameIndex, boolean exactMatch) {
        ...
        for (int s = 0; s < abc.script_info.size(); s++) {
            checkMultinameUsedInMethod(
                    abc,
                    multinameIndex,
                    exactMatch,
                    abc.script_info.get(s).init_index,
                    ret,
                    s,
                    -1,
                    0,
                    TraitMultinameUsage.TRAITS_TYPE_SCRIPT,
                    true,
                    null,
                    -1);
            findMultinameUsageInTraits(
                    abc,
                    abc.script_info.get(s).traits,
                    multinameIndex,
                    exactMatch,
                    TraitMultinameUsage.TRAITS_TYPE_SCRIPT,
                    s,
    ...
    public List<List<Usage>> findAllUsage(ABC abc) {
        ...
        for (int s = 0; s < abc.script_info.size(); s++) {
            checkAllMultinameUsedInMethod(
                    ...
                    null,
                    -1);
            findAllMultinameUsageInTraits(
                    abc, abc.script_info.get(s).traits, TraitMultinameUsage.TRAITS_TYPE_SCRIPT, s, -1, ret, -1);
        }
    ...
    private void findAllMultinameUsageInTraits(
            ABC abc,
            Traits traits,
            int traitsType,
            int scriptIndex,
        ...
        for (int t = 0; t < traits.traits.size(); t++) {
            if (traits.traits.get(t) instanceof TraitClass) {
                ...
                ret.get(classNameMultinameIndex)
                        .add(new ClassNameMultinameUsage(abc, classNameMultinameIndex, c, scriptIndex));
                int extendsMultinameIndex = abc.instance_info.get(c).super_index;
                ret.get(extendsMultinameIndex)
                        .add(new SuperClassMultinameUsage(abc, extendsMultinameIndex, c, scriptIndex));
                for (int i = 0; i < abc.instance_info.get(c).interfaces.length; i++) {
                    ...
                    ret.get(implementsMultinameIndex)
                            .add(new SuperInterfaceMultinameUsage(abc, implementsMultinameIndex, c, scriptIndex));
                }
                checkAllMultinameUsedInMethod(
                        abc,
                        abc.instance_info.get(c).iinit_index,
                        ret,
                        -1 /*FIXME*/,
                        c,
                        0,
                ...
                checkAllMultinameUsedInMethod(
                        abc,
                        abc.class_info.get(c).cinit_index,
                        ret,
                        -1 /*FIXME*/,
                        c,
                        0,
                        TraitMultinameUsage.TRAITS_TYPE_CLASS,
                        true,
                        null,
                        -1);
                findAllMultinameUsageInTraits(
                        abc,
                        abc.instance_info.get(c).instance_traits,
                        TraitMultinameUsage.TRAITS_TYPE_INSTANCE,
                        -1 /*FIXME*/,
                        c,
                        ret,
                ...
                findAllMultinameUsageInTraits(
                        abc,
                        abc.class_info.get(c).static_traits,
                        TraitMultinameUsage.TRAITS_TYPE_CLASS,
                        -1 /*FIXME*/,
                        c,
                        ret,
            ...
            if (traits.traits.get(t) instanceof TraitSlotConst) {
                ...
                ret.get(tsc.name_index)
                        .add(new ConstVarNameMultinameUsage(
                                abc, tsc.name_index, scriptIndex, classIndex, t, traitsType, traits, parentTraitIndex));
                ret.get(tsc.type_index)
                        .add(new ConstVarTypeMultinameUsage(
                                abc, tsc.type_index, scriptIndex, classIndex, t, traitsType, traits, parentTraitIndex));
            }
            if (traits.traits.get(t) instanceof TraitMethodGetterSetter) {
                ...
                ret.get(tmgs.name_index)
                        .add(new MethodNameMultinameUsage(
                                abc,
                                tmgs.name_index,
                                scriptIndex,
                                classIndex,
    ...
    private void checkAllMultinameUsedInMethod(
            ABC abc,
            int methodInfo,
            List<List<Usage>> ret,
            int scriptIndex,
        ...
        for (int p = 0; p < abc.method_info.get(methodInfo).param_types.length; p++) {
            ...
            if (!foundMultinames[methodParamsMultinameIndex]) {
                ret.get(methodParamsMultinameIndex)
                        .add(new MethodParamsMultinameUsage(
                                abc,
        ...
        ret.get(methodReturnTypeMultinameIndex)
                .add(new MethodReturnTypeMultinameUsage(
                        abc,
        ...
        if (body != null) {
            ...
            for (ABCException e : body.exceptions) {
                if (!foundMultinames[e.name_index]) {
                    ret.get(e.name_index)
                            .add(new MethodBodyMultinameUsage(
                                    abc,
                                    e.name_index,
                                    scriptIndex,
                                    classIndex,
    ...
    private void findMultinameUsageInTraits(
            ABC abc,
            Traits traits,
            int multinameIndex,
            boolean exactMatch,
        ...
        for (int t = 0; t < traits.traits.size(); t++) {
            ...
            if (traits.traits.get(t) instanceof TraitSlotConst) {
                ...
                if (isSameName(abc, multinameIndex, tsc.name_index, exactMatch)) {
                    ret.add(new ConstVarNameMultinameUsage(
                            abc, multinameIndex, scriptIndex, classIndex, t, traitsType, traits, parentTraitIndex));
                }
                if (isSameName(abc, multinameIndex, tsc.type_index, exactMatch)) {
                    ret.add(new ConstVarTypeMultinameUsage(
                            abc, multinameIndex, scriptIndex, classIndex, t, traitsType, traits, parentTraitIndex));
                }
            }
            if (traits.traits.get(t) instanceof TraitMethodGetterSetter) {
                ...
                if (isSameName(abc, multinameIndex, tmgs.name_index, exactMatch)) {
                    ret.add(new MethodNameMultinameUsage(
                            abc,
                            multinameIndex,
                            scriptIndex,
                            classIndex,
    ...
    private void checkMultinameUsedInMethod(
        ...
        for (int p = 0; p < abc.method_info.get(methodInfo).param_types.length; p++) {
            if (isSameName(abc, multinameIndex, abc.method_info.get(methodInfo).param_types[p], exactMatch)) {
                ret.add(new MethodParamsMultinameUsage(
                        abc,
                        multinameIndex,
                        scriptIndex,
                        classIndex,
        ...
        if (isSameName(abc, multinameIndex, abc.method_info.get(methodInfo).ret_type, exactMatch)) {
            ret.add(new MethodReturnTypeMultinameUsage(
                    abc,
                    multinameIndex,
                    scriptIndex,
                    classIndex,
        ...
        if (body != null) {
            findMultinameUsageInTraits(
                    abc, body.traits, multinameIndex, exactMatch, traitsType, scriptIndex, classIndex, ret, traitIndex);
            for (ABCException e : body.exceptions) {
                if ((isSameName(abc, multinameIndex, e.name_index, exactMatch))
                        || (isSameName(abc, multinameIndex, e.type_index, exactMatch))) {
                    ret.add(new MethodBodyMultinameUsage(
                            abc,
                            multinameIndex,
                            scriptIndex,
                            classIndex,
            ...
            for (AVM2Instruction ins : body.getCode().code) {
                for (int o = 0; o < ins.definition.operands.length; o++) {
                    if (ins.definition.operands[o] == AVM2Code.DAT_MULTINAME_INDEX
                            && ins.operands[o] < abc.constants.getMultinameCount()) {
