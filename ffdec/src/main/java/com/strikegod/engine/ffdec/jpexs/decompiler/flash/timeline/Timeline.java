 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.timeline;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.TagRemoveListener;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.configuration.Configuration;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.BlendModeSettable;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.FrameExporter;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.RequiresNormalizedFonts;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.commonshape.SVGExporter;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.tags.base.DisplayObjectCacheKey;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.tags.base.StaticTextTag;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.types.BlendMode;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.types.filters.BlendComposite;
...
public class Timeline {
        ...
        for (int d = 0; d <= maxDepth; d++) {
            ...
            for (int f = 0; f <= frames.size(); f++) {
                ...
                if (ds != null
                        ...
                        && ((ds.getCharacter() instanceof MorphShapeTag) || Objects.equals(ds.matrix, matrix))) {
                    len++;
                } else {
    ...
    private void createASPackages() {
        for (ASMSource asm : asmSources) {
            if (asm instanceof DoInitActionTag) {
                ...
                for (int pos = 0; pos < pathParts.length - 1; pos++) {
                    if (Configuration.flattenASPackages.get()) {
                        if (isNamedPackages && pos == 0) {
                            ...
                            String fullPath;
                            if (isNamedPackages) {
                                fullPath = path.substring(
                                        pathParts[0].length() + 1,
                                        path.length() - pathParts[pathParts.length - 1].length() - 1);
                            } else {
                                fullPath =
                                        path.substring(0, path.length() - pathParts[pathParts.length - 1].length() - 1);
                            }
                ...
                if (Configuration.flattenASPackages.get()
                        && ((pathParts.length == 2 && isNamedPackages) || pathParts.length == 1)) {
    ...
    private void drawDrawable(
            ...
            SerializableImage fullImage,
            DrawableTag drawable,
            List<FILTER> filters,
            double unzoom,
            ColorTransform mergedColorTransform,
            boolean sameImage,
            ExportRectangle viewRect,
            ExportRectangle viewRectRaw,
        ...
        ExportRectangle fullRect =
                fullTransformation.concatenate(layerMatrix).transform(new ExportRectangle(boundRect));
        ...
        if (img == null) {
            ...
            if (drawable instanceof ButtonTag) {
                ...
                if (renderContext.cursorPosition != null && renderContext.enableButtons) {
                    ...
                    Point cursorPositionInView = new Point(
                            (int) Math.round(renderContext.cursorPosition.x * unzoom) - dx,
                            (int) Math.round(renderContext.cursorPosition.y * unzoom) - dy);

                    Shape buttonShape = ((ButtonTag) drawable)
                            .getOutline(
                                    true,
                                    ButtonTag.FRAME_HITTEST,
                                    dtime,
                                    ratio,
                                    renderContext,
                                    absMat,
            ...
            if (drawable instanceof TextTag) {
                ...
                if (renderContext.enableTexts) {
                    int dx = (int) (viewRect.xMin * unzoom);
                    int dy = (int) (viewRect.yMin * unzoom);
                    Point cursorPositionInView = renderContext.cursorPosition == null
                            ? new Point(0, 0)
                            : new Point(
                                    (int) Math.round(renderContext.cursorPosition.x * unzoom) - dx,
                                    (int) Math.round(renderContext.cursorPosition.y * unzoom) - dy);

                    Shape textShape = ((TextTag) drawable)
                            .getOutline(true, 0, 0, 0, renderContext, absMat, true, viewRect, unzoom);
                    ...
                    if (textShape.contains(cursorPositionInView)
                            || (drawable == renderContext.selectionText && renderContext.mouseButton == 1)) {
                        ...
                        if (textTag instanceof DefineEditTextTag) {
                            ...
                            textRecords =
                                    ((DefineEditTextTag) textTag).getTextRecords(textTag.getSwf(), normalizedFonts);
                        }
                        ...
                        Point cursorPosNoTrans =
                                absMat.concatenate(textMatrix).inverse().transform(cursorPositionInView);
                        ...
                        for (RECT gp : glyphPositions) {
                            ...
                            Rectangle2D r =
                                    new Rectangle2D.Double(gp.Xmin, gp.Ymin, gp.Xmax - gp.Xmin, gp.Ymax - gp.Ymin);
            ...
            if (!sameImage && cacheAsBitmap && renderContext.displayObjectCache != null) {
                ...
                renderContext.displayObjectCache.put(
                        new DisplayObjectCacheKey(layer.placeObjectTag, unzoom, viewRect), img);
            }
        }
        ...
        if (clipDepth > -1) {
            ...
            Clip clip = new Clip(
                    Helper.imageToShape(mask),
                    clipDepth); // Maybe we can get current outline instead converting from image (?)
            clips.add(clip);
        } else {
            if (renderContext.cursorPosition != null) {
                ...
                if (drawable instanceof DefineSpriteTag) {
                    ...
                    Shape shape = drawable.getOutline(
                            true, dframe, dtime, layer.ratio, renderContext, absMat, true, viewRect, unzoom);
            ...
            if (renderContext.borderImage != null) {
                ...
                Shape shape = drawable.getOutline(
                        ...
                        layer.ratio,
                        renderContext,
                        absMat.preConcatenate(Matrix.getScaleInstance(1 / SWF.unitDivisor)),
                        true,
                        viewRect,
                        unzoom);
                g2.draw(shape);
            ...
            if (!(sameImage && canUseSameImage)) {
                ...
                if (!((blendMode == 11 || blendMode == 12)
                        && parentBlendMode <= 1)) { // alpha and erase modes require parent blendmode not normal
        ...
        for (int i = 0; i <= maxDepth; i++) {
            ...
            if (character instanceof DrawableTag) {
                ...
                if (scalingRect != null) {
                    ...
                    DefineScalingGridTag.getSlices(
                            targetBoundsRect,
                            boundsRect,
                            new ExportRectangle(scalingRect),
                            sourceRect,
                            targetRect,
                            transforms);
                    Shape c = g.getClip();
            ...
            if (showPlaceholder) {
                AffineTransform trans = mat.preConcatenate(Matrix.getScaleInstance(1 / SWF.unitDivisor))
                        .toTransform();
                g.setTransform(trans);
    ...
    private boolean tagHasSmallStrokes(SVGExporter exporter, DrawableTag drawableTag, Matrix matrix) {
        ...
        if (drawableTag instanceof ShapeTag) {
            com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.commonshape.Point p00 = matrix.transform(0, 0);
            com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.commonshape.Point p11 = matrix.transform(1, 1);
        ...
        for (int i = 0; i <= maxDepth; i++) {
            ...
            if (clipChanged) {
                ...
                if (!clips.isEmpty()) {
                    ...
                    PathIterator pathIterator = new FlatteningPathIterator(
                            a.getPathIterator(AffineTransform.getScaleInstance(1 / 20.0, 1 / 20.0)), 0.01);
                    ...
                    while (!pathIterator.isDone()) {
                        ...
                        switch (type) {
                            case PathIterator.SEG_MOVETO:
                                sb.append("M ")
                                        .append(formatDoubleSvg(coords[0]))
                                        .append(" ")
                                        .append(formatDoubleSvg(coords[1]))
                                        .append(" ");
                                break;
                            ...
                            case PathIterator.SEG_QUADTO:
                                sb.append("Q ")
                                        ...
                                        .append(formatDoubleSvg(coords[1]))
                                        .append(" ")
                                        .append(formatDoubleSvg(coords[2]))
                                        .append(" ")
                                        .append(formatDoubleSvg(coords[3]))
                                        .append(" ");
                                break;
                            case PathIterator.SEG_CUBICTO:
                                sb.append("C ")
                                        ...
                                        .append(formatDoubleSvg(coords[3]))
                                        .append(" ")
                                        .append(formatDoubleSvg(coords[4]))
                                        .append(" ")
                                        .append(formatDoubleSvg(coords[5]))
                                        .append(" ");
                                break;
            ...
            if (character instanceof DrawableTag) {
                ...
                if (layer.clipDepth > -1) {
                    Shape shape = drawable.getOutline(
                            false, 0, 0, layer.ratio, new RenderContext(), layerMatrix, false, null, 1);
                    ;
                    ...
                    SVGExporter.ExportKey exportKey = new SVGExporter.ExportKey(
                            drawableTag, clrTrans, layer.ratio, layer.clipDepth > -1, dframe, dtime);
                    ...
                    if (createNew) {
                        ...
                        drawable.toSVG(
                                dframe, dtime, exporter, layer.ratio, clrTrans, level + 1, transformation, absMat);
                        exporter.endGroup();
                    }
                    Matrix mat = new Matrix(
                            layer.matrix); // Matrix.getTranslateInstance(rect.xMin, rect.yMin).preConcatenate(new
                    // Matrix(layer.matrix));
                    exporter.addUse(
                            mat,
                            boundRect,
                            assetName,
                            layer.instanceName,
                            scalingGrid == null ? null : scalingGrid.splitter,
                            String.valueOf(drawable.getCharacterId()),
                            String.join("___", drawable.getClassNames()),
                            layer.blendMode,
                            layer.filters);
                }
            }
    ...
    public void getSounds(
            int frame,
            int time,
            ButtonTag mouseOverButton,
            int mouseButton,
            List<Integer> sounds,
            List<String> soundClasses,
        ...
        for (int d = maxDepth; d >= 0; d--) {
            ...
            if (ds != null) {
                ...
                if (c instanceof Timelined) {
                    ...
                    ((Timelined) c)
                            .getTimeline()
                            .getSounds(dframe, 0, mouseOverButton, mouseButton, sounds, soundClasses, soundInfos);
                }
            }
        }
    }
            ...
            if (character instanceof DrawableTag) {
                ...
                if (character instanceof ButtonTag) {
                    ...
                    if (renderContext.cursorPosition != null && renderContext.enableButtons) {
                        ...
                        Shape buttonShape = buttonTag.getOutline(
                                fast,
                                ButtonTag.FRAME_HITTEST,
                                time,
                                layer.ratio,
                ...
                Shape cshape = ((DrawableTag) character)
                        .getOutline(fast, dframe, time, layer.ratio, renderContext, m, stroked, viewRect, unzoom);
