...
public class Cache<K, V> implements Freed {
    ...
    static {
        Runtime.getRuntime().addShutdownHook(FfdecExecutors.newPlatformShutdownHook("cache-cleanup", () -> {
    ...
    public static <K, V> Cache<K, V> getInstance(boolean weak, boolean memoryOnly, String name, boolean temporary) {
        if (oldCleaner == null) {
            oldCleaner = FfdecExecutors.startPlatformDaemon("cache-cleaner", Thread.MIN_PRIORITY, () -> {
                while (!Thread.interrupted()) {
                    ...
                    try {
                        ...
                        Logger.getLogger(Cache.class.getSimpleName())
                                .log(Level.SEVERE, "Error during clearing cache thread", cme);
                    }
                }
            });
        }
    ...
    private static int storageType = STORAGE_MEMORY;
    ...
    public static void setStorageType(int storageType) {
        if (storageType == Cache.storageType || storageType == STORAGE_FILES) {
            return;
        }
        switch (storageType) {
            case STORAGE_FILES:
        ...
        Cache.storageType = STORAGE_MEMORY;
    }
    ...
    private void initCache() {
        Map<K, V> newCache = weak ? new WeakHashMap<>() : new HashMap<>();
