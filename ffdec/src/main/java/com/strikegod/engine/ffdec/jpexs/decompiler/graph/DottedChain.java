...
public class DottedChain implements Serializable, Comparable<DottedChain> {
    ...
    protected String toString(Set<String> usedDeobfuscations, SWF swf, boolean as3, boolean raw, boolean withSuffix) {
        ...
        for (int i = 0; i < parts.size(); i++) {
            ...
            ret.append(
                    (raw || lastStar)
                            ? part
                            : IdentifiersDeobfuscation.printIdentifier(swf, usedDeobfuscations, as3, part));
            if (withSuffix) {
    ...
    public String toFilePath(SWF swf) {
        ...
        for (int i = 0; i < parts.size(); i++) {
            ...
            ret.append(Helper.makeFileName(
                    IdentifiersDeobfuscation.printIdentifier(swf, new LinkedHashSet<>(), true, parts.get(i).name)));
        }
        return ret.toString();
    }
