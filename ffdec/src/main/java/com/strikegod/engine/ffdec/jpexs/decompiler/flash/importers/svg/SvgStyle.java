...
class SvgStyle {
     ...
     */
    public SvgStyle(
            SvgImporter importer, Map<String, Element> idMap, Element element, Map<String, Integer> cachedBitmaps) {
    ...
    private Map<String, String> getStyleAttributeValues(Element element) {
        ...
        if (!styleStr.isEmpty()) {
            ...
            for (int i = 0; i < rulesBySpecList.size(); i++) {
                rulesBySpecList.set(
                        i,
                        rulesBySpecList.get(i).substring(rulesBySpecList.get(i).indexOf("}") + 1));
            }
        ...
        if (el.hasAttribute("gradientUnits")) {
            switch (el.getAttribute("gradientUnits")) {
                ...
                case "objectBoundingBox":
                    ...
                    break;
                default:
                    importer.showWarning(
                            "fillNotSupported", "Unsupported  gradientUnits: " + el.getAttribute("gradientUnits"));
                    return new SvgColor(random.nextInt(256), random.nextInt(256), random.nextInt(256));
            }
        }
                    ...
                    if (element != null && "image".equals(element.getTagName())) {
                        ...
                        if (attr.startsWith("data:image/") && attr.contains("base64,")) {
                            ...
                            try {
                                ...
                                if (e.hasAttribute("ffdec:smoothed")) { // backwards compatibility
                                    String smoothedValue =
                                            e.getAttribute("ffdec:smoothed").trim();
                                    if ("true".equals(smoothedValue)) {
                                        bitmapFill.smoothed = true;
                                    }
                                    if ("false".equals(smoothedValue)) {
        ...
        try {
            switch (name) {
                ...
                case "stroke":
                    ...
                    break;
                case "stroke-width": {
                    return importer.parseLength(
                            value,
                            Math.sqrt(importer.getViewBox().width * importer.getViewBox().width
