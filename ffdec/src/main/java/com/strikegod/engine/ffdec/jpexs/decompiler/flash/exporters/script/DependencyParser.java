...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.instructions.alchemy.AlchemyTypeIns;
...
public class DependencyParser {
     ...
     * @param uses Uses
     */
    public static void parseDependenciesFromNS(
            AbcIndexing abcIndex,
            String ignoredCustom,
            ABC abc,
        ...
        if (ns.kind == Namespace.KIND_NAMESPACE || ns.kind == Namespace.KIND_PACKAGE_INTERNAL) {
            ...
            if (nsimport != null) {
                ...
                if (!nsimport.isEmpty()) {
                    ...
                    if ((ignorePackage == null || !nsimport.getWithoutLast().equals(ignorePackage))
                            && !dependencies.contains(depNs)) {
     ...
     * @param uses Uses
     */
    public static void parseDependenciesFromMultiname(
        ...
        if (m != null) {
            if (m.kind == Multiname.TYPENAME) {
                if (m.qname_index != 0) {
                    parseDependenciesFromMultiname(
                            usedDeobfuscations,
                            abcIndex,
                            ignoredCustom,
                            abc,
                            dependencies,
                            abc.constants.getMultiname(m.qname_index),
                            ignorePackage,
                            fullyQualifiedNames,
                            dependencyType,
                            uses);
    ...
    public static void parseDependenciesFromMethodInfo(
        ...
        if (abc.method_info.get(method_index).ret_type != 0) {
            parseDependenciesFromMultiname(
                    ...
                    abc,
                    dependencies,
                    abc.constants.getMultiname(abc.method_info.get(method_index).ret_type),
                    ignorePackage,
                    fullyQualifiedNames,
                    DependencyType.SIGNATURE,
                    uses);
        }
        ...
        if (body != null && body.convertException == null) {
            body = body.convertMethodBodyCanUseLast(
                    Configuration.autoDeobfuscate.get(), "", isStatic, scriptIndex, classIndex, abc, trait);
            body.traits.getDependencies(
                    usedDeobfuscations,
                    abcIndex,
                    scriptIndex,
                    classIndex,
            ...
            for (ABCException ex : body.exceptions) {
                parseDependenciesFromMultiname(
                        ...
                        abc,
                        dependencies,
                        abc.constants.getMultiname(ex.type_index),
                        ignorePackage,
                        fullyQualifiedNames,
                        DependencyType.EXPRESSION /* or signature?*/,
                        uses);
            }
            ...
            for (int i = 0; i < body.getCode().code.size(); i++) {
                ...
                if (ins.definition instanceof NewFunctionIns) {
                    if (ins.operands[0] != method_index) {
                        if (!visitedMethods.contains(ins.operands[0])) {
                            parseDependenciesFromMethodInfo(
                                    usedDeobfuscations,
                                    abcIndex,
                                    trait,
                                    scriptIndex,
                    ...
                    if (m != null) {
                        if (m.kind == Multiname.MULTINAMEL) {
                            Dependency dep = new Dependency(
                                    DottedChain.parseNoSuffix("flash.utils.getDefinitionByName"),
                ...
                if (ins.definition instanceof ConstructIns
                        && prevIns != null
                        && prevIns.definition instanceof GetPropertyIns) {
                ...
                if (classIndex > -1 && ins.definition instanceof GetOuterScopeIns) {
                    if (ins.operands[0] > 0) { // first is global
                        DottedChain type = abc.instance_info
                                .get(classIndex)
