...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.GraphicsTextDrawable;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.shape.SVGShapeExporter;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.helpers.HighlightedText;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.importers.TextImportResizeTextBoundsMode;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.tags.DefineEditTextTag;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.tags.text.JustifyAlignGlyphEntry;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.tags.text.TextAlign;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.tags.text.TextParseException;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.types.ColorTransform;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.types.DynamicTextGlyphEntry;
...
public abstract class TextTag extends DrawableTag {
     ...
     */
    public abstract boolean setFormattedText(
            MissingCharacterHandler missingCharHandler, String formattedText, String[] texts) throws TextParseException;
    ...
    public static List<RECT> getGlyphEntriesPositions(List<TEXTRECORD> list, SWF swf) {
        ...
        for (int r = 0; r < list.size(); r++) {
            ...
            if (rec.styleFlagsHasFont) {
                ...
                if (font == null) {
                    Logger.getLogger(TextTag.class.getName())
                            .log(Level.SEVERE, "Font with id={0} was not found.", rec.fontId);
                    continue;
                }
            ...
            for (GLYPHENTRY entry : rec.glyphEntries) {
                ret.add(new RECT(x, x + entry.glyphAdvance, (int) Math.round(y - ascent), (int)
                        Math.round(y + descent + leading)));
     ...
     */
    public static Map<String, Object> getTextRecordsAttributes(
            List<TEXTRECORD> list, SWF swf, Map<Integer, FontTag> normalizedFonts) {
        ...
        for (int r = 0; r < list.size(); r++) {
            ...
            for (int e = 0; e < rec.glyphEntries.size(); e++) {
                ...
                if (font.hasLayout()) {
                    ...
                    defaultAdvance = (int) (Math.round(textHeight
                            * (font.getGlyphAdvance(entry.glyphIndex) + kerningAdjustment)
                            / (1024.0 * font.getDivider())));

                } else {
                    defaultAdvance = (int) Math.round(SWF.unitDivisor
                            * FontTag.getSystemFontAdvance(
                                    aFont,
                                    font.glyphToChar(entry.glyphIndex),
                                    nextEntry == null ? null : font.glyphToChar(nextEntry.glyphIndex)));
                }
    ...
    public static void drawBorder(
        ...
        BitmapExporter.export(
                ...
                1,
                swf,
                getBorderShape(borderColor, fillColor, rect),
                null,
                image,
                1 /*FIXME??*/,
                mat,
     ...
     */
    public static void drawBorderHtmlCanvas(
            SWF swf,
        ...
        result.append("\tctx.transform(")
                .append(mat.scaleX)
                .append(",")
                .append(mat.rotateSkew0)
                .append(",")
                .append(mat.rotateSkew1)
                .append(",")
                .append(mat.scaleY)
                .append(",")
                .append(mat.translateX)
                .append(",")
                .append(mat.translateY)
                .append(");\r\n");
        SHAPE shape = getBorderShape(borderColor, fillColor, rect);
        CanvasShapeExporter cse =
                new CanvasShapeExporter(ShapeTag.WIND_EVEN_ODD, 1, null, unitDivisor, swf, shape, colorTransform, 0, 0);
        cse.export();
     ...
     * @param zoom Zoom
     */
    public static void drawBorderSVG(
            SWF swf,
        ...
        mat = mat.concatenate(new Matrix(textMatrix));
        // ??FIXME
        SVGShapeExporter shapeExporter = new SVGShapeExporter(
                ShapeTag.WIND_EVEN_ODD, 1, swf, shape, 0, exporter, null, colorTransform, 1, zoom, mat);
     ...
     */
    public static void staticTextToImage(
            SWF swf,
        ...
        if (image.getGraphics() instanceof GraphicsTextDrawable) {
            // custom drawing
            ((GraphicsTextDrawable) image.getGraphics())
                    .drawTextRecords(swf, textRecords, numText, textMatrix, transformation, colorTransform);
            return;
        }
            ...
            if (rec.styleFlagsHasFont) {
                ...
                if (font != null) {
                    if (!font.hasLayout()) {
                        ...
                        ascent = font.getAscent(); // ((double) font.getAscent() * textHeight / 1024.0 /
                        // font.getDivider());
                        descent = font.getDescent(); // (double) font.getDescent() * textHeight / 1024.0 /
                        // font.getDivider());
                        leading = font.getLeading(); // (double) font.getLeading()* textHeight / 1024.0 /
                        // font.getDivider());
                    }
                }
            }
            ...
            for (GLYPHENTRY entry : rec.glyphEntries) {
                ...
                if (entry.glyphIndex != -1 && glyphs != null) {
                    ...
                    if (dynamicEntry.fontFace != null) {
                        ...
                        if (fnt != null && entry.glyphIndex != -1) {
                            ...
                            shape = SHAPERECORD.fontCharacterToSHAPE(
                                    new Font(dynamicEntry.fontFace, dynamicEntry.fontStyle, 12),
                                    (int) Math.round(divider * 1024),
                                    dynamicEntry.character);
                ...
                if (pos >= selectionStart && pos < selectionEnd) {
                    ...
                    RECT bounds = new RECT(
                            0,
                            (int) Math.round(entry.glyphAdvance * 1024 * font.getDivider() / textHeight),
                            (int) Math.round(-ascent),
                            (int) Math.round((descent + leading)));
                    // bounds = shape.getBounds(1);
                    // bounds.Ymin = (int) Math.round(-ascent);
                    // bounds.Ymax = (int) Math.round(descent + leading);
                    Matrix mat2 = Matrix.getTranslateInstance(bounds.Xmin, bounds.Ymin)
                            .preConcatenate(mat);
                    TextTag.drawBorder(
                            swf, image, borderColor, fillColor, bounds, new MATRIX(), mat2, colorTransform, aaScale);
                }
                ...
                if (shape != null) {
                    BitmapExporter.export(
                            ...
                            swf,
                            shape,
                            pos >= selectionStart && pos < selectionEnd ? Color.white : textColor3,
                            image,
                            1 /*FIXME??*/,
                            mat,
                            mat,
                    ...
                    if (SHAPERECORD.DRAW_BOUNDING_BOX) {
                        ...
                        mat = Matrix.getTranslateInstance(bounds.Xmin, bounds.Ymin)
                                .preConcatenate(mat);
                        TextTag.drawBorder(
                                swf, image, borderColor, fillColor, bounds, new MATRIX(), mat, colorTransform, aaScale);
                    }
                }
     ...
     * @return Text bounds
     */
    public static ExportRectangle calculateTextBounds(
            SWF swf, TextTag tag, List<TEXTRECORD> textRecords, MATRIX textMatrix) {
        FontTag font = null;
        int textHeight = 12;
        int x = 0;
        int y = 0;
        ...
        for (TEXTRECORD rec : textRecords) {
            ...
            for (GLYPHENTRY entry : rec.glyphEntries) {
                ...
                if (entry.glyphIndex != -1 && glyphs != null) {
                    ...
                    if (font.hasLayout()) {
                        glyphBounds = new ExportRectangle(
                                0, -font.getAscent(), entry.glyphAdvance / rat, font.getDescent() + leading);
                    } else {
    ...
    protected void updateTextBounds(RECT textBounds) {
        ...
        if (resizeMode != null
                && (resizeMode.equals(TextImportResizeTextBoundsMode.GROW_ONLY)
                        || resizeMode.equals(TextImportResizeTextBoundsMode.GROW_AND_SHRINK))) {
     ...
     * @param colorTransform Color transform
     */
    public static void staticTextToHtmlCanvas(
            double unitDivisor,
            SWF swf,
            List<TEXTRECORD> textRecords,
            int numText,
        ...
        for (TEXTRECORD rec : textRecords) {
            ...
            result.append("\tvar textColor = ")
                    .append(CanvasShapeExporter.color(textColor))
                    .append(";\r\n");
            for (GLYPHENTRY entry : rec.glyphEntries) {
                Matrix mat = (new Matrix(textMatrix).concatenate(Matrix.getTranslateInstance(x, y)))
                        .concatenate(Matrix.getScaleInstance(rat));
                if (entry.glyphIndex != -1) {
                    ...
                    result.append("\tfont")
                            .append(fontId)
                            .append("(ctx,\"")
                            .append(("" + font.glyphToChar(entry.glyphIndex))
                                    .replace("\\", "\\\\")
                                    .replace("\"", "\\\""))
                            .append("\",textColor);\r\n");
                    result.append("\tctx.restore();\r\n");
     ...
     * @param transformation Transformation
     */
    public static void staticTextToSVG(
            SWF swf,
            List<TEXTRECORD> textRecords,
            int numText,
            SVGExporter exporter,
            ...
            if (exporter.useTextTag) {
                ...
                for (GLYPHENTRY entry : rec.glyphEntries) {
                    ...
                    if (entry.glyphIndex != -1) {
                        ...
                        if (charId == null) {
                            charId = exporter.getUniqueId(
                                    Helper.getValidHtmlId("font_" + font.getFontNameIntag() + "_" + ch));
