...
public class UnresolvedAVM2Item extends AssignableAVM2Item {
    ...
    public AssignableAVM2Item copy() {
        UnresolvedAVM2Item c = new UnresolvedAVM2Item(
                subtypes,
                importedClasses,
                mustBeType,
                type,
     ...
     */
    public UnresolvedAVM2Item(
            List<GraphTargetItem> subtypes,
            List<DottedChain> importedClasses,
            boolean mustBeType,
            GraphTargetItem type,
            int line,
            DottedChain name,
            GraphTargetItem storeValue,
     ...
     */
    public static AVM2Instruction generateCoerce(
            SourceGeneratorLocalData localData, SourceGenerator generator, GraphTargetItem type)
        ...
        switch (type.toString()) {
            ...
            default:
                int type_index =
                        AVM2SourceGenerator.resolveType(localData, type, ((AVM2SourceGenerator) generator).abcIndex);
     ...
     * @throws CompilationException On compilation error
     */
    public GraphTargetItem resolve(
            SourceGeneratorLocalData localData /*can be null!!!*/,
            String currentClassFullName,
            GraphTargetItem thisType,
            ...
            for (AssignableAVM2Item a : variables) {
                if (a instanceof NameAVM2Item) {
                    ...
                    if (n.isDefinition() && name.get(0).equals(n.getVariableName())) {
                        NameAVM2Item ret = new NameAVM2Item(
                                n.type,
                                n.line,
                                name.isAttribute(0),
                                name.get(0),
                                name.getNamespaceSuffix(0),
                                null,
                                false,
                                openedNamespaces,
                                abcIndex,
                        ...
                        for (int i = 1; i < name.size(); i++) {
                            resolved = new PropertyAVM2Item(
                                    resolved,
                                    name.isAttribute(i),
                                    name.get(i),
                                    name.getNamespaceSuffix(i),
                                    abc,
                                    openedNamespaces,
                                    new ArrayList<>(),
                                    false,
        ...
        if ((paramNames.contains(name.get(0)) || name.get(0).equals("arguments"))) {
            ...
            GraphTargetItem ret = new NameAVM2Item(
                    t,
                    line,
                    name.isAttribute(0),
                    name.get(0),
        ...
        if (localData != null) { // resolve can be called without localData
            PropertyAVM2Item resolvedx = new PropertyAVM2Item(
                    null,
                    name.isAttribute(0),
                    name.get(0),
                    name.getNamespaceSuffix(0),
                ...
                for (DottedChain imp : importedClasses) {
                    ...
                    if (impName.equals(name.get(0))) {
                        throw new CompilationException(
                                "The type \"" + name.get(0)
                                        + "\" exists on toplevel package and also as an import from different package. Please make it fully qualified so it matches the desired import.",
                                line);
                    }
                }
                resolved = ti;
                ...
                if (impName.equals(name.get(0))) {
                    ...
                    if (name.size() == 1) {
                        AbcIndexing.ClassIndex ci = abc.findClass(
                                ret, abc.getSelectedAbc(), localData == null ? null : localData.scriptIndex);
                        if (ci == null) {
        ...
        if (!isProperty) {
            for (int i = 0; i < name.size(); i++) {
                ...
                AbcIndexing.ClassIndex ci = abc.findClass(
                        new TypeItem(fname),
                        localData != null ? abc.getSelectedAbc() : null,
                        localData != null ? localData.scriptIndex : null);
                if (ci != null) {
            ...
            for (NamespaceItem n : openedNamespaces) {
                ...
                TypeItem ti =
                        new TypeItem(ons.getName(abc.getSelectedAbc().constants).addWithSuffix(name.get(0)));
                AbcIndexing.ClassIndex ci = abc.findClass(ti, null, null /*FIXME?*/);
                if (ci != null) {
        ...
        if (!isProperty && (name.get(0).equals("this") || name.get(0).equals("super"))) {
            ...
            if (isSuper) {
                ...
                if (ci == null) {
                    ...
                    ntype = new TypeItem(ci.abc
                            .instance_info
                            .get(ci.index)
                            .getName(ci.abc.constants)
                            .getNameWithNamespace(new LinkedHashSet<>(), ci.abc, ci.abc.constants, true));
                }
            }
        ...
        if (mustBeType) {
            throw new CompilationException(
                    name.toPrintableString(
                                    new LinkedHashSet<>(),
                                    abcIndex.getSelectedAbc().getSwf(),
                                    true) + " is not an existing type",
                    line);
        }
        resolved = null;
