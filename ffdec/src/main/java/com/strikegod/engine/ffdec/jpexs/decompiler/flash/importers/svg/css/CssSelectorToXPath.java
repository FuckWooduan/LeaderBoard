 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.importers.svg.css;
...
public class CssSelectorToXPath {
    ...
    private String xpath_url(String s) {
        return "substring-before(concat(substring-after(" + (s == null ? xpath_url_attrs : s)
                + ",\"://\"),\"?\"),\"?\")";
    }
    ...
    private String xpath_url_domain(String s) {
        return "substring-before(concat(substring-after(" + (s == null || s.isEmpty() ? xpath_url_attrs : s)
                + ",\"://\"),\"/\"),\"/\")";
    }
    ...
    private final String xpath_has_protocol =
            "(starts-with(" + xpath_url_attrs + ",\"http://\") or starts-with(" + xpath_url_attrs + ",\"https://\"))";
    private final String xpath_is_internal = "starts-with(" + xpath_url(null) + "," + xpath_url_domain(xpath_ns_uri)
            + ") or " + xpath_ends_with(xpath_url_domain(null), xpath_url_domain(xpath_ns_uri));
    private final String xpath_is_local =
            "(" + xpath_has_protocol + " and starts-with(" + xpath_url(null) + "," + xpath_url(xpath_ns_uri) + "))";
    ...
    private final String regex_css_wrap_pseudo =
            "(\\x1F\\)|[^\\)])\\:(first|limit|last|gt|lt|eq|nth)([^\\-]|$)"; // bez g
    ...
    private final String css_combinators_regex =
            "\\s*(!?[+>~,^ ])\\s*(\\.?\\/+|[a-z\\-]+::)?([a-z\\-]+\\()?((and\\s*|or\\s*|mod\\s*)?[^+>~,\\s'\"\\]\\|\\^\\$\\!\\<\\=\\x1C-\\x1F]+)?"; // /g;
    ...
    private String css_combinators_callback(
            String match,
            String operator,
            String axis,
            String func,
        ...
        switch (operator) {
            ...
            // case '~~'
            // return '/following-sibling::*/self::|'+selectorStart(orig,
            // offset)+'/preceding-sibling::*/self::'+literal;
        }
        return null;
    }

    private final String css_attributes_regex =
            "\\[([^\\@\\|\\*\\=\\^\\~\\$\\!\\(\\/\\s\\x1C-\\x1F]+)\\s*(([\\|\\*\\~\\^\\$\\!]?)=?\\s*(\\x1E+))?\\]"; // /g;

    private String css_attributes_callback(
            String str, String attr, String comp, String op, String val, int offset, String orig) {
        String axis = "";
        ...
        switch (op) {
            ...
            case "$":
                return axis + "[substring(@" + attr + ",string-length(@" + attr + ")-(string-length(\"" + val
                        + "'\")-1))=\"" + val + "\"]";
            case "^":
    ...
    private final String css_pseudo_classes_regex =
            ":([a-z\\-]+)(\\((\\x1F+)(([^\\x1F]+(\\3\\x1F+)?)*)(\\3\\)))?"; // /g;

    private String css_pseudo_classes_callback(
            String match,
            String name,
            String g1,
            String g2,
        ...
        switch (name) { // name.toLowerCase()?
            ...
            case "nth-child":
                ...
                switch (arg) {
                    ...
                    default:
                        String[] a = (arg == null || arg.isEmpty() ? "0" : arg)
                                .replaceAll(regex_nth_equation, "$1+$2")
                                .split("\\+");
                        String a0;
                        ...
                        return "[(count(preceding-sibling::*)+1)>=" + a1 + " and ((count(preceding-sibling::*)+1)-" + a1
                                + ") mod " + a0 + "=0]";
                }
            case "nth-of-type":
                if (isNumeric(arg)) {
            ...
            case "domain":
                return "[(string-length(" + xpath_url_domain(null) + ")=0 and contains("
                        + xpath_url_domain(xpath_ns_uri) + "," + arg + ")) or contains(" + xpath_url_domain(null) + ","
                        + arg + ")]";
            case "path":
    ...
    private String css_ids_classes_regex =
            "(#|\\.)([^\\#\\@\\.\\/\\(\\[\\)\\]\\|\\:\\s\\+\\>\\<\\'\\\"\\x1D-\\x1F]+)"; // /g;
    ...
    public String css2xpath(String s, boolean nested) {
        if (nested == true) {
            ...
            s = replace(s, css_pseudo_classes_regex, new ReplaceCallBack() {
                @Override
                public String run(Matcher mat, String s) {
                    return css_pseudo_classes_callback(
                            mat.group(),
                            mat.group(1),
                            mat.group(1),
                            mat.group(2),
        ...
        s = replace(s, css_combinators_regex, new ReplaceCallBack() {
            @Override
            public String run(Matcher mat, String s) {
                return css_combinators_callback(
                        mat.group(),
                        mat.group(1),
                        mat.group(2),
                        mat.group(3),
        ...
        s = replace(s, css_attributes_regex, new ReplaceCallBack() {
            @Override
            public String run(Matcher mat, String s) {
                return css_attributes_callback(
                        mat.group(), mat.group(1), mat.group(2), mat.group(3), mat.group(4), mat.start(), s);
            }
        });
