 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.types;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.action.ConstantPoolTooBigException;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.types.annotations.HideInRawEdit;
...
public class BUTTONCONDACTION implements ASMSource, Serializable, HasSwfAndTag {
    ...
    public BUTTONCONDACTION(SWF swf, SWFInputStream sis, Tag tag) throws IOException {
        ...
        actionBytes = sis.readByteRangeEx(
                condActionSize <= 0 ? sis.available() : condActionSize - 4,
                "actionBytes",
                DumpInfoSpecialType.ACTION_BYTES,
                sis.getPos());
    }
     ...
     */
    @Override
    public GraphTextWriter getASMSource(ScriptExportMode exportMode, GraphTextWriter writer, ActionList actions)
    ...
    @Override
    public GraphTextWriter getActionScriptSource(GraphTextWriter writer, ActionList actions)
            throws InterruptedException {
        if (actions == null) {
            actions = getActions();
        }
    ...
    @Override
    public GraphTextWriter getActionScriptSource(
            GraphTextWriter writer, ActionList actions, List<ActionTreeOperation> treeOperations)
            throws InterruptedException {
        if (actions == null) {
            actions = getActions();
        }

        return Action.actionsToSource(
                new HashMap<>(), this, actions, getScriptName(), writer, actions.getCharset(), treeOperations);
    }
