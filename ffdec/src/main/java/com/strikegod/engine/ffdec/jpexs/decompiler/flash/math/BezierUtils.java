...
public class BezierUtils {
    ...
    public double closestPointToBezier(Point2D _p, Point2D p0, Point2D p1, Point2D p2) {
        ...
        // Find roots of the Bezier curve with control points stored in 'w' (algorithm is recursive, this is root depth
        // of 0)
    ...
    private List<Point2D> toBezierForm(Point2D _p, List<Point2D> _v) {
        ...
        List<Point2D> w =
                new ArrayList<>(); // control-points for Bezier curve whose zeros represent candidates for closest point
        // to the input parametric curve
    ...
    // how many times does the Bezier curve cross the horizontal axis - the number of roots is less than or equal to
    // this count
    ...
    // convert 2D array indices in a k x n matrix to a linear index (this is an interim step ahead of a future
    // implementation optimized for 1D array indexing)
    ...
    public void subdivide(List<Point2D> _c, double _t, List<Point2D> _left, List<Point2D> _right) {
        ...
        for (int i = 1; i <= degree; ++i) {
            for (int j = 0; j <= degree - i; ++j) {
                ...
                vertex.setLocation(
                        t1 * p.get(im1j).getX() + _t * p.get(im1jp1).getX(),
                        t1 * p.get(im1j).getY() + _t * p.get(im1jp1).getY());
                while (ij >= p.size()) {
    ...
    private boolean isControlPolygonLinear(List<Point2D> _v, int _degree) {
        // Given array of control points, _v, find the distance from each interior control point to line connecting v[0]
        // and v[degree]
        ...
        double c = _v.get(0).getX() * _v.get(_degree).getY()
                - _v.get(_degree).getX() * _v.get(0).getY();
