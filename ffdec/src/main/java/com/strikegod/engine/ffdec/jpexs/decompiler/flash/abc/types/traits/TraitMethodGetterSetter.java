...
public class TraitMethodGetterSetter extends Trait {
    ...
    public GraphTextWriter toStringHeader(
        ...
        abc.method_info
                .get(method_info)
                .getReturnTypeStr(writer, abc, abc.constants, fullyQualifiedNames, usedDeobfuscations);
        return writer;
    }
    ...
    public void convert(
        ...
        writer.startMethod(
                method_info,
                getName(abc).getName(usedDeobfuscations, abc, abc.constants, new ArrayList<>(), true, false));
        path = path + "."
                + getName(abc).getName(usedDeobfuscations, abc, abc.constants, fullyQualifiedNames, false, true);
        convertHeader(
                usedDeobfuscations,
                swfVersion,
                parent,
    ...
    public GraphTextWriter toString(
        ...
        if (classIndex != -1 && abc.instance_info.get(classIndex).isInterface() || bodyIndex == -1) {
            ...
            if (exportMode != ScriptExportMode.AS_METHOD_STUBS) {
                ...
                String retTypeRaw =
                        abc.method_info.get(method_info).getReturnTypeRaw(abc, abc.constants, fullyQualifiedNames);
                switch (retTypeRaw) {
                    case "void":
                        break;
                    case "int":
    ...
    public boolean isVisible(boolean isStatic, ABC abc) {
        if (Configuration.handleSkinPartsAutomatically.get()) {
            if ("skinParts"
                    .equals(getName(abc)
                            .getName(new LinkedHashSet<>(), abc, abc.constants, new ArrayList<>(), true, true))) {
                if (kindType == TRAIT_GETTER) {
                    MethodInfo mi = abc.method_info.get(method_info);
                    if (mi.param_types.length == 0
                            && "Object"
                                    .equals(abc.constants
                                            .getMultiname(mi.ret_type)
                                            .getNameWithNamespace(new LinkedHashSet<>(), abc, abc.constants, true)
                                            .toRawString())) {
                        if (abc.constants.getNamespace(abc.constants.getMultiname(name_index).namespace_index).kind
