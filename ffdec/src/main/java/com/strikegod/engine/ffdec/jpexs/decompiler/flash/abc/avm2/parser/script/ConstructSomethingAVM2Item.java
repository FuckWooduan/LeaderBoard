...
public class ConstructSomethingAVM2Item extends CallAVM2Item {
     ...
     */
    public ConstructSomethingAVM2Item(
            int line,
    ...
    public List<GraphSourceItem> toSource(
        ...
        if (resname instanceof TypeItem) {
            ...
            if (localData.isStatic
                    && localData
                            .pkg
                            .addWithSuffix(localData.currentClassBaseName)
                            .equals(prop.fullTypeName)) {
                return toSourceMerge(
                        localData,
                        generator,
                        new AVM2Instruction(0, AVM2Instructions.GetLocal0, new int[] {}),
                        arguments,
                        new AVM2Instruction(0, AVM2Instructions.Construct, new int[] {arguments.size()}),
                        needsReturn ? null : ins(AVM2Instructions.Pop));
            }
            int type_index =
                    AVM2SourceGenerator.resolveType(localData, resname, ((AVM2SourceGenerator) generator).abcIndex);
            return toSourceMerge(
                    localData,
                    generator,
                    new AVM2Instruction(
                            0, AVM2Instructions.FindPropertyStrict, new int[] {type_index, arguments.size()}),
                    arguments,
        ...
        if (resname instanceof PropertyAVM2Item) {
            ...
            return toSourceMerge(
                    localData,
                    generator,
                    prop.resolveObject(localData, generator, true),
                    arguments,
        ...
        if (resname instanceof NameAVM2Item) {
            return toSourceMerge(
                    ...
                    resname,
                    arguments,
                    ins(AVM2Instructions.Construct, arguments.size()),
                    needsReturn ? null : ins(AVM2Instructions.Pop));
        }
        ...
        if (resname instanceof NamespacedAVM2Item) {
            return ((NamespacedAVM2Item) resname)
                    .toSource(localData, generator, needsReturn, false, arguments, false, true);
        }
        return toSourceMerge(
                localData,
                generator,
