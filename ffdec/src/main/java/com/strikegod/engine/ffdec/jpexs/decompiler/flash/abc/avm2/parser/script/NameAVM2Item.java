...
public class NameAVM2Item extends AssignableAVM2Item {
    ...
    public AssignableAVM2Item copy() {
        NameAVM2Item c = new NameAVM2Item(
                type,
                line,
                attribute,
                variableName,
    ...
    public static AVM2Instruction generateCoerce(
            SourceGeneratorLocalData localData, SourceGenerator generator, GraphTargetItem ttype)
        ...
        switch (ttype.toString()) {
            ...
            default:
                int type_index =
                        AVM2SourceGenerator.resolveType(localData, ttype, ((AVM2SourceGenerator) generator).abcIndex);
    ...
    private List<GraphSourceItem> toSource(
        ...
        if (assignedValue != null) {
            ...
            if (slotNumber > -1) {
                return toSourceMerge(
                        ...
                        assignedValue,
                        !(("" + assignedValue.returnType()).equals("" + type) && (basicTypes.contains("" + type)))
                                ? generateCoerce(localData, generator, type)
                                : null,
    ...
    private void addTraitUsage(SourceGeneratorLocalData localData, List<MethodBody> callStack) {
        ...
        for (MethodBody b : callStack) {
            for (int i = 0; i < b.traits.traits.size(); i++) {
                ...
                if (t.getName(abcV)
                        .getName(new LinkedHashSet<>(), abcV, constants, null, true, true)
                        .equals(variableName)) {
        ...
        if (!needsReturn) {
            if (slotNumber > -1) {
                ...
                if (isInteger) {
                    ...
                    changeIns = ins(
                            decrement ? AVM2Instructions.DecrementP : AVM2Instructions.IncrementP,
                            localData.numberContext);
                } else {
                ...
                if (isInteger) {
                    ...
                    changeIns = ins(
                            decrement ? AVM2Instructions.DecLocalP : AVM2Instructions.IncLocalP,
                            localData.numberContext,
                            regNumber);
                } else {
                ...
                return toSourceMerge(localData, generator, changeIns);
            }
        }
        return toSourceMerge(
                localData,
                ...
                (!post)
                        ? (decrement
                                ? ins(isInteger ? AVM2Instructions.DecrementI : AVM2Instructions.Decrement)
                                : ins(isInteger ? AVM2Instructions.IncrementI : AVM2Instructions.Increment))
                        : null,
                needsReturn ? ins(AVM2Instructions.Dup) : null,
                (post)
                        ? (decrement
