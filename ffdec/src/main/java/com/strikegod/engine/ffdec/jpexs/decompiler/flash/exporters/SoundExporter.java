...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.flv.AUDIODATA;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.flv.FLVOutputStream;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.flv.FLVTAG;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.types.sound.SoundExportFormat;
...
public class SoundExporter {
    ...
    public List<File> exportSounds(
            AbortRetryIgnoreHandler handler,
            String outdir,
            ReadOnlyTagList tags,
            final SoundExportSettings settings,
            EventListener evl)
    ...
    public List<File> exportSounds(
        ...
        for (SoundTag st : tags) {
            ...
            Set<String> classNames =
                    (st instanceof CharacterTag) ? ((CharacterTag) st).getClassNames() : new HashSet<>();
            if (Configuration.as3ExportNamesUseClassNamesOnly.get() && !classNames.isEmpty()) {
                for (String className : classNames) {
                    ...
                    new RetryTask(
                                    () -> {
                                        Files.copy(
                                                file.toPath(), classFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
                                    },
                                    handler)
                            .run();
                    ret.add(classFile);
    ...
    public void exportSound(OutputStream fos, SoundTag st, SoundExportMode mode) throws IOException {
        ...
        if (nativeFormat == SoundExportFormat.MP3 && mode.hasMP3()) {
            ...
            if ((st instanceof DefineSoundTag)
                    || (st instanceof DefineExternalSound)
                    || (st instanceof DefineExternalStreamSound)) {
                FLVOutputStream flv = new FLVOutputStream(fos);
                flv.writeHeader(true, false);
                List<ByteArrayRange> datas = st.getRawSoundData();
                for (ByteArrayRange data : datas) {
                    flv.writeTag(new FLVTAG(
                            0,
                            new AUDIODATA(
                                    st.getSoundFormatId(),
                                    st.getSoundRate(),
