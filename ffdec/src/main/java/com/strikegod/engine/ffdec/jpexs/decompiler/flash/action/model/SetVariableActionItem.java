...
public class SetVariableActionItem extends ActionItem implements SetTypeActionItem {
     ...
     * @param value Value
     */
    public SetVariableActionItem(
            GraphSourceItem instruction, GraphSourceItem lineStartIns, GraphTargetItem name, GraphTargetItem value) {
    ...
    private boolean isValidName(LocalData localData) {
        return ((name instanceof DirectValueActionItem))
                && (((DirectValueActionItem) name).isString())
                && (IdentifiersDeobfuscation.isValidName(
                        false, ((DirectValueActionItem) name).toStringNoQuotes(localData), "this", "super"));
    }
    ...
    public GraphTextWriter appendTo(GraphTextWriter writer, LocalData localData) throws InterruptedException {
        if (isValidName(localData) || resultNeeded()) {
            ...
            writer.append(IdentifiersDeobfuscation.printIdentifier(
                    localData.swf, localData.usedDeobfuscations, false, name.toStringNoQuotes(localData)));
    ...
    public List<GraphSourceItem> toSource(SourceGeneratorLocalData localData, SourceGenerator generator)
        ...
        try {
            return toSourceMerge(
                    ...
                    name,
                    value,
                    new ActionStoreRegister(tmpReg, charset),
                    new ActionSetVariable(),
                    new ActionPush(new RegisterNumber(tmpReg), charset));
        } finally {
