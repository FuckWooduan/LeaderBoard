...
public class LinkReportExporter {
    ...
    public String generateReport(SWF swf, List<ScriptPack> as3scripts, EventListener evl) throws InterruptedException {
        ...
        for (ScriptPack sp : as3scripts) {
            existingObjects.add(
                    sp.getClassPath().packageStr.add(sp.getClassPath().className, sp.getClassPath().namespaceSuffix));
        }
    ...
    private String reportTrait(
            int scriptIndex, Set<String> externalDefs, List<DottedChain> existingObjects, SWF swf, ABC abc, Trait t)
        ...
        if (t instanceof TraitClass) {
            ...
            sb.append(indent(3))
                    .append("<def id=\"")
                    .append(multiNameToId(abc, tc.getName(abc)))
                    .append("\" />")
                    .append(newLineChar);
            ...
            tc.getDependencies(
                    ...
                    abc,
                    dependencies,
                    new DottedChain(new String[] {"FAKE!PACKAGE"}),
                    new ArrayList<>(),
                    new ArrayList<>(),
                    new Reference<>(null));
