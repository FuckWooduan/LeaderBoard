 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.types.traits;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.script.Dependency;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.script.DependencyParser;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.script.DependencyType;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.search.MethodId;
...
public class TraitClass extends Trait implements TraitWithSlot {
    ...
    public void getDependencies(
        ...
        DottedChain packageName = instanceInfo
                .getName(abc.constants)
                .getNamespace(abc.constants)
                .getName(abc.constants); // assume not null name

        // DependencyParser.parseDependenciesFromMultiname(customNs, abc, dependencies, uses,
        // abc.constants.getMultiname(instanceInfo.name_index), packageName, fullyQualifiedNames);
        if (instanceInfo.super_index > 0) {
            DependencyParser.parseDependenciesFromMultiname(
                    usedDeobfuscations,
                    abcIndex,
                    customNamespace,
                    abc,
        ...
        // static
        classInfo.static_traits.getDependencies(
                usedDeobfuscations,
                abcIndex,
                scriptIndex,
                class_info,
        ...
        // static initializer
        DependencyParser.parseDependenciesFromMethodInfo(
                usedDeobfuscations,
                abcIndex,
                null,
                scriptIndex,
        ...
        // instance
        instanceInfo.instance_traits.getDependencies(
                usedDeobfuscations,
                abcIndex,
                scriptIndex,
                class_info,
    ...
    @Override
    public GraphTextWriter toStringHeader(
            Set<String> usedDeobfuscations,
            int swfVersion,
            Trait parent,
    ...
    public String toString(ABC abc, List<DottedChain> fullyQualifiedNames) {
        return "Class " + abc.constants.getMultiname(name_index).toString(abc.constants, fullyQualifiedNames) + " slot="
                + slot_id + " class_info=" + class_info + " metadata=" + Helper.intArrToString(metadata);
    }

    @Override
    public GraphTextWriter toString(
            Set<String> usedDeobfuscations,
            int swfVersion,
            AbcIndexing abcIndex,
            DottedChain packageName,
        ...
        // DottedChain packageName = instanceInfoMultiname.getNamespace(abc.constants).getName(abc.constants); //assume
        // not null name

        Reference<Boolean> first = new Reference<>(true);

        String instanceInfoName =
                instanceInfoMultiname.getName(usedDeobfuscations, abc, abc.constants, fullyQualifiedNames, false, true);
        ...
        if (convertData.exportEmbedFlaMode) {
            allowEmbed = false;
            if (abc.getSwf() != null) {
                CharacterTag ct = abc.getSwf()
                        .getCharacterByClass(instanceInfoMultiname
                                .getNameWithNamespace(usedDeobfuscations, abc, abc.constants, false)
                                .toRawString());
                if (ct == null) {
                    allowEmbed = false;
                } else {
                    ...
                    if (ct instanceof ImageTag) {
                        allowEmbed = true;
                        if (abcIndex.isInstanceOf(
                                abc, class_info, DottedChain.parseNoSuffix("flash.display.BitmapData"))) {
                            allowEmbed = false;
                        }
                    }
        ...
        // class header
        instanceInfo.getClassHeaderStr(
                usedDeobfuscations, convertData.assetsDir, writer, abc, fullyQualifiedNames, false, allowEmbed);
        writer.endTrait();
        ...
        classInfo.static_traits.toString(
                ...
                first,
                abcIndex,
                new Class[] {TraitSlotConst.class},
                this,
                convertData,
                path + "/" + instanceInfoName,
                abc,
                true,
                exportMode,
                false,
                ...
                if (!classInitializerIsEmpty) {
                    ...
                    abc.bodies
                            ...
                            .toString(
                                    ...
                                    callStack,
                                    abcIndex,
                                    path + "/" + instanceInfoName + ".staticinitializer",
                                    exportMode,
                                    abc,
                                    this,
                                    writer,
                    ...
                    // Note: There must be trait/method highlight even if the initializer is empty to TraitList in GUI
                    // to work correctly
        ...
        if (!instanceInfo.isInterface()) {
            ...
            writer.appendNoHilight(m.getName(
                    usedDeobfuscations, abc, abc.constants, null /*do not want full names here*/, false, true));
            writer.appendNoHilight("(");
            bodyIndex = abc.findBodyIndex(instanceInfo.iinit_index);
            MethodBody body = bodyIndex == -1 ? null : abc.bodies.get(bodyIndex);
            abc.method_info
                    .get(instanceInfo.iinit_index)
                    .getParamStr(writer, abc.constants, body, abc, fullyQualifiedNames, usedDeobfuscations);
            ...
            if (exportMode != ScriptExportMode.AS_METHOD_STUBS) {
                if (body != null) {
                    ...
                    body.toString(
                            ...
                            callStack,
                            abcIndex,
                            path + "/" + instanceInfoName + ".initializer",
                            exportMode,
                            abc,
                            this,
                            writer,
        ...
        classInfo.static_traits.toString(
                ...
                first,
                abcIndex,
                new Class[] {TraitClass.class, TraitFunction.class, TraitMethodGetterSetter.class},
                this,
                convertData,
                path + "/" + instanceInfoName,
                abc,
        ...
        if (convertData.ignoreAccessibility) {
            for (Trait t : instanceInfo.instance_traits.traits) {
                String traitName =
                        t.getName(abc).getName(usedDeobfuscations, abc, abc.constants, new ArrayList<>(), true, false);
                ;
                if (traitName.startsWith("__setAcc_") || traitName.startsWith("__setTab_")) {
    ...
    public void convert(
        ...
        InstanceInfo instanceInfo = abc.instance_info.get(class_info);
        String instanceInfoName = instanceInfo
                .getName(abc.constants)
                .getName(usedDeobfuscations, abc, abc.constants, fullyQualifiedNames, false, true);
        ClassInfo classInfo = abc.class_info.get(class_info);
        ...
        if (sIndex > -1) {
            ...
            if (nsIndex > -1) {
                ...
                convertData.thisHasDefaultToPrimitive = null
                        == index.findProperty(
                                new AbcIndexing.PropertyDef(
                                        "toString",
                                        new TypeItem(instanceInfo
                                                .getName(abc.constants)
        ...
        if (bodyIndex != -1) {
            ...
            if (!abc.instance_info.get(class_info).isInterface()) {
                AbcIndexing.ClassIndex cls = abcIndex.findClass(
                        AbcIndexing.multinameToType(
                                new LinkedHashSet<>(),
                                abc.instance_info.get(class_info).name_index,
                                abc,
                                abc.constants),
                        abc,
                        scriptIndex);
                ...
                for (AbcIndexing.ClassIndex cls2 : clsList) {
                    newScopeStack.push(new ClassAVM2Item(cls2.abc
                            .instance_info
                            .get(cls2.index)
                            .getName(cls2.abc.constants)
                            .getNameWithNamespace(usedDeobfuscations, cls2.abc, cls2.abc.constants, true)));
                }
            }
            ...
            newScopeStack.push(
                    new ClassAVM2Item(abc.instance_info.get(class_info).getName(abc.constants)));
                                            ...
                                            if (callProp.arguments.get(i) instanceof IntegerValueAVM2Item) {
                                                if (callProp.arguments.get(i + 1) instanceof GetLexAVM2Item) {
                                                    ...
                                                    frameTraitNames.add(lex.propertyName.getName(
                                                            ...
                                                            false,
                                                            true));
                                                } else if (callProp.arguments.get(i + 1)
        ...
        classInfo.static_traits.convert(
                usedDeobfuscations,
                swfVersion,
                abcIndex,
                this,
        ...
        instanceInfo.instance_traits.convert(
                usedDeobfuscations,
                swfVersion,
                abcIndex,
                this,
    ...
    public GraphTextWriter convertTraitHeader(ABC abc, GraphTextWriter writer) {
        ...
        writer.appendNoHilight("instance ")
                .hilightSpecial(abc.constants.multinameToString(ii.name_index), HighlightSpecialType.INSTANCE_NAME)
                .newLine();
        writer.indent();
        writer.appendNoHilight("extends ")
                .hilightSpecial(abc.constants.multinameToString(ii.super_index), HighlightSpecialType.EXTENDS)
                .newLine();
        for (int iface : ii.interfaces) {
            writer.appendNoHilight("implements ")
                    .hilightSpecial(abc.constants.multinameToString(iface), HighlightSpecialType.IMPLEMENTS)
                    .newLine();
        }
        ...
        if ((ii.flags & InstanceInfo.CLASS_PROTECTEDNS) == InstanceInfo.CLASS_PROTECTEDNS) {
            writer.appendNoHilight("protectedns ")
                    .hilightSpecial(
                            Multiname.namespaceToString(abc.constants, ii.protectedNS),
                            HighlightSpecialType.PROTECTEDNS)
                    .newLine();
        }
        writer.unindent();
