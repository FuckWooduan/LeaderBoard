 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.model;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.parser.script.AVM2SourceGenerator;
...
public abstract class AVM2Item extends GraphTargetItem {
     ...
     */
    protected GraphTextWriter formatProperty(
            GraphTextWriter writer,
            GraphTargetItem object,
            GraphTargetItem propertyName,
            LocalData localData,
            boolean isStatic,
        ...
        if (object instanceof LocalRegAVM2Item) {
            if (((LocalRegAVM2Item) object).computedValue != null) {
                if (((LocalRegAVM2Item) object).computedValue.getThroughNotCompilable()
                        instanceof FindPropertyAVM2Item) {
                    empty = true;
                }
            }
        }
        ...
        if (empty) {
            ...
            if (localData.classIndex > -1) {
                DottedChain thisClass = localData
                        .abc
                        .instance_info
                        .get(localData.classIndex)
                        .getName(localData.constantsAvm2)
                        .getNameWithNamespace(
                                localData.usedDeobfuscations, localData.abc, localData.constantsAvm2, false);
        ...
        if (propertyName instanceof FullMultinameAVM2Item) {
            ...
            if (multinameIndex >= 0 && multinameIndex < localData.constantsAvm2.getMultinameCount()) {
                ...
                isAttribute =
                        localData.constantsAvm2.getMultiname(multinameIndex).isAttribute();
                localName = localData
                        .constantsAvm2
                        .getMultiname(multinameIndex)
                        .getNameAndCustomNamespace(
                                new HashSet<>(), localData.abc, new ArrayList<>(), true, true, customNsRef);
                namespaceSuffix =
                        localData.constantsAvm2.getMultiname(multinameIndex).getNamespaceSuffix();
     ...
     */
    public static String localRegName(
            SWF swf, HashMap<Integer, String> localRegNames, int reg, Set<String> usedDeobfuscations) {
    ...
    public boolean isIdentical(GraphTargetItem other) {
        ...
        while (tiName instanceof LocalRegAVM2Item) {
            if (((LocalRegAVM2Item) tiName).computedValue != null) {
                tiName = ((LocalRegAVM2Item) tiName)
                        .computedValue
        ...
        while (tiName2 instanceof LocalRegAVM2Item) {
            if (((LocalRegAVM2Item) tiName2).computedValue != null) {
                tiName2 = ((LocalRegAVM2Item) tiName2)
                        .computedValue
                        .getThroughNotCompilable()
                        .getThroughDuplicate();
            } else {
