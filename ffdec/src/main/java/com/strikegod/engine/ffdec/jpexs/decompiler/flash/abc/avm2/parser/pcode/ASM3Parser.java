 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.parser.pcode;
...
public class ASM3Parser {
    ...
    private static void expected(ParsedSymbol s, int type, String expStr, int line)
    ...
    private static float getFloat(ParsedSymbol s, int line, boolean orNull) throws AVM2ParseException {
        ...
        String nval = (String) s.value;
        if (nval.endsWith("d") || nval.endsWith("m")) {
            throw new AVM2ParseException(expected, line);
        }

        if (nval.endsWith("i") || nval.endsWith("u") || nval.endsWith("f")) {
            nval = nval.substring(0, nval.length() - 1);
        }
    ...
    private static Decimal128 getDecimal(ParsedSymbol s, int line, boolean orNull) throws AVM2ParseException {
        ...
        if (nval.endsWith("i") || nval.endsWith("u") || nval.endsWith("d") || nval.endsWith("m")) {
            nval = nval.substring(0, nval.length() - 1);
        }
     ...
     */
    public static boolean parseClass(ABC abc, Reader reader, AVM2ConstantPool constants, TraitClass tc)
            throws IOException, AVM2ParseException {
        Flasm3Lexer lexer = new Flasm3Lexer(reader);
        return parseClass(abc, lexer, constants, tc);
    }

    private static boolean parseClass(ABC abc, Flasm3Lexer lexer, AVM2ConstantPool constants, TraitClass tc)
            throws IOException, AVM2ParseException {
            ...
            switch (symb.type) {
                ...
                default:
                    throw new AVM2ParseException(
                            "SEALED,FINAL,INTERFACE,PROTECTEDNS or NON_NULLABLE expected", lexer.yyline());
            }
            symb = lexer.lex();
        }
        int protectedns = 0;
        if (symb.type == ParsedSymbol.TYPE_KEYWORD_PROTECTEDNS_BLOCK
                && ((instanceFlags & InstanceInfo.CLASS_PROTECTEDNS) == InstanceInfo.CLASS_PROTECTEDNS)) {
    ...
    private static boolean parseSlotConst(ABC abc, Flasm3Lexer lexer, AVM2ConstantPool constants, TraitSlotConst tsc)
            throws IOException, AVM2ParseException {
        expected(ParsedSymbol.TYPE_KEYWORD_TRAIT, "trait", lexer);
        ParsedSymbol symb = lexer.lex();
     ...
     * @throws AVM2ParseException On parse error
     */
    public static boolean parseSlotConst(ABC abc, Reader reader, AVM2ConstantPool constants, TraitSlotConst tsc)
            throws IOException, AVM2ParseException {
        Flasm3Lexer lexer = new Flasm3Lexer(reader);
    ...
    private static int parseNumberContext(Flasm3Lexer lexer) throws AVM2ParseException, IOException {
        ...
        switch (s.type) {
            ...
            default:
                throw new AVM2ParseException(
                        "Usage expected - one of: Number, decimal, double, int or uint", lexer.yyline());
        }
    ...
    private static int parseNamespaceSet(AVM2ConstantPool constants, Flasm3Lexer lexer)
            throws AVM2ParseException, IOException {
    ...
    private static int parseNamespace(AVM2ConstantPool constants, Flasm3Lexer lexer)
            throws AVM2ParseException, IOException {
        ...
        return constants.getNamespaceId(
                kind, name.type == ParsedSymbol.TYPE_KEYWORD_NULL ? null : (String) name.value, index, true);
    }

    private static int parseMultiName(AVM2ConstantPool constants, Flasm3Lexer lexer)
            throws AVM2ParseException, IOException {
        ParsedSymbol s = lexer.lex();
        int kind = 0;
     ...
     * @throws AVM2ParseException On parse error
     */
    public static ValueKind parseValue(AVM2ConstantPool constants, Flasm3Lexer lexer)
            throws IOException, AVM2ParseException {
        ParsedSymbol type = lexer.lex();
        ParsedSymbol value;
        ParsedSymbol temp;
     ...
     */
    public static AVM2Code parse(ABC abc, Reader reader, Trait trait, MethodBody body, MethodInfo info)
     ...
     * @throws InterruptedException On interrupt
     */
    public static AVM2Code parse(
            ABC abc, Reader reader, Trait trait, MissingSymbolHandler missingHandler, MethodBody body, MethodInfo info)
            throws IOException, AVM2ParseException, InterruptedException {
                            ...
                            switch (def.operands[i]) {
                                    ...
                                    if (parsedOperand.type == ParsedSymbol.TYPE_KEYWORD_NULL) {
                                        ...
                                        if (sid == -1) {
                                            if ((missingHandler != null)
                                                    && (missingHandler.missingString((String) parsedOperand.value))) {
                                ...
                                case AVM2Code.DAT_DECIMAL_INDEX:
                                    if (parsedOperand.type == ParsedSymbol.TYPE_KEYWORD_NULL) {
                                        ...
                                        if (did == -1) {
                                            if ((missingHandler != null)
                                                    && (missingHandler.missingDecimal(decimalVal))) {
                                ...
                                case AVM2Code.DAT_OFFSET:
                                    if (parsedOperand.type == ParsedSymbol.TYPE_IDENTIFIER) {
                                        offsetItems.add(new OffsetItem(
                                                (String) parsedOperand.value, code.code.size(), i, lexer.yyline()));
                                        operandsList.add(0);
                                    } else {
                                ...
                                case AVM2Code.OPT_CASE_OFFSETS:
                                    if (parsedOperand.type == ParsedSymbol.TYPE_BRACKET_OPEN) {
                                        ...
                                        if (c == 0) {
                                            throw new AVM2ParseException(
                                                    "At least single offset expected", lexer.yyline());
                                        }
                                        ...
                                        throw new AVM2ParseException(
                                                "Bracket open [ or case count expected", lexer.yyline());
                                    }
                                    break;
                                case AVM2Code.OPT_S8:
                                    if (parsedOperand.type == ParsedSymbol.TYPE_NUMBER) {
                                        ...
                                        if (val < Byte.MIN_VALUE || val > Byte.MAX_VALUE) {
                                            throw new AVM2ParseException(
                                                    "Byte value expected (" + Byte.MIN_VALUE + " to " + Byte.MAX_VALUE
                                ...
                                case AVM2Code.OPT_S16:
                                    if (parsedOperand.type == ParsedSymbol.TYPE_NUMBER) {
                                        ...
                                        if (val < Short.MIN_VALUE || val > Short.MAX_VALUE) {
                                            throw new AVM2ParseException(
                                                    "Short value expected (" + Short.MIN_VALUE + " to "
                                                            + Short.MAX_VALUE + "). Use pushint to push larger values",
                                                    lexer.yyline());
                                        }
                                        operandsList.add((int) val);
                                    } else {
        ...
        for (int i = 0; i < exceptions.size(); i++) {
            if (!labelToOffset.containsKey(exceptionsFrom.get(i))) {
                throw new AVM2ParseException(
                        "Label " + exceptionsFrom.get(i) + " for exception from not defined", exceptionLines.get(i));
            }
            exceptions.get(i).start = labelToOffset.get(exceptionsFrom.get(i));

            if (!labelToOffset.containsKey(exceptionsTo.get(i))) {
                throw new AVM2ParseException(
                        "Label " + exceptionsTo.get(i) + " for exception to not defined", exceptionLines.get(i));
            }
            exceptions.get(i).end = labelToOffset.get(exceptionsTo.get(i));

            if (!labelToOffset.containsKey(exceptionsTargets.get(i))) {
                throw new AVM2ParseException(
                        "Label " + exceptionsTargets.get(i) + "for exception target not defined",
                        exceptionLines.get(i));
            }
