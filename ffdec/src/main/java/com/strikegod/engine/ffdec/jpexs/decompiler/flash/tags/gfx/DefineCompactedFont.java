 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.tags.gfx;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.gfx.GfxConvertor;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.types.gfx.GFxInputStream;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.types.gfx.GFxOutputStream;
...
public final class DefineCompactedFont extends FontTag {
    ...
    public final void readData(
        ...
        if (fonts.size() > 1) {
            Logger.getLogger(DefineCompactedFont.class.getName())
                    .log(
                            Level.WARNING,
                            "Compacted font has more than one FontType inside. This may cause problems while editing.");
        }
        rebuildShapeCache();
    }
    ...
    public boolean addCharacter(char character, Font cfont) {
        ...
        for (int k = 0; k < font.kerning.size(); k++) {
            if (font.kerning.get(k).char1 == character || font.kerning.get(k).char2 == character) {
    ...
    public RECT getGlyphBounds(int glyphIndex) {
        ...
        return new RECT(
                resize(gt.boundingBox[0]),
                resize(gt.boundingBox[1]),
                resize(gt.boundingBox[2]),
                resize(gt.boundingBox[3]));
    }
    ...
    public void setKerningTable(List<KERNINGRECORD> kerningTable) {
        ...
        for (KERNINGRECORD rec : kerningTable) {
            val.add(new KerningPairType(
                    rec.fontKerningCode1, rec.fontKerningCode2, (int) unresize(rec.fontKerningAdjustment)));
        }
