...
public class Polynomial {
    ...
    private double[] boundsUpperRealFujiwara() {
        ...
        return new double[] {-2 * max_neg, 2 * max_pos};
    }
    ...
    private static double newtonSecantBisection(
            double x0, DoubleFunc f, DoubleFunc df, int max_iterations, Double min, Double max) {
        double x;
        double prev_dfx = 0;
        double dfx;
    ...
    private List<Double> getQuarticRoots() {
        ...
        if (n == 4) {
            ...
            if (nr > -1) {
                ...
                if (dery.get(nr) != 0
                        && Double.compare(sign(dery.get(nr)), sign(poly.eval(derrt.get(nr) + dx) - dery.get(nr)))
                                != 0) {
            ...
            if (!guesses.isEmpty()) {
                for (i = 0; i < guesses.size(); i++) {
                    guesses.set(
                            i,
                            Polynomial.newtonSecantBisection(
                                    guesses.get(i), f, df, 32, minmax.get(i)[0], minmax.get(i)[1]));
                }
            }
