...
public class ActionIf extends Action {
    ...
    public List<Integer> getBranches(GraphSource code) {
        ...
        if (jmp == -1) {
            Logger.getLogger(ActionIf.class.getName())
                    .log(Level.SEVERE, "Invalid IF jump to ofs{0} from ofs{1} in {2}", new Object[] {
                        Helper.formatAddress(targetAddress),
                        Helper.formatAddress(getAddress()),
                        ((ActionGraphSource) code).getPath()
                    });
            ret.add(after);
        } else {
            ret.add(jmp);
