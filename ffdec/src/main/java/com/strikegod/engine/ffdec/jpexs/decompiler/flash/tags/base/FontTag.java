...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.configuration.SwfSpecificConfiguration;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.helpers.FontHelper;
...
public abstract class FontTag extends DrawableTag implements AloneTag {
    ...
    private static Map<String, Map<String, Map<Integer, List<FontHelper.KerningPair>>>>
            installedFontKerningPairsByFamily;
     ...
     */
    public static float getSystemFontAdvance(
            String fontName, int fontStyle, int fontSize, Character character, Character nextCharacter) {
    ...
    public static float getSystemFontAdvance(Font aFont, Character character, Character nextCharacter) {
        ...
        GlyphVector gv = aFont.layoutGlyphVector(
                new FontRenderContext(aFont.getTransform(), true, true),
                chars.toCharArray(),
                0,
                chars.length(),
    ...
    protected static List<FontHelper.KerningPair> getFontKerningPairs(Font font, int size) {
        ...
        if (customFontToFile.containsKey(font)) {
            if (!customFontKerningPairs.containsKey(font)
                    || !customFontKerningPairs.get(font).containsKey(size)) {
                if (!customFontKerningPairs.containsKey(font)) {
                    customFontKerningPairs.put(font, new HashMap<>());
                }
                customFontKerningPairs
                        .get(font)
                        .put(size, FontHelper.getFontKerningPairs(customFontToFile.get(font), size));
            }
        ...
        if (installedFontKerningPairsByFamily.containsKey(font.getFamily(Locale.ENGLISH))
                ...
                && installedFontKerningPairsByFamily
                        .get(font.getFamily())
                        .get(font.getFontName(Locale.ENGLISH))
                        .containsKey(size)) {
            return installedFontKerningPairsByFamily
                    .get(font.getFamily())
                    .get(font.getFontName(Locale.ENGLISH))
                    .get(size);
        }

        if (installedFontFilesByFamily.containsKey(font.getFamily(Locale.ENGLISH))
                && installedFontFilesByFamily.get(font.getFamily()).containsKey(font.getFontName(Locale.ENGLISH))) {
            File file = installedFontFilesByFamily
                    .get(font.getFamily(Locale.ENGLISH))
                    .get(font.getFontName(Locale.ENGLISH));
            if (!installedFontKerningPairsByFamily.containsKey(font.getFamily(Locale.ENGLISH))) {
                installedFontKerningPairsByFamily.put(font.getFamily(Locale.ENGLISH), new HashMap<>());
            }
            if (!installedFontKerningPairsByFamily
                    .get(font.getFamily(Locale.ENGLISH))
                    .containsKey(font.getFontName(Locale.ENGLISH))) {
                installedFontKerningPairsByFamily
                        .get(font.getFamily(Locale.ENGLISH))
                        .put(font.getFontName(Locale.ENGLISH), new HashMap<>());
            }

            installedFontKerningPairsByFamily
                    .get(font.getFamily(Locale.ENGLISH))
                    .get(font.getFontName(Locale.ENGLISH))
                    .put(size, FontHelper.getFontKerningPairs(file, size));
        }
        if (installedFontKerningPairsByFamily.containsKey(font.getFamily(Locale.ENGLISH))
                && installedFontKerningPairsByFamily.get(font.getFamily()).containsKey(font.getFontName(Locale.ENGLISH))
                && installedFontKerningPairsByFamily
    ...
    public static void reload() {
        ...
        for (String fam : installedFontFilesByFamily.keySet()) {
            for (String nam : installedFontFilesByFamily.get(fam).keySet()) {
                installedFontFilesByName.put(
                        nam, installedFontFilesByFamily.get(fam).get(nam));
            }
        }

        if (installedFontsByFamily.containsKey("Times New Roman")) {
            defaultFontName = installedFontsByFamily
                    .get("Times New Roman")
                    .keySet()
                    .iterator()
                    .next();
            ...
            defaultFontName =
                    installedFontsByFamily.get("Arial").keySet().iterator().next();
        } else {
            defaultFontName = installedFontsByFamily
                    .get(installedFontsByFamily.keySet().iterator().next())
                    .keySet()
                    .iterator()
                    .next();
        }
    ...
    @Override
    public synchronized void toImage(
            int frame,
            int time,
            int ratio,
            RenderContext renderContext,
            ...
            int aaScale) {
        SHAPERECORD.shapeListToImage(
                ShapeTag.WIND_EVEN_ODD, 1, swf, getGlyphShapeTable(), image, frame, Color.black, colorTransform);
    }
    ...
    public void toHtmlCanvas(StringBuilder result, double unitDivisor) {
        ...
        for (int i = 0; i < shapes.size(); i++) {
            ...
            CanvasShapeExporter exporter = new CanvasShapeExporter(
                    ShapeTag.WIND_EVEN_ODD, 1, null, unitDivisor, swf, shapes.get(i), null, 0, 0);
            exporter.export();
