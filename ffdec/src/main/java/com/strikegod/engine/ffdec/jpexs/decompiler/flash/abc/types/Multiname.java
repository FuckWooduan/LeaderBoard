...
public class Multiname {
     ...
     * Multiname kinds
     */
    private static final int[] multinameKinds = new int[] {
        QNAME, QNAMEA, MULTINAME, MULTINAMEA, RTQNAME, RTQNAMEA, MULTINAMEL, RTQNAMEL, RTQNAMELA, MULTINAMELA, TYPENAME
    };

    /**
     * Multiname kind names
     */
    private static final String[] multinameKindNames = new String[] {
        "QName",
        "QNameA",
        "Multiname",
        "MultinameA",
     ...
     */
    private Multiname(
            int kind, int name_index, int namespace_index, int namespace_set_index, int qname_index, int[] params) {
        this.kind = kind;
    ...
    public static String namespaceToString(AVM2ConstantPool constants, int index) {
        ...
        String name = name_index == 0
                ? null
                : constants.getNamespace(index).getName(constants).toRawString();
        int sub = -1;
        ...
        return constants.getNamespace(index).getKindStr() + "("
                + (name == null ? "null" : "\"" + Helper.escapePCodeString(name) + versionAdd + "\"")
                + (sub > 0 ? ",\"" + sub + "\"" : "") + ")";
    }
     ...
     * @return Multiname as string
     */
    private static String multinameToString(
            AVM2ConstantPool constants, int index, List<DottedChain> fullyQualifiedNames) {
        if (index == 0) {
            return "null";
        }
    ...
    public String toString(AVM2ConstantPool constants, List<DottedChain> fullyQualifiedNames) {
        ...
        switch (kind) {
            case QNAME:
            case QNAMEA:
                return getKindStr() + "(" + namespaceToString(constants, namespace_index) + ","
                        + (name_index == 0
                                ? "null"
                                : "\"" + Helper.escapePCodeString(constants.getString(name_index)) + "\"")
                        + ")";
            case RTQNAME:
            case RTQNAMEA:
                return getKindStr() + "("
                        + (name_index == 0 ? "null" : "\"" + Helper.escapePCodeString(constants.getString(name_index)))
                        + "\"" + ")";
            case RTQNAMEL:
            case RTQNAMELA:
                return getKindStr() + "()";
            ...
            case MULTINAMEA:
                return getKindStr() + "("
                        + (name_index == 0
                                ? "null"
                                : "\"" + Helper.escapePCodeString(constants.getString(name_index)) + "\"")
                        + "," + namespaceSetToString(constants, namespace_set_index) + ")";
            case MULTINAMEL:
            case MULTINAMELA:
     ...
     */
    private String typeNameToStr(
            Set<String> usedDeobfuscations,
            ABC abc,
            AVM2ConstantPool constants,
            List<DottedChain> fullyQualifiedNames,
            boolean dontDeobfuscate,
            boolean withSuffix) {
        if (cyclic) {
            return "§§cyclic_typename()";
        }
        StringBuilder typeNameStr = new StringBuilder();
        typeNameStr.append(constants
                .getMultiname(qname_index)
                .getName(usedDeobfuscations, abc, constants, fullyQualifiedNames, dontDeobfuscate, withSuffix));
    ...
    public String getNameWithCustomNamespace(
            ...
            boolean withSuffix) {
        if (kind == TYPENAME) {
            return typeNameToStr(
                    usedDeobfuscations, abc, abc.constants, fullyQualifiedNames, dontDeobfuscate, withSuffix);
        }
        if (name_index == -1) {
            return "";
        }
        if (name_index == 0) {
            ...
            if (nskind == Namespace.KIND_NAMESPACE || nskind == Namespace.KIND_PACKAGE_INTERNAL) {
                ...
                if (nsname != null && !"AS3".equals(nsname)) {
                    String identifier = dontDeobfuscate
                            ? nsname
            ...
            if (nskind == Namespace.KIND_PACKAGE
                    ...
                    && !fullyQualifiedNames.isEmpty()
                    && fullyQualifiedNames.contains(DottedChain.parseWithSuffix(name))) {
                DottedChain dc = getNameWithNamespace(usedDeobfuscations, abc, abc.constants, withSuffix);
                return dontDeobfuscate
                        ? dc.toRawString()
                        : dc.toPrintableString(usedDeobfuscations, abc.getSwf(), true);
            }
            return (isAttribute() ? "@" : "")
                    + (dontDeobfuscate
                            ? name
                            : IdentifiersDeobfuscation.printIdentifier(abc.getSwf(), usedDeobfuscations, true, name))
                    + (withSuffix ? getNamespaceSuffix() : "");
        }
    }
     ...
     * @return Name with custom namespace
     */
    public String getNameAndCustomNamespace(
            Set<String> usedDeobfuscations,
            ABC abc,
            List<DottedChain> fullyQualifiedNames,
            boolean dontDeobfuscate,
            boolean withSuffix,
            Reference<DottedChain> customNamespaceRef) {
        if (kind == TYPENAME) {
            return typeNameToStr(
                    usedDeobfuscations, abc, abc.constants, fullyQualifiedNames, dontDeobfuscate, withSuffix);
        }
     ...
     */
    public DottedChain getNameWithNamespace(
            Set<String> usedDeobfuscations, ABC abc, AVM2ConstantPool constants, boolean withSuffix) {
    ...
    public boolean isMULTINAMEwithOneNs(AVM2ConstantPool pool) {
        return (kind == MULTINAME || kind == MULTINAMEL || kind == MULTINAMELA)
                && pool.getNamespaceSet(namespace_set_index).namespaces.length == 1;
    }
     ...
     */
    public boolean qnameEquals(
            ABC thisAbc, AVM2ConstantPool thisCpool, Multiname other, ABC otherAbc, AVM2ConstantPool otherCpool) {
        ...
        if (thisNs != null && otherNs != null) {
            ...
            if (!Objects.equals(
                    otherNs.getName(otherCpool).toRawString(),
                    thisNs.getName(thisCpool).toRawString())) {
                return false;
            }
        }
        ...
        if (!Objects.equals(
                other.getName(new LinkedHashSet<>(), otherAbc, otherCpool, new ArrayList<>(), true, true),
                getName(new LinkedHashSet<>(), thisAbc, thisCpool, new ArrayList<>(), true, true))) {
            return false;
        }
