...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.modes.TextExportMode;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.settings.TextExportSettings;
...
public class TextExporter {
    ...
    public List<File> exportTexts(
            AbortRetryIgnoreHandler handler,
            String outdir,
            ReadOnlyTagList tags,
            final TextExportSettings settings,
            EventListener evl)
            ...
            for (Tag t : tags) {
                if (t instanceof TextTag) {
                    ...
                    final File file = new File(outdir
                            + File.separator
                            + Helper.makeFileName(textTag.getCharacterExportFileName() + ".svg"));
                    new RetryTask(
                                    () -> {
        ...
        if (settings.singleFile) {
            ...
            if (fileName != null && !fileName.isEmpty()) {
                String swfName = Path.getFileNameWithoutExtension(
                        new File(tags.get(0).getSwf().getShortFileName()));
                fileName = fileName.replace("{fileName}", swfName);
            } else {
                fileName = settings.mode == TextExportMode.FORMATTED
                        ? TEXT_EXPORT_FILENAME_FORMATTED
                        : TEXT_EXPORT_FILENAME_PLAIN;
            }
                    ...
                    if (t instanceof TextTag) {
                        ...
                        new RetryTask(
                                        () -> {
                                            ...
                                            fos.write(Utf8Helper.getBytes(Helper.newLine
                                                    + Configuration.textExportSingleFileSeparator.get()
                                                    + Helper.newLine));
                                        },
                                        handler)
                                .run();
            ...
            for (Tag t : tags) {
                if (t instanceof TextTag) {
                    ...
                    final File file = new File(outdir
                            + File.separator
                            + Helper.makeFileName(textTag.getCharacterExportFileName() + ".txt"));
                    new RetryTask(
                                    () -> {
                                        try (OutputStream fos = new BufferedOutputStream(new FileOutputStream(file))) {
                                            if (settings.mode == TextExportMode.FORMATTED) {
