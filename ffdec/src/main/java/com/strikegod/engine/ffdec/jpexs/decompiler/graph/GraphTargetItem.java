...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.helpers.hilight.HighlightData;
import com.strikegod.engine.ffdec.jpexs.decompiler.graph.model.BinaryOp;
...
public abstract class GraphTargetItem implements Serializable, Cloneable {
    ...
    public GraphTargetItem(
            GraphTargetDialect dialect, GraphSourceItem src, GraphSourceItem lineStartItem, int precedence) {
     ...
     * @throws InterruptedException On interrupt
     */
    public final GraphTextWriter toStringBoolean(GraphTextWriter writer, LocalData localData)
            throws InterruptedException {
     ...
     * @throws InterruptedException On interrupt
     */
    public final GraphTextWriter toStringString(GraphTextWriter writer, LocalData localData)
            throws InterruptedException {
     ...
     * @throws InterruptedException On interrupt
     */
    public final GraphTextWriter toStringNumber(GraphTextWriter writer, LocalData localData)
            throws InterruptedException {
    ...
    public GraphTextWriter toString(
            GraphTextWriter writer, LocalData localData, String implicitCoerce, boolean noParenthesis)
            throws InterruptedException {
        if (CancellableWorker.isInterrupted()) {
            throw new InterruptedException();
        }
     ...
     * @throws InterruptedException On interrupt
     */
    public GraphTextWriter appendNoParenthesis(GraphTextWriter writer, LocalData localData)
            throws InterruptedException {
     ...
     * @throws InterruptedException On interrupt
     */
    public GraphTextWriter appendTry(
            GraphTextWriter writer, LocalData localData, String implicitCoerce, boolean noParenthesis)
            throws InterruptedException {
    ...
    public static List<GraphSourceItem> toSourceMerge(
            SourceGeneratorLocalData localData, SourceGenerator gen, Object... tar) throws CompilationException {
        List<GraphSourceItem> ret = new ArrayList<>();
        for (Object o : tar) {
            if (o == null) {
                continue;
     ...
     * @throws InterruptedException On interrupt
     */
    public GraphTextWriter appendCommands(
            GraphTargetItem prevLineItem,
        ...
        // This may be useful in the future, but we must handle obfuscated SWFs where there is only one debugline
        ...
        for (GraphTargetItem ti : commands) {
            if (!ti.isEmpty()) {
                if (!first
                        && (!useLineInfo
                                || (ti.getLine() < 1 || prevLine < 1 || (prevLine >= 1 && prevLine != ti.getLine())))) {
     ...
     * @throws InterruptedException On interrupt
     */
    public GraphTextWriter appendBlock(
            GraphTargetItem prevLineItem, GraphTextWriter writer, LocalData localData, List<GraphTargetItem> commands)
            throws InterruptedException {
    ...
    public static void checkDup(
            TranslateStack stack, List<GraphTargetItem> output, GraphTargetItem dupSource, GraphTargetItem dup) {
        if (output.isEmpty()) {
            return;
        }
     ...
     */
    public static int checkDup2(
            GraphTargetItem item1, GraphTargetItem item2, List<GraphTargetItem> output, TranslateStack stack) {
        if (item1 == item2) {
            return -1;
        }
