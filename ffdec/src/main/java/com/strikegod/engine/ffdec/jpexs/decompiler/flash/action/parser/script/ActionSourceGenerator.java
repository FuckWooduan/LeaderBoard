...
public class ActionSourceGenerator implements SourceGenerator {
    ...
    private List<GraphSourceItem> generateIf(
        ...
        if (onTrueLen > 32767) {
            throw new CompilationException(
                    "Generated offset for onTrue part of " + errorTitle + " is larger than maximum allowed for SI16.",
                    errorLine);
        }
        ...
        if (onFalse != null) {
            ...
            if (onTrueLen > 32767) {
                throw new CompilationException(
                        "Generated offset for onTrue part of " + errorTitle
                                + " is larger than maximum allowed for SI16.",
                        errorLine);
            }
            ifaif.setJumpOffset(onTrueLen);
            ...
            if (onTrueLen > 32767) {
                throw new CompilationException(
                        "Generated offset for onFalse part of " + errorTitle
                                + " is larger than maximum allowed for SI16.",
                        errorLine);
            }
            ret.addAll(onFalse);
    ...
    private void fixLoop(List<Action> code, int breakOffset, int continueOffset, GraphTargetItem item)
            throws CompilationException {
        int pos = 0;
        for (Action a : code) {
            ...
            if (a instanceof ActionJump) {
                ...
                if (aj.isContinue && (continueOffset != Integer.MAX_VALUE)) {
                    ...
                    if (nOffset < -32768 || nOffset > 32767) {
                        throw new CompilationException(
                                "Generated offset for Continue is outside bounds of SI16.", item.line);
                    }
                ...
                if (aj.isBreak) {
                    ...
                    if (nOffset < -32768 || nOffset > 32767) {
                        throw new CompilationException(
                                "Generated offset for Break is outside bounds of SI16.", item.line);
                    }
                    aj.setJumpOffset(nOffset);
                    aj.isBreak = false;
                }
    ...
    private GraphTargetItem addGlobalPrefix(GraphTargetItem item) {
        ...
        if (item instanceof GetVariableActionItem) {
            ...
            item = new GetMemberActionItem(
                    null,
                    null,
                    new GetVariableActionItem(
                            null, null, new DirectValueActionItem(null, null, 0, "_global", new ArrayList<>())),
                    v.name);
            if (mem != null) {
                mem.object = item;
            }
     ...
     */
    public List<GraphSourceItem> generateTraits(
            SourceGeneratorLocalData localData,
            boolean isInterface,
        ...
        String constructorName = nameStr.get(nameStr.size() - 1); // com.strikegod.engine.ffdec.jpexs.MyClass => MyClass
                ...
                for (int t = 0; t < traits.size(); t++) {
                    ...
                    if (pass == 1 && isFunc) { // Add methods in first pass
                        ...
                        if (fi.isGetter || fi.isSetter) {
                            (traitsStatic.get(t) ? staticProperties : properties)
                                    .add(fi.calculatedFunctionName.toString());
                        }

                        if (!FunctionActionItem.DECOMPILE_GET_SET) {
                            if (fi.calculatedFunctionName.toString().startsWith("__get__")
                                    || fi.calculatedFunctionName.toString().startsWith("__set__")) {
                                (traitsStatic.get(t) ? staticProperties : properties)
                                        .add(fi.calculatedFunctionName
                                                .toString()
                                                .substring(7));
                            }
                            if (fi.calculatedFunctionName.toString().startsWith("__get__")) {
                                (traitsStatic.get(t) ? staticGetters : getters)
                                        .add(fi.calculatedFunctionName
                                                .toString()
                                                .substring(7));
                            }
                            if (fi.calculatedFunctionName.toString().startsWith("__set__")) {
                                (traitsStatic.get(t) ? staticSetters : setters)
                                        .add(fi.calculatedFunctionName
                                                .toString()
                                                .substring(7));
                            }
        ...
        if (ifOffset > 0x7fff) {
            if (!localData.secondRun) { // to avoid printing the warning twice
                Logger.getLogger(ActionSourceGenerator.class.getName())
                        .warning(
                                "The class is long. If offset for its header would be longer than max value for SI16, 0 was stored instead. This should not have any negative impact.");
            }
            ifOffset = 0;
        }
    ...
    public List<GraphSourceItem> generate(SourceGeneratorLocalData localData, WhileItem item)
        ...
        if (whileBodyLen > 32767) {
            throw new CompilationException(
                    "Generated offset for While is larger than maximum allowed for SI16.", item.line);
        }

        int whileJumpOffset = -(whileExprLen + whileBodyLen);
        if (whileJumpOffset < -32768) {
            throw new CompilationException(
                    "Generated offset for While is lower than minimum allowed for SI16.", item.line);
        }
    ...
    public List<GraphSourceItem> generate(SourceGeneratorLocalData localData, DoWhileItem item)
        ...
        if (-offset < -32768) {
            throw new CompilationException(
                    "Generated offset for DoWhile is lower than minimum allowed for SI16.", item.line);
        }
    ...
    public List<GraphSourceItem> generate(SourceGeneratorLocalData localData, ForInActionItem item)
            throws CompilationException {
        List<GraphSourceItem> ret = new ArrayList<>();
    ...
    public List<GraphSourceItem> generate(SourceGeneratorLocalData localData, ForItem item)
        ...
        if (jmpOffset < -32768) {
            throw new CompilationException(
                    "Generated offset for For is lower than minimum allowed for SI16.", item.line);
        }
        if (ifOffset > 32767) {
            throw new CompilationException(
                    "Generated offset for For is larger than maximum allowed for SI16.", item.line);
        }
    ...
    private int checkOffsetBounds(int offset, String errorItem, int errorLine) throws CompilationException {
        if (offset < -32768) {
            throw new CompilationException(
                    "Generated offset for " + errorItem + " is lower than minimum allowed for SI16.", errorLine);
        }
        if (offset > 32767) {
            throw new CompilationException(
                    "Generated offset for " + errorItem + " is larger than maximum allowed for SI16.", errorLine);
        }
        return offset;
    }
}
