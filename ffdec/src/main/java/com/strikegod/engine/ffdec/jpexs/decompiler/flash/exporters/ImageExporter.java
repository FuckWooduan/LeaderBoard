...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.tags.base.HasSeparateAlphaChannel;
        ...
        for (Tag t : tags) {
            if (t instanceof ImageTag) {
                ...
                final File file = new File(outdir
                        + File.separator
                        + Helper.makeFileName(imageTag.getCharacterExportFileName() + "."
                                + ImageHelper.getImageFormatString(fileFormat)));
                ...
                new RetryTask(
                                () -> {
                                    if (ffileFormat == originalFormat) {
                                        ...
                                        BMPFile.saveBitmap(
                                                imageTag.getImageCached().getBufferedImage(), file);
                                    } else if (ffileFormat == ImageFormat.WEBP) {
                                        try (OutputStream fos = new BufferedOutputStream(new FileOutputStream(file))) {
                                            byte[] webPData;
                                            if (originalFormat == ImageFormat.PNG
                                                    || originalFormat == ImageFormat.GIF) {
                                                webPData = WebPCodec.encodeLosslessImage(imageTag.getImageCached()
                                                        .getBufferedImage());
                                            } else {
                                                webPData = WebPCodec.encodeImage(
                                                        imageTag.getImageCached()
                                                                .getBufferedImage(),
                                                        100f);
                                            }
                                            fos.write(webPData);
                                        ...
                                        ImageHelper.write(
                                                imageTag.getImageCached().getBufferedImage(), ffileFormat, file);
                                    }
                                },
                                handler)
                        .run();

                final File alphaPngFile = new File(outdir
                        + File.separator
                        + Helper.makeFileName(imageTag.getCharacterExportFileName() + ".alpha.png"));
                    ...
                    for (String className : classNames) {
                        if (Configuration.autoDeobfuscateIdentifiers.get()) {
                            className = DottedChain.parseNoSuffix(className)
                                    .toPrintableString(new LinkedHashSet<>(), imageTag.getSwf(), true);
                        }
                        File classFile = new File(outdir
                                + File.separator
                                + Helper.makeFileName(className + "." + ImageHelper.getImageFormatString(fileFormat)));
                        File classAlphaPngFile =
                                new File(outdir + File.separator + Helper.makeFileName(className + ".alpha.png"));
                        new RetryTask(
                                        () -> {
                                            Files.copy(
                                                    file.toPath(),
