...
public class AVM2Deobfuscation {
    ...
    private String fooString(
            HashMap<DottedChain, DottedChain> deobfuscated,
            String orig,
     ...
     */
    public int deobfuscatePackageName(
        ...
        if (!isValidPackageName(strIndex)) {
            ...
            if (namesMap.containsKey(sChain)) {
                ...
                return constants.getStringId(
                        newName.toRawString(), true); // constants.setString(strIndex, newName.toRawString());
            }
    ...
    public int deobfuscateName(
        ...
        if (!isValidName(strIndex)) {
            ...
            String str = fooString(
                    namesMap,
                    constants.getString(strIndex),
                    firstUppercase,
                    stringUsageTypes.get(strIndex),
                    renameType,
                    false);
            newname = DottedChain.parseNoSuffix(str);

            if (stringUsages.contains(strIndex)
                    || namespaceUsages.contains(strIndex)) { // this name is already referenced as String
