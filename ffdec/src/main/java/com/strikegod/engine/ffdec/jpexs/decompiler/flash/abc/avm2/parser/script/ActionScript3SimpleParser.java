...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.CatchScope;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.ClassScope;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.ClassTrait;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.FunctionScope;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.Import;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.LinkHandler;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.MethodScope;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.Namespace;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.Path;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.Separator;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.SimpleParseException;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.SimpleParser;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.TraitVarConstValueScope;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.Type;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.Variable;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.simpleparser.VariableOrScope;
...
public class ActionScript3SimpleParser implements SimpleParser {
    ...
    private void commands(
            ...
            ABC abc)
            throws IOException, AVM2ParseException, SimpleParseException, InterruptedException {
        if (debugMode) {
                ...
                if (s.type == SymbolType.NAMESPACE_OP) {
                    ...
                    if (nsKeyword != null) {
                        errors.add(new SimpleParseException(
                                nsKeyword.value + " not expected in this situation",
                                lexer.yyline(),
                                nsKeyword.position));
                    }
    ...
    private Path name(
        ...
        if (s.type == SymbolType.NAMESPACESUFFIX) {
            if (nsKeyword != null) {
                errors.add(new SimpleParseException(
                        nsKeyword.value + " not expected in this situation", lexer.yyline(), nsKeyword.position));
            }
            s = lex();
            lastName += "#" + s.value;
        }
        ...
        while (s.isType(SymbolType.DOT)) {
            ...
            if (s.type == SymbolType.ATTRIBUTE) {
                ...
                if (s.type == SymbolType.MULTIPLY) {
                    ...
                    if (s.type != SymbolType.BRACKET_OPEN) {
                        errors.add(new SimpleParseException(
                                "Attribute identifier or bracket expected", lexer.yyline(), s.position));
                    }
                    attrBracket = true;
                    continue;
                }
        ...
        if (s.type == SymbolType.NAMESPACE_OP) {
            ...
            if (s.group == SymbolGroup.IDENTIFIER) {
                ...
                variables.add(new Variable(
                        false, fullName.getParent().add(fullName.getLast().toString() + "::" + nsprop), s.position));
            } else if (s.type == SymbolType.BRACKET_OPEN) {
                expression(
                        errors,
                        thisType,
    ...
    private boolean expected(List<SimpleParseException> errors, ParsedSymbol symb, int line, Object... expected)
            throws IOException, AVM2ParseException, SimpleParseException {
        boolean found = false;
        ...
        if (!found) {
            ...
            errors.add(new SimpleParseException(
                    "" + expStr + " expected but " + symb.type + " found", line, symb.position));
        }
        return found;
    }

    private ParsedSymbol expectedType(List<SimpleParseException> errors, Object... type)
            throws IOException, AVM2ParseException, SimpleParseException, InterruptedException {
    ...
    private void method(
            ...
            ABC abc,
            int methodNamePos,
            Reference<Path> returnTypeRef,
            Reference<Variable> returnSubTypeRef)
            throws IOException, AVM2ParseException, SimpleParseException, InterruptedException {
        function(
                errors,
                isInterface,
    ...
    private void function(
        ...
        if (hasRest) {
            subvariables.add(new Variable(
                    ...
                    paramPositions.get(paramNames.size() - 1),
                    null,
                    new Path("Array"),
                    null));
        }
    ...
    private void parseMetadata(List<SimpleParseException> errors)
            throws IOException, AVM2ParseException, SimpleParseException, InterruptedException {
        ParsedSymbol s = lex();
    ...
    private void classTraits(
            ...
            ABC abc,
            List<VariableOrScope> classVariables)
        ...
        while (true) {
            ...
            while (s.isType(
                ...
                if (s.type == SymbolType.FINAL) {
                    if (isFinal) {
                        errors.add(
                                new SimpleParseException("Only one final keyword allowed", lexer.yyline(), s.position));
                    }
                    preSymbols.add(s);
                    isFinal = true;
                    ...
                    if (isOverride) {
                        errors.add(new SimpleParseException(
                                "Only one override keyword allowed", lexer.yyline(), s.position));
                    }
                    preSymbols.add(s);
                    isOverride = true;
                    ...
                    if (isInterface) {
                        errors.add(new SimpleParseException(
                                "Interface cannot have static traits", lexer.yyline(), s.position));
                    }
                    if (classNameStr == null) {
                        errors.add(
                                new SimpleParseException("No static keyword allowed here", lexer.yyline(), s.position));
                    }
                    if (isStatic) {
                        errors.add(new SimpleParseException(
                                "Only one static keyword allowed", lexer.yyline(), s.position));
                    }
                    preSymbols.add(s);
                    isStatic = true;
                    ...
                    if (isNative) {
                        errors.add(new SimpleParseException(
                                "Only one native keyword allowed", lexer.yyline(), s.position));
                    }
                    preSymbols.add(s);
                    isNative = true;
                } else if (s.group == SymbolGroup.IDENTIFIER) {
            ...
            switch (s.type) {
                case FUNCTION:
                    ...
                    if (fname.toString().equals(classNameStr)) { // constructor
                        if (isStatic) {
                            errors.add(new SimpleParseException(
                                    "Constructor cannot be static", lexer.yyline(), s.position));
                        }
                        if (isOverride) {
                            errors.add(new SimpleParseException(
                                    "Override flag not allowed for constructor", lexer.yyline(), s.position));
                        }
                        if (isFinal) {
                            errors.add(new SimpleParseException(
                                    "Final flag not allowed for constructor", lexer.yyline(), s.position));
                        }
                        if (isInterface) {
                            errors.add(new SimpleParseException(
                                    "Interface cannot have constructor", lexer.yyline(), s.position));
                        }
                        method(
                                errors,
                                outsidePackage,
                        ...
                        traitVariables.add(new ClassTrait(
                                fullClassName,
                                fname,
                                customNs,
                                fnamePos,
                                isStatic,
                                new Path("Function"),
                                returnTypeRef.getVal(),
                                null,
                                returnSubTypeRef.getVal()));
                ...
                case NAMESPACE:
                    if (isInterface) {
                        errors.add(new SimpleParseException(
                                "Interface cannot have namespace fields", lexer.yyline(), s.position));
                    }
                    s = lex();
                    if (!expected(errors, s, lexer.yyline(), SymbolGroup.IDENTIFIER)) {
                        break;
                    ...
                    traitVariables.add(new ClassTrait(
                            fullClassName, nname, customNs, npos, isStatic, new Path("Namespace"), null, null, null));
                    s = lex();
                ...
                case VAR:
                    ...
                    if (isOverride) {
                        errors.add(new SimpleParseException(
                                "Override flag not allowed for " + (isConst ? "consts" : "vars"),
                                lexer.yyline(),
                                s.position));
                    }
                    if (isFinal) {
                        errors.add(new SimpleParseException(
                                "Final flag not allowed for " + (isConst ? "consts" : "vars"),
                                lexer.yyline(),
                                s.position));
                    }
                    if (isInterface) {
                        errors.add(new SimpleParseException(
                                "Interface cannot have variable/const fields", lexer.yyline(), s.position));
                    }
                    ...
                    traitVariables.add(new ClassTrait(
                            fullClassName,
                            new Path(nameSymbol.value.toString()),
                            customNs,
                            nameSymbol.position,
                            isStatic,
                            traitType,
                            "Function".equals(traitType) ? new Path("*") : null,
                            subTypeRef.getVal(),
                            null));
                    ...
                    if (s.type == SymbolType.ASSIGN) {
                        ...
                        s = lex();
                        classVariables.add(
                                new TraitVarConstValueScope(scopePos, s.position, constVarVariables, isStatic));
                    }
                    if (s.type != SymbolType.SEMICOLON) {
                        lexer.pushback(s);
                    }
    ...
    private boolean scriptTraitsBlock(
            ...
            List<VariableOrScope> sinitVariables,
            List<Path> externalTypes)
            throws AVM2ParseException, SimpleParseException, SimpleParseException, IOException, CompilationException,
        ...
        while (true) {
            ...
            while (s.isType(
                ...
                if (s.type == SymbolType.INTERNAL) {
                    if (!inPackage) {
                        errors.add(new SimpleParseException(
                                "internal only allowed inside package", lexer.yyline(), s.position));
                    }
                    if (isPublic || isInternal) {
                        errors.add(new SimpleParseException(
                                "Only one public/internal keyword allowed", lexer.yyline(), s.position));
                    }
                    isInternal = true;
                    // ns = internalNs;
                    preSymbols.add(s);
                ...
                if (s.type == SymbolType.PUBLIC) {
                    if (!inPackage) {
                        errors.add(new SimpleParseException(
                                "public only allowed inside package", lexer.yyline(), s.position));
                    }
                    if (isPublic || isInternal) {
                        errors.add(new SimpleParseException(
                                "Only one public/internal keyword allowed", lexer.yyline(), s.position));
                ...
                if (s.type == SymbolType.DYNAMIC) {
                    if (isDynamic) {
                        errors.add(new SimpleParseException(
                                "Only one dynamic keyword allowed", lexer.yyline(), s.position));
                    }
                    isDynamic = true;
                    preSymbols.add(s);
                }
            ...
            switch (s.type) {
                ...
                case INTERFACE:
                    ...
                    classVariables.add(
                            new Variable(true, new Path("this"), s.position, false, pkgName.add(subNameStr), null));
                ...
                case FUNCTION:
                    ...
                    sinitTraitVariables.add(new Variable(
                            true,
                            new Path(fname),
                            s.position,
                            true,
                ...
                case VAR:
                    ...
                    sinitTraitVariables.add(new Variable(
                            true,
                            new Path(traitSymb.value.toString()),
                            traitSymb.position,
                            null,
                            traitType,
                            traitType.equals("Function") ? new Path("*") : null,
                            subTypeRef.getVal(),
                            null));
                ...
                case NAMESPACE:
                    ...
                    if (isFinal) {
                        errors.add(new SimpleParseException(
                                "Final flag not allowed for namespaces", lexer.yyline(), s.position));
                    }
                    s = lex();
                    if (!expected(errors, s, lexer.yyline(), SymbolGroup.IDENTIFIER)) {
                        break;
                    ...
                    sinitTraitVariables.add(new Variable(
                            true, new Path(s.value.toString()), s.position, null, new Path("Namespace"), null));
                ...
                default:
                    ...
                    if (parseImportsUsages(
                            errors, sinitVariables, importedClasses, openedNamespaces, abc, externalTypes)) {
                        break;
                    }
                    boolean cmd = command(
                            errors,
            ...
            switch (s.type) {
                ...
                case CONST:
                    ...
                    if (s.type == SymbolType.ASSIGN) {
                        expression(
                                ...
                                abc);
                        variables.add(new Variable(
                                true, varIdentifier, varPos, null, varType, null, varSubTypeRef.getVal(), null));
                        ret = true;
                    } else {
                        variables.add(new Variable(
                                true, varIdentifier, varPos, null, varType, null, varSubTypeRef.getVal(), null));
                ...
                case BREAK:
                    ...
                    if (s.group == SymbolGroup.IDENTIFIER) {
                        ...
                        if (bloopId == 0) {
                            errors.add(new SimpleParseException(
                                    "Identifier of loop expected", lexer.yyline(), s.position));
                        }
                    } else {
                        lexer.pushback(s);
                ...
                case TRY:
                    ...
                    while (s.type == SymbolType.CATCH) {
                        ...
                        s = lex();
                        expected(errors, s, lexer.yyline(), SymbolType.CURLY_CLOSE);
                        variables.add(new CatchScope(
                                eScopePos,
                                s.position,
                                new Variable(true, enamestr, ePos, null, catchType, null, catchSubType.getVal(), null),
                                catchVars));
                        s = lex();
                        found = true;
                    }
    ...
    private boolean expression(
        ...
        ParsedSymbol symb;
        do {
            boolean prim = expressionPrimary(
                    errors,
                    thisType,
                    needsActivation,
                    importedClasses,
    ...
    private ParsedSymbol peekExprToken()
            throws IOException, AVM2ParseException, SimpleParseException, InterruptedException {
    ...
    private boolean expressionPrimary(
            List<SimpleParseException> errors,
            TypeItem thisType,
            Reference<Boolean> needsActivation,
            List<DottedChain> importedClasses,
            ...
            case PREPROCESSOR:
                ...
                switch ("" + s.value) {
                    ...
                    case "goto":
                        ...
                        // errors.add(new SimpleParseException("Compiling §§" + s.value + " is not available, sorry",
                        // lexer.yyline(), s.position));
                        ret = true;
                        break;
                    case "multiname":
                    ...
                    default:
                        errors.add(new SimpleParseException(
                                "Unknown preprocessor instruction: §§" + s.value, lexer.yyline(), s.position));
                        break;
                }
                expectedType(errors, SymbolType.PARENT_CLOSE);
                break;
            ...
            case FLOAT4:
                if (!abc.hasFloat4Support()) {
                    ...
                    lexer.yypushbackstr(
                            ...
                            "float4".length());
                    lexer.pushback(new ParsedSymbol(
                            lexer.yychar() /*???*/, SymbolGroup.IDENTIFIER, SymbolType.IDENTIFIER, "float4"));
                    name(
                            errors,
                            thisType,
                            needsActivation,
                        ...
                        switch (pragmaItemName) {
                            ...
                            case "precision":
                                ...
                                if (precision < 1 || precision > 34) {
                                    errors.add(new SimpleParseException(
                                            "Invalid precision - must be between 1 and 34",
                                            lexer.yyline(),
                                            s.position));
                                }
                                break;
    ...
    public void parse(
        ...
        SimpleParser.parseVariablesList(
                ...
                linkHandler,
                referenceToExternalTraitKey,
                externalTraitKeyToReference,
                separatorPosToType,
                separatorIsStatic,
                localTypeTraits,
                definitionToType,
                definitionToCallType,
                caretPosition,
                variableSuggestions);
    }
