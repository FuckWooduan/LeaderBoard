...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.iggy.streams.DataStreamInterface;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.iggy.streams.IggyIndexBuilder;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.iggy.streams.RandomAccessFileDataStream;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.iggy.streams.ReadDataStreamInterface;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.iggy.streams.SeekMode;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.iggy.streams.StructureInterface;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.iggy.streams.TemporaryDataStream;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.iggy.streams.WriteDataStreamInterface;
import java.io.File;
    ...
    public static void extractIggyFile(File iggyFile, File extractDir) throws IOException {
        ...
        for (int i = 0; i < ir.getNumEntries(); i++) {
            ...
            try (FileOutputStream fos =
                    new FileOutputStream(new File(extractDir, String.format(FILENAME_FORMAT, i, entry.type)))) {
    ...
    public boolean updateFlashEntry() throws IOException {
        ...
        for (int i = 0; i < subFileEntries.size(); i++) {
            ...
            if (entry.type == IggySubFileEntry.TYPE_FLASH) {
                ...
                entry.size2 = newSize;
                offsetsChange =
                        offsetsChange + (newSize - oldSize); // entries after this one will have modified offsets
                subFileEntriesData.set(i, replacementData);
            }
        }

        removeIndexEntries();
        IggySubFileEntry indexEntry = new IggySubFileEntry(
                IggySubFileEntry.TYPE_INDEX,
                replacementIndexData.length,
                replacementIndexData.length,
                0 /*offset will be set automatically*/);
