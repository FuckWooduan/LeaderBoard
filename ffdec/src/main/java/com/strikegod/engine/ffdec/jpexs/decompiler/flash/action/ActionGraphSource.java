...
public class ActionGraphSource extends GraphSource {
    ...
    public void translatePart(
        ...
        Action.actionsPartToTree(
                localData.usedDeobfuscations,
                output,
                (ActionGraph) graph,
                localData.secondPassData,
                this.insideDoInitAction,
                fi,
                registerNames,
                variables,
                functions,
        ...
        if (ret == -1) {
            ...
            if (ret == -1) {
                Logger.getLogger(ActionGraphSource.class.getName())
                        .log(Level.SEVERE, "address loc{1} not found in {0}", new Object[] {
                            path, Helper.formatAddress(adr)
                        });
            }
        }
        return ret;
