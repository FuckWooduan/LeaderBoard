...
public class PropertyAVM2Item extends AssignableAVM2Item {
    ...
    public AssignableAVM2Item copy() {
        PropertyAVM2Item p = new PropertyAVM2Item(
                object,
                attribute,
                propertyName,
                namespaceSuffix,
    ...
    public void resolve(
            ...
            Reference<GraphTargetItem> objectType,
            Reference<GraphTargetItem> propertyType,
            Reference<Integer> propertyIndex,
            Reference<ValueKind> propertyValue,
            Reference<ABC> propertyValueABC,
            Reference<Trait> propertyTrait)
        ...
        if (nsKeyword != null) {
            switch (nsKeyword) {
                case "public":
                    int publicNs =
                            abcIndex.getSelectedAbc().constants.getNamespaceId(Namespace.KIND_PACKAGE, "", 0, false);
                    ;
                    ...
                    if (nsKeyword == null) {
                        for (MethodBody b : callStack) {
                            for (int i = 0; i < b.traits.traits.size(); i++) {
                                ...
                                if (t.getName(abc)
                                        .getName(new LinkedHashSet<>(), abc, constants, null, true, true)
                                        .equals(propertyName)) {
                                    if (t instanceof TraitSlotConst) {
                                        TraitSlotConst tsc = (TraitSlotConst) t;
                                        objType = new TypeItem(DottedChain.FUNCTION);
                                        propType = AbcIndexing.multinameToType(
                                                new LinkedHashSet<>(), tsc.type_index, abc, constants);
                ...
                if (!foundInCallStack) {
                    if (ttype instanceof TypeItem) {
                        ...
                        if ((object instanceof NameAVM2Item)
                                && "super".equals(((NameAVM2Item) object).getVariableName())) {
                            // super is special cause its static type is the super class, but it still allows access to
                            // protected members
                            // so for super to work we need to also allow the protected namespace of the super class
                            // however this namespace is in the ABC of the super class and not in
                            ...
                            AbcIndexing.TraitIndex sp = abcIndex.findProperty(
                                    new AbcIndexing.PropertyDef(propertyName, objType, ci.abc, superProtectedNs),
                                    false,
                                    true,
                                    true,
                                    foundStatic);
                            if (sp != null) {
                                ...
                                propIndex = constants.getMultinameId(
                                        Multiname.createQName(
                                                false,
                                                constants.getStringId(propertyName, true),
                                                constants.getNamespaceId(
                                                        ns.kind,
                                                        ns.getName(sp.abc.constants),
                                                        sp.abc == abc
                                                                ? abc.constants.getNamespaceSubIndex(
                                                                        sp.trait.getName(sp.abc).namespace_index)
                                                                : 0,
                                                        true)),
                                        true);
                        ...
                        if (propType == null
                                && AVM2SourceGenerator.searchPrototypeChain(
                                        nsKeyword,
                                        namespaceSuffixInt,
                                        otherNs,
                                        localData.privateNs,
                                        localData.protectedNs,
                                        localData.staticProtectedNs,
                                        localData.internalNs,
                                        false,
                                        abcIndex,
                                        ftn.getWithoutLast(),
                                        ftn.getLast(),
                            ...
                            propIndex = constants.getMultinameId(
                                    Multiname.createQName(
                                            false,
                                            constants.getStringId(propertyName, true),
                                            namespaceSuffixInt != null
                                                    ? namespaceSuffixInt
                                                    : constants.getNamespaceId(
                                                            outPropNsKind.getVal(),
                                                            outPropNs.getVal(),
                                                            outPropNsIndex.getVal(),
                                                            true)),
                                    true);
                            ...
                            switch (ns.kind) {
                                case Namespace.KIND_PRIVATE:
                                    if (!(otherNs.contains(nsIndex) && propValueAbc == abc)) {
                                        throw new CompilationException(
                                                "Property " + propertyName + " has private access in "
                                                        + objType.toString(),
                                                line);
                                    }
                                    break;
                                case Namespace.KIND_PACKAGE_INTERNAL:
                                    if (!pkg.equals(localData.pkg.toRawString())) {
                                        throw new CompilationException(
                                                "Property " + propertyName + " has package internal access in "
                                                        + objType.toString(),
                                                line);
                                    }
                                    break;
                                ...
                                case Namespace.KIND_STATIC_PROTECTED:
                                    AbcIndexing.ClassIndex ci =
                                            abcIndex.findClass(thisType, abc, localData.scriptIndex);
                                    boolean found = false;
                                    String nsName =
                                            ns.getName(propValueAbc.constants).toRawString();
                                    while (ci != null) {
                                        DottedChain clsName = ci.abc
                                                .instance_info
                                                .get(ci.index)
                                                .getName(ci.abc.constants)
                                                .getNameWithNamespace(new HashSet<>(), ci.abc, ci.abc.constants, false);
                                        String clsNsName = clsName.isTopLevel()
                                                ? clsName.getLast()
                                                : clsName.getWithoutLast().toRawString() + ":" + clsName.getLast();
                                    ...
                                    if (!found) {
                                        throw new CompilationException(
                                                "Property " + propertyName + " has protected access in "
                                                        + objType.toString(),
                                                line);
                                    }
                                    break;
                ...
                if (objType == null) {
                    ...
                    for (int nsindex : openedNamespaceIds) {
                        ...
                        if (nsKind == Namespace.KIND_PACKAGE && propertyName != null) {
                            Reference<Boolean> foundStatic = new Reference<>(null);
                            AbcIndexing.TraitIndex p = abcIndex.findNsProperty(
                                    new AbcIndexing.PropertyNsDef(propertyName, nsname, abc, nsindex),
                                    true,
                                    true,
                                    foundStatic);
                            ...
                            if (p != null && (p.objType instanceof TypeItem)) {
                                ...
                                if (AVM2SourceGenerator.searchPrototypeChain(
                                        nsKeyword,
                                        namespaceSuffixInt,
                                        otherns,
                                        localData.privateNs,
                                        ...
                                        abcIndex,
                                        nsname,
                                        (((TypeItem) p.objType).fullTypeName.getLast()),
                                        propertyName,
                                        outName,
                                        outNs,
                                        outPropNs,
        ...
        if (propIndex == 0 && !mustExist) {
            ...
            if (attribute && pname.isEmpty()) {
                multiname = Multiname.createMultinameL(
                        true,
                        constants.getNamespaceSetId(
                                new int[] {
                                    constants.getNamespaceId(Namespace.KIND_PACKAGE_INTERNAL, localData.pkg, 0, true)
                                },
                                true));
            } else {
                int name_index = constants.getStringId(
                        "*".equals(pname) ? null : pname, true); // Note: name = * is for .@* attribute
    ...
    public List<GraphSourceItem> toSource(
            SourceGeneratorLocalData localData, SourceGenerator generator, boolean needsReturn)
            throws CompilationException {
        ...
        if (assignedValue != null) {
            ...
            if (obj instanceof AVM2Instruction
                    && (((AVM2Instruction) obj).definition instanceof FindPropertyStrictIns)) {
                return toSourceMerge(
                        localData,
                        generator,
                        ins(AVM2Instructions.GetLex, propertyId),
                        needsReturn ? null : ins(AVM2Instructions.Pop));
            }

            List<GraphSourceItem> onFalse = toSourceMerge(
                    localData,
                    generator,
            ...
            List<GraphSourceItem> result = toSourceMerge(
                    localData, generator, obj, ins(AVM2Instructions.PushNull), ins(AVM2Instructions.Equals), ifFalse);
     ...
     * @throws CompilationException On compilation error
     */
    public Object resolveObject(SourceGeneratorLocalData localData, SourceGenerator generator, boolean mustExist)
            throws CompilationException {
        Object obj = object;
        ...
        if (obj == null) {
            ...
            obj = ins(
                    mustExist ? AVM2Instructions.FindPropertyStrict : AVM2Instructions.FindProperty,
                    propIndex.getVal());
            // }
        }
        return obj;
    ...
    @Override
    public List<GraphSourceItem> toSourceChange(
            SourceGeneratorLocalData localData,
        ...
        if (isInteger) {
            ...
            changeIns =
                    ins(decrement ? AVM2Instructions.DecrementP : AVM2Instructions.IncrementP, localData.numberContext);
        } else {
            changeIns = ins(decrement ? AVM2Instructions.Decrement : AVM2Instructions.Increment);
        }

        List<GraphSourceItem> ret = toSourceMerge(
                localData,
                generator,
                obj,
                dupSetTemp(localData, generator, obj_temp),
                // Start get original
