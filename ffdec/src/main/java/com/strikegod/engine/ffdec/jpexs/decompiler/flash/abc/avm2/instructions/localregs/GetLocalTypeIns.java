 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.instructions.localregs;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.model.DecLocalAVM2Item;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.abc.avm2.model.IncLocalAVM2Item;
    ...
    public void translate(
        ...
        if (regId == 0) {
            if (localData.classIndex == -1) {
                stack.push(new ThisAVM2Item(
                        ins, localData.lineStartInstruction, DottedChain.parseNoSuffix("global"), false, false));
                return;
            }
            if ((localData.classIndex >= localData.getInstanceInfo().size())) {
                stack.push(
                        new ThisAVM2Item(ins, localData.lineStartInstruction, DottedChain.OBJECT /*?*/, false, false));
                return;
            }
            if (localData.isStatic) {
                stack.push(new ClassAVM2Item(
                        localData.getInstanceInfo().get(localData.classIndex).getName(localData.getConstants())));
            } else {
                ...
                stack.push(new ThisAVM2Item(
                        ...
                        m.getNameWithNamespace(
                                localData.usedDeobfuscations, localData.abc, localData.getConstants(), true),
                        isBasicObject,
                        false));
            }
            return;
            ...
            if (output.get(output.size() - 1) instanceof SetLocalAVM2Item) {
                ...
                if (setLocal.regIndex == regId) {
                    if (setLocal.getSrc() != null) {
                        if (localData
                                        .getSetLocalUsages(localData.code.adr2pos(
            ...
            if (lastOutput instanceof IncLocalAVM2Item) {
                ...
                if (inc.regIndex == regId) {
                    ...
                    stack.push(new PreIncrementAVM2Item(
                            lastOutput.getSrc(), lastOutput.getLineStartItem(), result, inc.type));
                    return;
                }
            }
            if (lastOutput instanceof DecLocalAVM2Item) {
                ...
                if (dec.regIndex == regId) {
                    ...
                    stack.push(new PreDecrementAVM2Item(
                            lastOutput.getSrc(), lastOutput.getLineStartItem(), result, dec.type));
                    return;
                }
            }
        }
                                    ...
                                    if (setLoc.value.value instanceof GetPropertyAVM2Item) {
                                        ...
                                        if (setProp.object instanceof DuplicateSourceItem) {
                                            ...
                                            if (getProp.object instanceof DuplicateItem) {
                                                ...
                                                if (output.size() >= 2
                                                        && output.get(output.size() - 2) instanceof SetTemporaryItem) {
                                                    SetTemporaryItem st =
                                                            (SetTemporaryItem) output.get(output.size() - 2);
                                                    if (st.tempIndex == d.tempIndex) {
                                                        ...
                                                        if (isIncrement) {
                                                            stack.push(new PostIncrementAVM2Item(
                                                                    setProp.value.getSrc(),
                                                                    setProp.value.getLineStartItem(),
                                                                    getProp,
                                                                    setLoc.value));
                                                        } else {
        ...
        if (!output.isEmpty()) {
            ...
            if ((output.get(output.size() - 1) instanceof SetTypeAVM2Item)) {
                ...
                if (setItem.value.getNotCoercedNoDup() instanceof SetLocalAVM2Item) {
                    ...
                    if (setLocal.regIndex == regId) {
                        int setLocalIp =
                                localData.code.adr2pos(setLocal.getSrc().getAddress());
                        if (localData.getSetLocalUsages(setLocalIp).size() == 1) {
                                            ...
                                            if (ds.tempIndex == d.tempIndex) {
                                                if (output.size() >= 2
                                                    ...
                                                    if (st.tempIndex == d.tempIndex) {
                                                        ...
                                                        if (isIncrement) {
                                                            stack.push(new PreIncrementAVM2Item(
                                                                    setLocal.value.getSrc(),
                                                                    setLocal.value.getLineStartItem(),
                                                                    getProp,
                                                                    TypeItem.NUMBER));
                                                        } else {
                                                            stack.push(new PreDecrementAVM2Item(
                            ...
                            if ((setItem.value instanceof CoerceAVM2Item)
                                    || (setItem.value instanceof ConvertAVM2Item)) {
