...
public class ShapeExporter {
    ...
    public List<File> exportShapes(
            AbortRetryIgnoreHandler handler,
            final String outdir,
            final SWF swf,
            ReadOnlyTagList tags,
            final ShapeExportSettings settings,
            EventListener evl,
            double unzoom,
            int aaScale)
        ...
        for (final Tag t : tags) {
            if (t instanceof ShapeTag) {
                ...
                final File file = new File(outdir
                        + File.separator
                        + Helper.makeFileName(st.getCharacterExportFileName() + settings.getFileExtension()));
                new RetryTask(
                                () -> {
                                    ...
                                    switch (settings.mode) {
                                        ...
                                        case CANVAS:
                                            try (OutputStream fos =
                                                ...
                                                CanvasShapeExporter cse = new CanvasShapeExporter(
                                                        st.getWindingRule(),
                                                        st.getShapeNum(),
                                                        null,
                                                ...
                                                fos.write(Utf8Helper.getBytes(cse.getHtml(
                                                        ...
                                                        SWF.getTypePrefix(st) + st.getCharacterId(),
                                                        st.getRect())));
                                            }
                                            break;
                                        case SWF:
