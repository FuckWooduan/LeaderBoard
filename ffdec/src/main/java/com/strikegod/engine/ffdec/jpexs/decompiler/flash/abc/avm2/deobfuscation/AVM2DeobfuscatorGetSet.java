...
public class AVM2DeobfuscatorGetSet extends SWFDecompilerAdapter {
     ...
     */
    protected boolean removeObfuscationGetSets(
            int classIndex,
            boolean isStatic,
            int scriptIndex,
            ABC abc,
     ...
     * @throws InterruptedException On interrupt
     */
    private void executeInstructions(MethodBody body, AVM2Code code, AVM2LocalData localData, int idx, int endIdx)
            throws InterruptedException {
        ...
        while (true) {
            ...
            if (def instanceof SetLocalTypeIns) {
                ...
                if (!stack.isEmpty()
                        && (stack.peek() instanceof LocalRegAVM2Item)
                        && (((LocalRegAVM2Item) stack.peek()).regIndex == regId)) {
                    stack.pop();
