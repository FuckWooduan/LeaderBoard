...
public class AVM2DeobfuscatorRegisters extends AVM2DeobfuscatorSimple {
     ...
     * @throws InterruptedException On interrupt
     */
    private void replaceSingleUseRegisters(
            Map<Integer, GraphTargetItem> singleRegisters,
            List<AVM2Instruction> setInss,
            int classIndex,
            boolean isStatic,
            int scriptIndex,
            ABC abc,
        ...
        for (int i = 0; i < code.code.size(); i++) {
            ...
            if (((setInss == null) || setInss.contains(ins)) && (ins.definition instanceof SetLocalTypeIns)) {
                ...
                if (singleRegisters.containsKey(regId)) {
                    code.replaceInstruction(
                            i, new AVM2Instruction(ins.getAddress(), DeobfuscatePopIns.getInstance(), null), body);
                }
            }
            ...
            if (ins.definition instanceof GetLocalTypeIns) {
                ...
                if (singleRegisters.containsKey(regId)) {
                    code.replaceInstruction(
                            i, cpool.makePush(singleRegisters.get(regId).getResult()), body);
                }
            }
        }
    }
            ...
            while (true) {
                ...
                if (ins.isBranch()) {
                    ...
                    for (int n = 1; n < branches.size(); n++) {
                        // visitCode(visited, (TranslateStack) stack.clone(), classIndex, isStatic, body, scriptIndex,
                        // abc, code, branches.get(n), endIdx, result);
