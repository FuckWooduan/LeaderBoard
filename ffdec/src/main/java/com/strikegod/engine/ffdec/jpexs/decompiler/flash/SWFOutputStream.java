...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.Amf3OutputStream;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.amf.amf3.ObjectTypeSerializeHandler;
...
public class SWFOutputStream extends OutputStream {
    ...
    public void writeFILLSTYLE(FILLSTYLE value, int shapeNum) throws IOException {
        ...
        if ((value.fillStyleType == FILLSTYLE.LINEAR_GRADIENT) || (value.fillStyleType == FILLSTYLE.RADIAL_GRADIENT)) {
     ...
     * @throws IOException On I/O error
     */
    private void writeSHAPERECORDS(List<SHAPERECORD> value, int fillBits, int lineBits, int shapeNum)
            throws IOException {
        for (SHAPERECORD sh : value) {
            if (sh instanceof CurvedEdgeRecord) {
                ...
                int numBits = Math.max(
                        getNeededBitsS(cer.controlDeltaX, cer.controlDeltaY, cer.anchorDeltaX, cer.anchorDeltaY) - 2,
                        0);
                if (Configuration._debugCopy.get()) {
                ...
                if (scr.stateNewStyles) {
                    ...
                    lineBits = getNeededBitsU(
                            shapeNum <= 3 ? scr.lineStyles.lineStyles.length : scr.lineStyles.lineStyles2.length);
     ...
     * @throws IOException On I/O error
     */
    public void writeTEXTRECORD(TEXTRECORD value, int defineTextNum, int glyphBits, int advanceBits)
            throws IOException {
        writeUB(1, 1);
        writeUB(3, 0);
     ...
     * @throws IOException On I/O error
     */
    public void writeBITMAPDATA(BITMAPDATA value, int bitmapFormat, int bitmapWidth, int bitmapHeight)
            throws IOException {
        int dataLen = 0;
        int pos = 0;
     ...
     * @throws IOException On I/O error
     */
    public void writeALPHABITMAPDATA(ALPHABITMAPDATA value, int bitmapFormat, int bitmapWidth, int bitmapHeight)
            throws IOException {
        int pos = 0;
        for (int y = 0; y < bitmapHeight; y++) {
     ...
     */
    public void writeAmf3Object(Amf3Value value, Map<String, ObjectTypeSerializeHandler> serializers)
