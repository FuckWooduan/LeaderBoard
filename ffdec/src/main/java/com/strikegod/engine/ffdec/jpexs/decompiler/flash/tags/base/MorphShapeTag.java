...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.morphshape.SVGMorphShapeExporter;
...
public abstract class MorphShapeTag extends DrawableTag {
    ...
    public SHAPEWITHSTYLE getShapeAtRatio(int ratio) {
        ...
        for (int startIndex = 0, endIndex = 0;
                startIndex < startEdges.shapeRecords.size() && endIndex < endEdges.shapeRecords.size();
                startIndex++, endIndex++) {
            ...
            if (edge1 instanceof CurvedEdgeRecord || edge2 instanceof CurvedEdgeRecord) {
                ...
                cer.controlDeltaX = cer1.controlDeltaX
                        + (int) Math.round((cer2.controlDeltaX - cer1.controlDeltaX) * ratio / (double) MAX_RATIO);
                cer.controlDeltaY = cer1.controlDeltaY
                        + (int) Math.round((cer2.controlDeltaY - cer1.controlDeltaY) * ratio / (double) MAX_RATIO);
                cer.anchorDeltaX = cer1.anchorDeltaX
                        + (int) Math.round((cer2.anchorDeltaX - cer1.anchorDeltaX) * ratio / (double) MAX_RATIO);
                cer.anchorDeltaY = cer1.anchorDeltaY
                        + (int) Math.round((cer2.anchorDeltaY - cer1.anchorDeltaY) * ratio / (double) MAX_RATIO);
    ...
    public void toSVG(SVGExporter exporter, ColorTransform colorTransform, double duration) {
        ...
        SVGMorphShapeExporter shapeExporter = new SVGMorphShapeExporter(
                getShapeNum(),
                swf,
                beginShapes,
                endShapes,
    ...
    public Shape getOutline(
        ...
        return transformation
                ...
                .createTransformedShape(
                        getShapeAtRatio(ratio).getOutline(fast, getShapeNum() == 2 ? 4 : 1, swf, stroked));
    }

    @Override
    public void toHtmlCanvas(StringBuilder result, double unitDivisor) {
        CanvasMorphShapeExporter cmse = new CanvasMorphShapeExporter(
                getShapeNum(), swf, getShapeAtRatio(0), getShapeAtRatio(MAX_RATIO), null, unitDivisor, 0, 0, 2);
        cmse.export();
    ...
    public void updateStartBounds() {
        startBounds = SHAPERECORD.getBounds(
                startEdges.shapeRecords,
                morphLineStyles.getStartLineStyles(getShapeNum()),
                getShapeNum() == 2 ? 4 : 3,
                false);
    }
    ...
    public void updateEndBounds() {
        endBounds = SHAPERECORD.getBounds(
                endEdges.shapeRecords,
                morphLineStyles.getEndLineStyles(getShapeNum()),
                getShapeNum() == 2 ? 4 : 3,
                false);
    }
