...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.importers.svg.SvgImporter;
...
public class ShapeImporter {
    ...
    public ImageTag addImage(Tag st, byte[] newData, int tagType) throws IOException {
        ...
        if (newData.length >= 4 && newData[0] == 'R' && newData[1] == 'I' && newData[2] == 'F' && newData[3] == 'F') {
            ...
            if (tag instanceof ShapeTag) {
                ...
                if (existingFilesForShapeTag.size() > 1) {
                    Logger.getLogger(ShapeImporter.class.getName())
                            .log(
                                    Level.WARNING,
                                    "Multiple matching files for shape tag {0} exists, {1} selected",
                                    new Object[] {
                                        characterId,
                                        existingFilesForShapeTag.get(0).getName()
                                    });
                }
                ...
                try {
                    ...
                    Logger.getLogger(ShapeImporter.class.getName())
                            .log(
                                    Level.WARNING,
                                    "Cannot import shape " + characterId + " from file " + sourceFile.getName(),
                                    ex);
                }
