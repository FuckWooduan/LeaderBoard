...
public class DefineFont2Tag extends FontTag {
    ...
    private void checkWideParameters() {
        ...
        if (!fontFlagsWideOffsets) {
            ...
            for (int i = 0; i < numGlyphs; i++) {
                long offset = ((glyphShapeTable.size() + 1 /*CodeTableOffset*/) * (fontFlagsWideOffsets ? 4 : 2)
                        + sos3.getPos());
                if (offset > 0xffff) {
    ...
    public synchronized void getData(SWFOutputStream sos) throws IOException {
        ...
        if (!swf.hasStrippedShapesFromFonts()) {
            ...
            SWFOutputStream sos3 = new SWFOutputStream(baosGlyphShapes, getVersion(), getCharset());
            for (int i = 0; i < numGlyphs; i++) {
                offsetTable.add((glyphShapeTable.size() + 1 /*CodeTableOffset*/) * (fontFlagsWideOffsets ? 4 : 2)
                        + sos3.getPos());
                sos3.writeSHAPE(glyphShapeTable.get(i), 1);
            }
            byte[] baGlyphShapes = baosGlyphShapes.toByteArray();
            ...
            if (!swf.hasStrippedShapesFromFonts() && numGlyphs > 0) {
    ...
    public synchronized boolean addCharacter(char character, Font font) {
        ...
        if (fontFlagsHasLayout) {
            Font advanceFont = font.deriveFont(
                    fontStyle,
                    1024); // Not multiplied with divider as it causes problems to create font with height around 20k
            ...
            if (adv > 0xFFFF) {
                ...
                Logger.getLogger(DefineFont2Tag.class.getName())
                        .log(
                                Level.WARNING,
                                "Character code {0} (glyph {1}) had advance value {2} which is larger than UI16. It was truncated to {3}.",
                                new Object[] {(int) character, pos, adv, 65535});
                adv = 0xFFFF;
            }
