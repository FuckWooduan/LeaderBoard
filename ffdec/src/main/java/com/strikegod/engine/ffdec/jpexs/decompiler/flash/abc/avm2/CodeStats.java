...
public class CodeStats {
    ...
    public GraphTextWriter toString(GraphTextWriter writer, ABC abc, List<DottedChain> fullyQualifiedNames) {
        writer.appendNoHilight("Stats: maxstack=" + maxstack + ", maxscope=" + maxscope + ", maxlocal=" + maxlocal)
                .newLine();
        int i = 0;
        int ms = 0;
        for (InstructionStats stats : instructionStats) {
            ...
            writer.appendNoHilight(i + ":" + stats.stackpos + (deltastack >= 0 ? "+" + deltastack : deltastack) + ","
                            + stats.scopepos + "    "
                            + stats.ins.toString(
                                    writer,
                                    LocalData.create(
