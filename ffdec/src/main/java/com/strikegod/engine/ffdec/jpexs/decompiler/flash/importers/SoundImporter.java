...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.tags.base.SoundImportException;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.tags.base.UnsupportedSamplingRateException;
...
public class SoundImporter {
     ...
     */
    public boolean importDefineSound(DefineSoundTag soundTag, InputStream is, int newSoundFormat)
        ...
        switch (newSoundFormat) {
                ...
                try (AudioInputStream audioIs = AudioSystem.getAudioInputStream(new BufferedInputStream(is))) {
                    ...
                    switch (newSoundRate) {
                        ...
                        case 44100:
                            ...
                            break;
                        default:
                            throw new UnsupportedSamplingRateException(
                                    newSoundRate, new int[] {5512, 11025, 22050, 44100});
                    }
            ...
            case SoundFormat.FORMAT_MP3:
                ...
                if (mp3data.length > ID3_V1_LENTGH) {
                    // ID3v1
                    if (mp3data[mp3data.length - ID3_V1_LENTGH] == 'T'
                            && mp3data[mp3data.length - ID3_V1_LENTGH + 1] == 'A'
                            && mp3data[mp3data.length - ID3_V1_LENTGH + 2] == 'G') {
                        mp3data = Arrays.copyOf(mp3data, mp3data.length - ID3_V1_LENTGH);
                        if (mp3data.length > ID3_V1_EXT_LENGTH) {
                            // ID3v1 extended
                            if (mp3data[mp3data.length - ID3_V1_EXT_LENGTH] == 'T'
                                    && mp3data[mp3data.length - ID3_V1_EXT_LENGTH + 1] == 'A'
                                    && mp3data[mp3data.length - ID3_V1_EXT_LENGTH + 2] == 'G'
                                    && mp3data[mp3data.length - ID3_V1_EXT_LENGTH + 3] == '+') {
                ...
                try {
                    ...
                    if (!snd.frames.isEmpty()) {
                        ...
                        switch (newSoundRate) {
                            ...
                            default:
                                throw new UnsupportedSamplingRateException(
                                        newSoundRate, new int[] {11025, 22050, 44100});
                        }
     ...
     */
    public boolean importSoundStream(SoundStreamHeadTypeTag streamHead, InputStream is, int newSoundFormat)
            throws UnsupportedSamplingRateException {
     ...
     * @throws UnsupportedSamplingRateException On unsupported sampling rate
     */
    public boolean importSoundStreamAtFrame(
            SoundStreamHeadTypeTag streamHead, InputStream is, int newSoundFormat, Integer startFrame)
            throws UnsupportedSamplingRateException {
        ...
        if (streamHead.getSoundFormatId() == SoundFormat.FORMAT_MP3) {
            ...
            if (!ranges.isEmpty()) {
                SWFInputStream sis;
                try {
                    sis = new SWFInputStream(
                            swf, ranges.get(0).blocks.get(0).streamSoundData.getRangeData());
        ...
        if (startFrame != null) {
            if (newSoundFormat != streamHead.getSoundFormatId()
                ...
                if (mp3data != null) {
                    ...
                    SoundFormat mp3SoundFormat =
                            new SoundFormat(SoundFormat.FORMAT_MP3, rateMap[newSoundRate], newSoundType);
                    try {
                        uncompressedSoundData =
                                mp3SoundFormat.decode(null, Arrays.asList(new ByteArrayRange(mp3data)), 0);
                    } catch (IOException ex) {
                        return false;
                    }
                    mp3Frames = null;
                ...
                if (newSoundRate > streamHead.getSoundRate()) {
                    for (int i = streamHead.getSoundRate(); i < newSoundRate; i++) {
                        wholeStreamUncompressedData =
                                toHigherRate(wholeStreamUncompressedData, newSoundSize, newSoundType);
                    }
    ...
    private void addStream(
        ...
        ByteArrayInputStream bais =
                uncompressedSoundData == null ? null : new ByteArrayInputStream(uncompressedSoundData);
     ...
     * @throws SoundImportException On sound import error
     */
    public boolean importSound(SoundTag soundTag, InputStream is, int newSoundFormat, Integer startFrame)
            throws SoundImportException {
        ...
        if (soundTag instanceof SoundStreamFrameRange) {
            return importSoundStreamAtFrame(
                    ((SoundStreamFrameRange) soundTag).getHead(),
                    is,
                    newSoundFormat,
                    ((SoundStreamFrameRange) soundTag).startFrame);
        }
        return false;
    }
    ...
    public int bulkImport(File soundDir, SWF swf, boolean printOut) {
        ...
        for (SoundTag tag : soundTags) {
                ...
                for (SoundStreamFrameRange r : ranges.get(pos)) {
                    for (File sourceFile : existingFilesForSoundTag) {
                        if (sourceFile.getName().startsWith("" + characterId + "_" + (r.startFrame + 1) + "-")) {
                            ...
                            try {
                                if (printOut) {
                                    System.out.println("Importing character " + characterId + ", start frame "
                                            + r.startFrame + " from file " + sourceFile.getName());
                                }
                                ...
                                Logger.getLogger(ShapeImporter.class.getName())
                                        .log(
                                                Level.WARNING,
                                                "Cannot import sound " + characterId + " from file "
                                                        + sourceFile.getName(),
                                                ex);
                            }
            ...
            if (existingFilesForSoundTag.size() > 1) {
                Logger.getLogger(SoundImporter.class.getName())
                        .log(
                                Level.WARNING,
                                "Multiple matching files for sound tag {0} exists, {1} selected",
                                new Object[] {
                                    characterId, existingFilesForSoundTag.get(0).getName()
                                });
            }
            ...
            try {
                ...
                Logger.getLogger(ShapeImporter.class.getName())
                        .log(
                                Level.WARNING,
                                "Cannot import sound " + characterId + " from file " + sourceFile.getName(),
                                ex);
            }
            if (CancellableWorker.isInterrupted()) {
                break;
