    ...
    private String getPattern(int bitmapId, Matrix matrix, ColorTransform colorTransform, boolean smoothed) {
        ...
        if (image != null) {
            ...
            if (img != null) {
                ...
                // Smoothed attribute was used in older FFDec versions - it is now only used for reading, for backwards
                // compatibility
                // pattern.setAttribute("ffdec:smoothed", smoothed ? "true" : "false");
                if (matrix != null) {
                    pattern.setAttribute(
                            "patternTransform",
                            matrix.getSvgTransformationString(SWF.unitDivisor / zoom, SWF.unitDivisor / zoom));
                }
    ...
    public void lineStyle(
        ...
        if (Configuration.useMinimumStrokeWidth1Px.get()) {
            ...
            if (thickness * displayZoom * thicknessScale < 1 * SWF.unitDivisor) {
                ...
                path.setAttribute(
                        "ffdec:original-stroke-width", Double.toString(thickness * displayZoom / SWF.unitDivisor));
