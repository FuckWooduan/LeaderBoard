 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.action.as2;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.action.model.ExtendsActionItem;
...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.action.model.ImplementsOpActionItem;
 ...
 *
 * This requires ActionClassHeaderCrippler is used on action list.
 *
 * @author JPEXS
 */
public class ActionScript2ClassDetectorCrippled {
     ...
     * Constructor.
     */
    public ActionScript2ClassDetectorCrippled() {}
    ...
    private GraphTargetItem setMemberToGetMember(GraphTargetItem item) {
        if (item instanceof SetMemberActionItem) {
            return new GetMemberActionItem(
                    null, null, ((SetMemberActionItem) item).object, ((SetMemberActionItem) item).objectName);
            ...
            if (v.name instanceof DirectValueActionItem) {
                if (((DirectValueActionItem) v.name).value instanceof String) {
                    if (((DirectValueActionItem) v.name).value.equals("_global")) {
                        GetVariableActionItem gvt =
                                new GetVariableActionItem(null, null, ((GetMemberActionItem) t).memberName);
                        if (lastMember == null) {
                            return gvt;
                        } else {
    ...
    private boolean checkClassContent(
        ...
        try {
                ...
                if (item instanceof StoreRegisterActionItem) {
                    ...
                    if (sr.value instanceof GetMemberActionItem) {
                        ...
                        if (gm.object instanceof TemporaryRegister) {
                            ...
                            if ((treg.value instanceof SetMemberActionItem)
            ...
            classNameTargetPath =
                    new GetVariableActionItem(null, null, new DirectValueActionItem(classNamePath.get(0)));
            for (int i = 1; i < classNamePath.size(); i++) {
                classNameTargetPath = new GetMemberActionItem(
                        null, null, classNameTargetPath, new DirectValueActionItem(classNamePath.get(i)));
            }
            ...
            for (; partsPos < parts.size(); partsPos++) {
                ...
                if (item instanceof SetMemberActionItem) {
                    ...
                    } else if ((sm.object instanceof DirectValueActionItem)
                            && (((DirectValueActionItem) sm.object).value instanceof RegisterNumber)) {
                        ...
                        for (; partsPos < parts.size(); partsPos++) {
                            ...
                            if (item instanceof ImplementsOpActionItem) {
                                ...
                                throw new AssertException(
                                        "unknown iface item: " + item.getClass().getSimpleName());
                            }
                        }
                    ...
                    if (currentRegId != instanceReg && currentRegId != classReg) {
                        ...
                        if (isPrototype
                                && ((sr.value instanceof NewMethodActionItem)
                                        || (sr.value instanceof NewObjectActionItem))) {
                            ...
                            throw new AssertException("invalid storeregister value: "
                                    + sr.value.getClass().getSimpleName());
                        }
                    }
                    ...
                    if ("addProperty".equals(pushMethodName)) {
                        ...
                        if (cm.arguments.size() != 3) {
                            throw new AssertException(
                                    "invalid number of arguments to addProperty: " + cm.arguments.size());
                        }
                        ...
                        if (propertyGetter instanceof GetMemberActionItem) {
                            int regId = getAsRegisterNum(
                                    ((GetMemberActionItem) propertyGetter).object, "getter member not register");
                            if (rnumObject != regId) {
                                throw new AssertException("getter register does not match property register " + regId
                                        + " <=> " + rnumObject);
                            }
                            String getterNameStr =
                                    getAsString(((GetMemberActionItem) propertyGetter).memberName, "getter memberName");
                            if (!(getterNameStr.equals("__get__" + propertyNameStr))) {
                                // throw new AssertException("getter does not match property name");
                                // Logger.getLogger(ActionScript2ClassDetector.class.getName()).warning(scriptPath + ":
                                // getter " + IdentifiersDeobfuscation.printIdentifier(false, getterNameStr) + " does
                                // not match property name " + IdentifiersDeobfuscation.printIdentifier(false,
                                // propertyNameStr));
                                continue;
                            }
                            ...
                            for (MyEntry<GraphTargetItem, GraphTargetItem> trait : traits) {
                                if (trait.getKey() instanceof DirectValueActionItem) {
                                    if (((DirectValueActionItem) trait.getKey()).isString()) {
                                        if (((DirectValueActionItem) trait.getKey())
                                                .toString()
                                                .equals(getterNameStr)) {
                            ...
                            if (!(getterFunc.actions.isEmpty()
                                    && getterFunc.functionName.isEmpty()
                                    && ((FunctionActionItem) propertyGetter).paramNames.isEmpty())) {
                                throw new AssertException("unexpected getter value for property " + propertyNameStr);
                            }
                            // we got empty getter
                        } else {
                            throw new AssertException("unexpected getter value for property " + propertyNameStr + ": "
                                    + propertyGetter.getClass().getSimpleName());
                        }

                        if (propertySetter instanceof GetMemberActionItem) {
                            int regId =
                                    getAsRegisterNum(((GetMemberActionItem) propertySetter).object, "setter member");
                            if (rnumObject != regId) {
                                throw new AssertException("setter register does not match property register " + regId
                                        + " <=> " + rnumObject);
                            }
                            String setterNameStr =
                                    getAsString(((GetMemberActionItem) propertySetter).memberName, "setter memberNAme");
                            if (!(setterNameStr.equals("__set__" + propertyNameStr))) {
                                // Logger.getLogger(ActionScript2ClassDetector.class.getName()).warning(scriptPath + ":
                                // setter " + IdentifiersDeobfuscation.printIdentifier(false, setterNameStr) + " does
                                // not match property name " + IdentifiersDeobfuscation.printIdentifier(false,
                                // propertyNameStr));
                                continue;
                                        ...
                                        if (((DirectValueActionItem) trait.getKey())
                                            ...
                                            if (trait.getValue() instanceof FunctionActionItem) {
                                                ...
                                                if (FunctionActionItem.DECOMPILE_GET_SET) {
                                                    ...
                                                    // There is return getter added at the end of every setter, gotta
                                                    // remove it, since it won't compile
                                                    ...
                                                    if (!func.actions.isEmpty()) {
                                                        ...
                                                        if (pos >= 0
                                                                && func.actions.get(pos) instanceof ReturnActionItem) {
                                                            GraphTargetItem val = func.actions.get(pos);
                                                            if (val.value instanceof CallMethodActionItem) {
                                                                if (((CallMethodActionItem) val.value).methodName
                                                                        instanceof DirectValueActionItem) {
                                                                    if (((CallMethodActionItem) val.value)
                                                                            .methodName
                                                                            .toString()
                                                                            .startsWith("__get__")) {
                            ...
                            if (!(setterFunc.actions.isEmpty()
                                    && setterFunc.functionName.isEmpty()
                                    && ((FunctionActionItem) propertySetter).paramNames.isEmpty())) {
                                throw new AssertException("unexpected getter value for property " + propertyNameStr);
                            }
                            // we got empty setter
                        } else {
                            throw new AssertException("unexpected setter value for property " + propertyNameStr + ": "
                                    + propertySetter.getClass().getSimpleName());
                        }
                    ...
                    if (funName.equals("ASSetPropFlags")) {
                        ...
                        if (cf.arguments.size() != 3) {
                            throw new AssertException("Invalid number of arguments to ASSetPropFlags:"
                                    + cf.arguments.size() + ", 3 expected");
                        }
                        ...
                        if ((obj instanceof DirectValueActionItem)
                                && (((DirectValueActionItem) obj).value instanceof RegisterNumber)) {
                            ...
                            if (path != null
                                    ...
                                    && "_global"
                                            .equals(path.get(
                                                    0))) { // For classes in toplevel package, there's _global in path
                        ...
                        if (!((props instanceof DirectValueActionItem)
                                && (((DirectValueActionItem) props).value == Null.INSTANCE))) {
                            throw new AssertException("ASSetPropFlags properties param not null");
                        }
                        if (!((flags instanceof DirectValueActionItem)
                                && (((DirectValueActionItem) flags).value == (Long) 1L))) {
            ...
            if (constructor != null) { // constructor should be there always, but just in case
                ...
                DirectValueActionItem classBaseName =
                        new DirectValueActionItem(classNamePath.get(classNamePath.size() - 1));
            ...
            if (uninitializedClassTraits.containsKey(fullClassName)) {
                ...
                for (String traitName :
                        uninitializedClassTraits.get(fullClassName).keySet()) {
            ...
            ClassActionItem clsItem =
                    new ClassActionItem(classNameTargetPath, extendsOp, implementsOp, traits, traitsStatic);
            for (int k = commandsStartPos; k <= commandsEndPos; k++) {
                commands.remove(commandsStartPos);
            }
    ...
    private void expandTernars(List<GraphTargetItem> commands) {
        for (int i = 0; i < commands.size(); i++) {
            if (commands.get(i) instanceof TernarOpItem) {
                ...
                commands.set(
                        i, new IfItem(ActionGraphTargetDialect.INSTANCE, null, null, ter.expression, onTrue, onFalse));
            }
        }
    }
     ...
     */
    private boolean checkIfVariants(
            Map<String, Map<String, Trait>> uninitializedClassTraits,
            List<GraphTargetItem> commands,
            HashMap<String, GraphTargetItem> variables,
            int pos,
        ...
        */
        ...
        for (int checkPos = pos; checkPos < commands.size(); checkPos++) {
            ...
            // _global.a.b.c = new Object();
            if ((t instanceof SetMemberActionItem)
                    && (((SetMemberActionItem) t).value instanceof NewObjectActionItem)) {
