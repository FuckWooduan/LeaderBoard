...
public class IggyIndexParser {
     ...
     */
    public static void parseIndex(
            boolean is64, ReadDataStreamInterface indexStream, List<Integer> indexTableEntry, List<Long> offsets)
            throws IOException {
            ...
            if (code < 0x80) { // 0-0x7F
                ...
                if (code >= indexTableSize) {
                    LOGGER.severe(String.format(
                            "< 0x80: index is greater than index_table_size. %x > %x", code, indexTableSize));
                    return;
                }
                ...
                if (index >= indexTableSize) {
                    LOGGER.severe(String.format(
                            "< 0xC0: index is greater than index_table_size. %x > %x", index, indexTableSize));
                    return;
                }
                ...
                LOGGER.finest(String.format(
                        "LENGTH = indexTable[index] * n = indexTable[%d] * %d = %d", index, n, indexTable[index] * n));
                offset += indexTable[index] * n;
            } else if (code < 0xD0) { // 0xC0-0xCF
                LOGGER.finest("0xC0-CF: code*2-0x17E");
                offset += ((code * 2) - 0x17E);
                LOGGER.finest(String.format(
                        "LENGTH = (code * 2) - 0x17E = (0x%02X * 2) - 0x17E = %d", code, ((code * 2) - 0x17E)));
                ...
                LOGGER.finer(String.format(
                        "stream.writeLengthCustom(%s,new int[]{%s},new int[]{%s}", n, locOffStr, platStr));
