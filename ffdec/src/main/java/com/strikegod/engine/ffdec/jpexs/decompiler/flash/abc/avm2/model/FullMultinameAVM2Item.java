...
public class FullMultinameAVM2Item extends AVM2Item {
    ...
    public boolean isTopLevel(
        ...
        String cname;
        if (name != null) {
            cname = name.toString(LocalData.create(
                    new ArrayList<>(),
                    null,
                    abc,
                    localRegNames,
            ...
            cname = (abc.constants
                    ...
                    .getName(new LinkedHashSet<>(), abc, abc.constants, fullyQualifiedNames, true, true));
        }
        String cns = "";
        if (namespace != null) {
            cns = namespace.toString(LocalData.create(
                    new ArrayList<>(),
                    null,
                    abc,
                    localRegNames,
    ...
    public GraphTextWriter appendTo(
            GraphTextWriter writer, LocalData localData, boolean afterDot, GraphTargetItem parentType, boolean isStatic)
            throws InterruptedException {
        if (namespace != null) {
        ...
        if (name != null) {
            ...
            if (multinameIndex > 0 && multinameIndex < constants.getMultinameCount()) {
                String simpleName = constants
                        .getMultiname(multinameIndex)
                        .getName(new HashSet<>(), localData.abc, constants, fullyQualifiedNames, true, false);
                if ("*".equals(simpleName)) {
                    ...
                    String namespaceSuffix =
                            constants.getMultiname(multinameIndex).getNamespaceSuffix();
                    if (!isAttribute
                            && afterDot
                            && namespaceSuffix.isEmpty()
                            && Configuration.as3QNameObfuscatedPropsInSquareBrackets.get()) {
                        // do not deobfuscate
                        localName = constants
                                .getMultiname(multinameIndex)
                                .getNameAndCustomNamespace(
                                        new HashSet<>(), localData.abc, fullyQualifiedNames, true, true, customNsRef);
                    } else {
                        localName = constants
                                .getMultiname(multinameIndex)
                                .getNameAndCustomNamespace(
                    ...
                    if (parentType instanceof TypeItem) { // not ApplyTypeAVM2Item or UnboundedTypeItem
                        ...
                        if (localData.abcIndex != null) {
                            isAmbiguous = localData.abcIndex.isPropertyAmbiguous(
                                    localData.abc, rawName, parentType, true, true);
                        }
                    }
                    ...
                    if (isAmbiguous == null || isAmbiguous) {
                        if (customNs != null) {
                            ...
                            String identifier = IdentifiersDeobfuscation.printIdentifier(
                                    localData.abc.getSwf(), localData.usedDeobfuscations, true, nsname);
                            writer.hilightSpecial(identifier, HighlightSpecialType.TYPE_NAME, customNs.toRawString());
                            writer.appendNoHilight("::");
                        } else if (isAmbiguous != null) {
                            Namespace ns =
                                    constants.getMultiname(multinameIndex).getSingleNamespace(localData.abc.constants);
                            if (ns != null) {
                    ...
                    if (!isAttribute
                        ...
                        if (IdentifiersDeobfuscation.isValidName(true, localName)) {
                            ...
                            if (localName.matches("^0|[1-9][0-9]*$")) {
                                ...
                                writer.append("[\"")
                                        .append(Helper.escapeActionScriptString(localName))
                                        .append("\"]");
                            }
                        }
                    } else {
    ...
    public boolean compareSame(FullMultinameAVM2Item other) {
        ...
        while (tiNameSpace instanceof LocalRegAVM2Item) {
            if (((LocalRegAVM2Item) tiNameSpace).computedValue != null) {
                tiNameSpace = ((LocalRegAVM2Item) tiNameSpace)
                        .computedValue
                        .getThroughNotCompilable()
                        .getThroughDuplicate();
            }
        ...
        while (tiNameSpace2 instanceof LocalRegAVM2Item) {
            if (((LocalRegAVM2Item) tiNameSpace2).computedValue != null) {
                tiNameSpace2 = ((LocalRegAVM2Item) tiNameSpace2)
                        .computedValue
                        .getThroughNotCompilable()
                        .getThroughDuplicate();
            }
