    ...
    protected void finalizePath() {
        if (path != null && pathData != null && pathData.length() > 0) {
            ...
            path.appendChild(createAnimateElement(
                    "d", pathData.toString().trim(), pathDataEnd.toString().trim()));
    ...
    protected void populateGradientElement(
        ...
        if (type == FILLSTYLE.LINEAR_GRADIENT) {
            ...
            if (focalPointRatio != 0 || focalPointRatioEnd != 0) {
                gradient.appendChild(createAnimateElement(
                        "fx", Double.toString(819.2 * focalPointRatio), Double.toString(819.2 * focalPointRatioEnd)));
            }
        }
        switch (spreadMethod) {
        ...
        for (int i = 0; i < gradientRecords.length; i++) {
            ...
            if (color instanceof RGBA) {
                ...
                RGBA colorAEnd = (RGBA) colorEnd;
                gradientEntry.appendChild(
                        createAnimateElement("stop-opacity", colorA.getAlphaFloat(), colorAEnd.getAlphaFloat()));
            }
