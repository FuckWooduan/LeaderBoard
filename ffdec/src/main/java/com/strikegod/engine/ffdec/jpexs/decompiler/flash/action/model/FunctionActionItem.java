 ...
 */
package com.strikegod.engine.ffdec.jpexs.decompiler.flash.action.model;
...
public class FunctionActionItem extends ActionItem {
    ...
    public FunctionActionItem(
            ...
            Map<Integer, String> regNames,
            List<GraphTargetItem> actions,
    ...
    public GraphTextWriter appendTo(GraphTextWriter writer, LocalData localData) throws InterruptedException {
        ...
        if (calculatedFunctionName != null) {
            ...
            if (!IdentifiersDeobfuscation.isValidName(false, fname)) {
                IdentifiersDeobfuscation.appendObfuscatedIdentifier(
                        localData.swf, localData.usedDeobfuscations, fname, writer);
            } else {
                writer.append(fname);
        ...
        for (int p = 0; p < paramNames.size(); p++) {
            ...
            if (!IdentifiersDeobfuscation.isValidName(false, pname)) {
                IdentifiersDeobfuscation.appendObfuscatedIdentifier(
                        localData.swf, localData.usedDeobfuscations, pname, writer);
            } else {
                writer.append(pname);
            }
        }
    ...
    private void getDeeplyUsedVariableNames(
            Set<String> topLevelDefinedVariableNames, FunctionActionItem fun, Set<String> deeplyUsedVariableNames) {
        ...
        for (VariableActionItem v : fun.variables) {
            if (!v.isDefinition()
                    ...
                    && topLevelDefinedVariableNames.contains(v.getVariableName())) {
        ...
        if (actions != null && !actions.isEmpty()) {
            ...
            for (VariableActionItem v : variables) {
                ...
                if (registerNames.contains(varName)) {
                    if (stored != null) {
                        v.setBoxedValue(new StoreRegisterActionItem(
                                null,
                                null,
                                new RegisterNumber(registerNames.indexOf(varName), varName),
                                stored,
                                false));
                    } else {
                        v.setBoxedValue(
                                new DirectValueActionItem(new RegisterNumber(registerNames.indexOf(varName), varName)));
                    }
                } else if (v.isDefinition()) {
                    v.setBoxedValue(new DefineLocalActionItem(
                            null, null, ((ActionSourceGenerator) generator).pushConstTargetItem(varName), stored));
                } else if (stored != null) {
                    v.setBoxedValue(new SetVariableActionItem(
                            null, null, ((ActionSourceGenerator) generator).pushConstTargetItem(varName), stored));
                } else {
                    v.setBoxedValue(new GetVariableActionItem(
                            null, null, ((ActionSourceGenerator) generator).pushConstTargetItem(varName)));
                }
            }
