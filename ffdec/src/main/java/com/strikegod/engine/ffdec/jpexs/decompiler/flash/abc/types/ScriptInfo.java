...
public class ScriptInfo {
    ...
    public DottedChain getSimplePackName(ABC abc, Set<String> usedDeobfuscations) {
        ...
        for (int j = 0; j < traits.traits.size(); j++) {
            ...
            if ((nskind == Namespace.KIND_PACKAGE_INTERNAL) || (nskind == Namespace.KIND_PACKAGE)) {
                packageTraits.add(j);
            }
        }
    ...
    public List<ScriptPack> getPacks(ABC abc, int scriptIndex, String packagePrefix, List<ABC> allAbcs) {
        ...
        if (ret.isEmpty()
                && !otherTraits.isEmpty()) { // no public/package internal traits to determine common pack name
        ...
        if (!isSimple) {
            ret.add(new ScriptPack(
                    new ClassPath(DottedChain.EMPTY, "script_" + scriptIndex, "", abc.getSwf()),
                    abc,
                    allAbcs,
                    scriptIndex,
                    new ArrayList<>()));
