...
public class PlaceObject4Tag extends PlaceObjectTypeTag implements ASMSourceContainer {
    ...
    public PlaceObject4Tag(
            SWF swf,
            boolean placeFlagMove,
            int depth,
            String className,
            int characterId,
            MATRIX matrix,
    ...
    public final void readData(
        ...
        if (sis.available() > 0) {
            try {
                ...
                Logger.getLogger(PlaceObject4Tag.class.getName())
                        .log(
                                Level.WARNING,
                                "AMFData in PlaceObject4 contains IExternalizable object which cannot be read. Data object is truncated.",
                                nse);
            }
        }
    }
        ...
        if (amfData != null && amfData.getValue() != null) {
            try {
                ...
                throw new IOException(
                        "Class \"" + ex.getClassName() + "\" implements IExternalizable, it cannot be saved");
            }
        }
    }
