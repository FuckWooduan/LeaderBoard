...
public class AbcMultiNameCollisionFixer {
    ...
    public int fixCollisions(ABC abc) {
        ...
        for (int multiNameIndex : collidingMultinameIndices) {
            ...
            int cnt = 0;
            while (!newNames.contains(selectedName)
                    && abc.constants.getStringId(selectedName, false)
                            > -1) { // already exists such name, but was not added during this collisionFixes
                cnt++;
