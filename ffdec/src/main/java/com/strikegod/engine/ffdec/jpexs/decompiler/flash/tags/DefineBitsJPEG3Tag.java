...
import com.strikegod.engine.ffdec.jpexs.helpers.JpegFixer;
...
/**
 * DefineBitsJPEG3 tag - Contains a JPEG image with alpha channel. (optionally a
 * PNG or GIF without the separate alpha channel)
 *
 * @author JPEXS
 */
...
public class DefineBitsJPEG3Tag extends ImageTag implements AloneTag, HasSeparateAlphaChannel {
    ...
    public InputStream getOriginalImageData() {
        ...
        try {
            jpegFixer.fixJpeg(
                    new ByteArrayInputStream(imageData.getArray(), imageData.getPos(), imageData.getLength()), baos);
    ...
    protected SerializableImage getImage() {
        try {
            ...
            try {
                jpegFixer.fixJpeg(
                        new ByteArrayInputStream(imageData.getArray(), imageData.getPos(), imageData.getLength()),
                        baos);
            } catch (IOException ex) {
                Logger.getLogger(DefineBitsJPEG3Tag.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    public Dimension getImageDimension() {
        ...
        try {
            ...
            ByteArrayInputStream bis = new ByteArrayInputStream(
                    imageData.getArray(), imageData.getPos() + errorLength, imageData.getLength() - errorLength);
