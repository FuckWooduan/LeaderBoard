...
public class SwfVsCodeExporter {
     ...
     */
    public void exportVsCodeProject(SWF swf, File outDir, boolean air, AbortRetryIgnoreHandler handler)
            throws IOException {
     ...
     * @throws IOException On I/O error
     */
    public void exportVsCodeProject(
            SWF swf, File outDir, boolean air, AbortRetryIgnoreHandler handler, EventListener eventListener)
            throws IOException {
        if (!swf.isAS3()) {
        ...
        try (FileOutputStream fos =
                new FileOutputStream(outDir.toPath().resolve("asconfig.json").toFile())) {
        ...
        String launch = "{\n"
                ...
                + "}";
        try (FileOutputStream fos =
                new FileOutputStream(vscodeDir.toPath().resolve("launch.json").toFile())) {
            fos.write(Utf8Helper.getBytes(launch));
        }

        String settings = "{\n" + "\t\"as3mxml.sdk.framework\": \"c:\\\\flex\"\n" + "}";
        try (FileOutputStream fos =
                new FileOutputStream(vscodeDir.toPath().resolve("settings.json").toFile())) {
        ...
        try (FileOutputStream fos = new FileOutputStream(
                srcDir.toPath().resolve(simpleNameNoSpaces + "-app.xml").toFile())) {
