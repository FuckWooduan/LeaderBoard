...
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.modes.MorphShapeExportMode;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.morphshape.CanvasMorphShapeExporter;
import com.strikegod.engine.ffdec.jpexs.decompiler.flash.exporters.settings.MorphShapeExportSettings;
...
public class MorphShapeExporter {
    ...
    public List<File> exportMorphShapes(
            AbortRetryIgnoreHandler handler,
            final String outdir,
            ReadOnlyTagList tags,
            final MorphShapeExportSettings settings,
            EventListener evl)
        ...
        for (final Tag t : tags) {
            if (t instanceof MorphShapeTag) {
                ...
                final File fileStart =
                        new File(outdir + File.separator + characterID + ".start" + settings.getFileExtension());
                final File fileEnd =
                        new File(outdir + File.separator + characterID + ".end" + settings.getFileExtension());
                                    ...
                                    switch (settings.mode) {
                                        case SVG_START_END:
                                            try (OutputStream fos =
                                                    new BufferedOutputStream(new FileOutputStream(fileStart))) {
                                                ...
                                                mst.getStartShapeTag()
                                                        .toSVG(0, 0, exporter, -2, new CXFORMWITHALPHA(), 0, m, m);
                                                fos.write(Utf8Helper.getBytes(exporter.getSVG()));
                                            }
                                            try (OutputStream fos =
                                                    new BufferedOutputStream(new FileOutputStream(fileEnd))) {
                                                ExportRectangle rect2 = new ExportRectangle(mst.getStartBounds());
                                                rect2.xMax *= settings.zoom;
                                                rect2.yMax *= settings.zoom;
                                                rect2.xMin *= settings.zoom;
                                        ...
                                        case SVG:
                                            try (OutputStream fos =
                                                ...
                                                SVGExporter exporter =
                                                        new SVGExporter(rect2, settings.zoom, "morphshape");
                                        ...
                                        case WEBP_START_END:
                                            ...
                                            int originalWidth =
                                                    (int) Math.ceil(rect.getWidth() * settings.zoom / SWF.unitDivisor);
                                            int originalHeight =
                                            ...
                                            SerializableImage img = new SerializableImage(
                                                    newWidth, newHeight, SerializableImage.TYPE_INT_ARGB_PRE);
                                            img.fillTransparent();
                                        ...
                                        case CANVAS:
                                            try (OutputStream fos =
                                                    new BufferedOutputStream(new FileOutputStream(file))) {
                                                int deltaX =
                                                        -Math.min(mst.getStartBounds().Xmin, mst.getEndBounds().Xmin);
                                                int deltaY =
                                                        -Math.min(mst.getStartBounds().Ymin, mst.getEndBounds().Ymin);
                                                CanvasMorphShapeExporter cse = new CanvasMorphShapeExporter(
                                                        mst.getShapeNum(),
                                                        ((Tag) mst).getSwf(),
                                                        mst.getShapeAtRatio(0),
                                                        mst.getShapeAtRatio(DefineMorphShapeTag.MAX_RATIO),
                                                        new CXFORMWITHALPHA(),
                                                        SWF.unitDivisor,
                                                        deltaX,
                                                        deltaY,
                                                ...
                                                baos.write(Utf8Helper.getBytes(
                                                        "var scalingGrids = {};\r\nvar boundRects = {};\r\n"));
                                                SWF.libraryToHtmlCanvas(ct.getSwf(), needed, baos);
                                                fos.write(Utf8Helper.getBytes(cse.getHtml(
                                                        new String(baos.toByteArray(), Utf8Helper.charset),
                                                        SWF.getTypePrefix(mst) + mst.getCharacterId(),
                                                        mst.getRect())));
                                            }
                                            break;
                                        case SWF:
                                            try (OutputStream fos =
                                                ...
                                                try {
                                                    new PreviewExporter()
                                                            .exportSwf(fos, mst, null, 0, false, settings.duration);
                                                } catch (ActionParseException ex) {
                                                    Logger.getLogger(MorphShapeExporter.class.getName())
                ...
                if (settings.mode.hasDuration()) {
                    iteratorNumFrames =
                            (int) Math.round(settings.duration * PreviewExporter.MORPH_SHAPE_ANIMATION_FRAME_RATE);
                ...
                MyFrameIterator frameImages =
                        new MyFrameIterator(mst, iteratorNumFrames, evl, usesTransparency, backgroundColor, settings);
                ...
                switch (settings.mode) {
                    case SVG_FRAMES:
                        ...
                        for (int f = 0; f < settings.numberOfFrames; f++) {
                            ...
                            File frameFile = new File(outdir + File.separator + fCharacterID + File.separator + (f + 1)
                                    + "_of_" + settings.numberOfFrames + settings.getFileExtension());

                            new RetryTask(
                                            () -> {
                                                try (OutputStream fos =
                                                        new BufferedOutputStream(new FileOutputStream(frameFile))) {
                                                    SVGExporter exporter =
                                                            new SVGExporter(rect2, settings.zoom, "shape");
                                                    mst.getShapeTagAtRatio(ratio)
                                                            .toSVG(0, 0, exporter, -2, new CXFORMWITHALPHA(), 0, m, m);
                                                    fos.write(Utf8Helper.getBytes(exporter.getSVG()));
                                                }
                                            },
                    ...
                    case GIF:
                        ...
                        new RetryTask(
                                        () -> {
                                            FrameExporter.makeGIF(
                                                    frameImages,
                                                    PreviewExporter.MORPH_SHAPE_ANIMATION_FRAME_RATE,
                                                    file,
                                                    evl);
                                            ret.add(file);
                                        },
                    ...
                    case WEBP:
                        ...
                        new RetryTask(
                                        () -> {
                                            FrameExporter.makeAnimatedWebP(
                                                    frameImages,
                                                    PreviewExporter.MORPH_SHAPE_ANIMATION_FRAME_RATE,
                                                    file,
                                                    evl);
                    ...
                    case WEBP_FRAMES:
                        ...
                        for (int i = 0; frameImages.hasNext(); i++) {
                            ...
                            new RetryTask(
                                            () -> {
                                                File frameFile = new File(outdir + File.separator + fCharacterID
                                                        + File.separator + (fi + 1) + "_of_" + settings.numberOfFrames
                    ...
                    for (String className : classNames) {
                        if (Configuration.autoDeobfuscateIdentifiers.get()) {
                            className = DottedChain.parseNoSuffix(className)
                                    .toPrintableString(new LinkedHashSet<>(), mst.getSwf(), true);
                        }
                        File classFile = new File(
                                outdir + File.separator + Helper.makeFileName(className + settings.getFileExtension()));
                        File classFileStart = new File(outdir
                                + File.separator
                                + Helper.makeFileName(className + ".start" + settings.getFileExtension()));
                        File classFileEnd = new File(outdir
                                + File.separator
                                + Helper.makeFileName(className + ".end" + settings.getFileExtension()));
                        new RetryTask(
                                        () -> {
                                            Files.copy(
                                                    file.toPath(),
        ...
        if (settings.mode == MorphShapeExportMode.CANVAS) {
            ...
            Helper.saveStream(
                    SWF.class
                            .getClassLoader()
                            .getResourceAsStream("com/strikegod/engine/ffdec/jpexs/helpers/resource/canvas.js"),
                    fcanvas);
            ret.add(fcanvas);
        }
        return ret;
